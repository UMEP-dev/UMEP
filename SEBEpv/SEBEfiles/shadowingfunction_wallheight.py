...
def shadowingfunction_wallheight(
    a,
    azimuth,
    altitude,
    scale,
    ...
    if vegdem is not None and vegdem2 is not None:
        ...
        g = np.copy(sh)
        bushplant = bush > 1
    elif (vegdem is not None and vegdem2 is None) or (
        vegdem is None and vegdem2 is not None
    ):
        raise ValueError(
            "Error with inputs, pass both vegdem and vegdem2 or neither, as args"
        )
    ...
    while True:
        ...
        if not (
            (amaxvalue >= dz) and (np.abs(dx) < sizex) and (np.abs(dy) < sizey)
        ):
            break
    ...
    if not usevegdem:  # shadowing_13 case
        facesun = np.logical_and(
            (facesh + (walls > 0).astype(int)) == 1, walls > 0
        ).astype(float)
