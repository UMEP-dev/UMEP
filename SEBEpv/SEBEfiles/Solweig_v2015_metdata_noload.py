...
def Solweig_2015a_metdata_noload(inputdata, location, UTC):
    ...
    dectimemin = met[:, 3] / 60.   # minutes in hours (float)
    ...
    print("ID, HH:MM, Sun altitude, Sun azimuth")
    for i, row in enumerate(met[:, 0]):
        ...
        half = datetime.timedelta(days=halftimestepdec)   # half of diffbetween 2 timesteps, in units of days
        ...
        print("%d, %d:%d, %.2f, %.2f" % (i, time['hour'], time['min'],
                                         90-sun["zenith"], sun["azimuth"])
              )
