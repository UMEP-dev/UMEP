...
def shadowingfunction_wallheight_13(
    a, azimuth, altitude, scale, walls, aspect
):
    """
    ...
    # NOTE: x and y coordinates are interchanged in comparison to sebeworker!!
    sizex = np.shape(a)[0]
    sizey = np.shape(a)[1]

    # initialise parameters
    f = np.copy(a)
    # for gnd-reflect; i.e. z-inverse dsm (former a2)
    a_inv = np.copy(a) * (-1)
    # for gnd-reflect; envelope around walls & a_inv
    a_inv_w = np.copy(a) * (-1) - walls
    # ground shadow volume (in direction of negative z)
    f_inv = np.copy(a_inv)
    dx = 0
    dy = 0
    dz = 0
    ...
    # this is enough! max(a) might be much larger, if e.g. min(a) = 200
    amaxvalue = np.max(a) - np.min(a)
    pibyfour = np.pi / 4
    ...
    if azilow < 0:  # azimuth (0;90)
        azilow += 2 * np.pi
        facesun = np.logical_or(aspect > azilow, aspect < azihigh).astype(
            float
        )
        ...
        facesun = np.logical_and(aspect > azilow, aspect < azihigh).astype(
            float
        )
    facesh = (facesun * -1) + 1
    ...
    while (amaxvalue >= dz) and (np.abs(dx) < sizex) and (np.abs(dy) < sizey):
        ...
        temp2[xp1:xp2, yp1:yp2] = (
            a_inv[xc1:xc2, yc1:yc2] - dz
        )  # temporary for moving ground view volume
        if index == 0:
            dz_buffer = 0
        else:
            dz_buffer = dz / index
        ...
        bool_hit_wall = np.logical_and(
            np.logical_and(temp2 > (a_inv_w - dz_buffer), temp2 <= f_inv),
            facesun,
        )
        bool_hit_wall[xp1:xp2, yp1:yp2] = np.logical_and(
            bool_hit_wall[xp1:xp2, yp1:yp2],
        ...
        status_hit_wall[xc1:xc2, yc1:yc2] = np.where(
            np.logical_and(
                bool_hit_wall[xp1:xp2, yp1:yp2],
                (status_hit_wall[xc1:xc2, yc1:yc2] < 2),
            ),
            1,
            status_hit_wall[xc1:xc2, yc1:yc2],
        )
        #   2nd cond: last time a point hit a wall, now not anymore
        status_hit_wall[xc1:xc2, yc1:yc2] = np.where(
            np.logical_and(
                np.logical_not(bool_hit_wall[xp1:xp2, yp1:yp2]),
        ...
        index = index + 1
    # profile of ground reflectance on wall
    wshinv = np.copy(f_inv) * (-1) - a

    sh = np.copy(f - a)  # shadow volume
    # Calc wallheight in shadow + remove walls in "self"-shadow
    wallsun = facesun * np.copy(walls - sh)
    wallsun[wallsun < 0] = 0
