            ...
            for k in range(len(met[:, 0])):
                ...
                if alt > 2:
                    ...
                    if output["energymonth"] == 1:
                        ...
                        radmatR[:, met[k, 1] + 2] = (
                            radmatR[:, met[k, 1] + 2] + G * (1 / 145) * albedo
                        )
                ...
                for i in range(skyvaultaltint.size):
                    for j in range(aziinterval[i]):
                        ...
                        suniroof = np.sin(slope) * np.cos(
                            ...
                            (radmatI[index, 1] * deg2rad) - aspect
                        ) + np.cos(
                            slope
                        ) * np.sin(
                            (radmatI[index, 0] * deg2rad)
                        ...
                        suniwall = np.sin(np.pi / 2) * np.cos(
                            ...
                            (radmatI[index, 1] * deg2rad) - dirwalls * deg2rad
                        ) + np.cos(
                            np.pi / 2
                        ) * np.sin(
                            (radmatI[index, 0] * deg2rad)
                        ...
                        if usevegdem == 1:
                            ...
                            sh, wallsh, wallsun, facesh, facesun, wallgndr = (
                                shadowingfunction_wallheight_13(
                                    a,
                                    radmatI[index, 1],
                                    radmatI[index, 0],
                        ...
                        wallsun = (
                            np.floor(wallsun * (1 / voxelheight)) * voxelheight
                        )
                        wallsh = (
                            np.floor(wallsh * (1 / voxelheight)) * voxelheight
                        )
                        wallgndr = (
                            np.floor(wallgndr * (1 / voxelheight))
                            * voxelheight
                        )
                        if usevegdem == 1:
                            wallshve = (
                        ...
                        for p in range(np.shape(wallmatrix)[0]):
                            ...
                            if (
                                usevegdem == 1
                                and wallshve[wallrow[p], wallcol[p]] > 0
                            ):
                                wallmatrix[
                                    p,
                                    0 : int(
