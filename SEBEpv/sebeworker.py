...
from SEBEfiles.shadowingfunction_wallheight_mr import (
    shadowingfunction_wallheight_13,
)
from SEBEfiles.shadowingfunction_wallheight_23 import (
...
class Worker(QtCore.QObject):
    ...
    def run(self):
        ...
        try:
            ...
            # total irradiance on ground/roof
            Energyyearroof = np.copy(Knight)
            # direct sun irradiance on ground/roof
            EnergyyearroofI = np.copy(Knight)
            # diffuse sky irradiance on ground/roof
            EnergyyearroofD = np.copy(Knight)
            # ground reflected irradiance on ground/roof
            EnergyyearroofR = np.copy(Knight)

            dectime = (
                met[:, 1] * 24 + met[:, 2] + met[:, 3] / 60.0
            )  # time stamp in units of hour (excluding year)
            # print(dectime)
            ...
            # Creating wallmatrix
            # row (y) and col (x) for each wall pixel
            wallcol, wallrow = np.where(np.transpose(walls) > 0)
            wallstot = np.floor(walls * (1 / voxelheight)) * voxelheight
            # finding tallest wall
            wallsections = np.floor(np.max(walls) * (1 / voxelheight))
            wallmatrix = np.zeros((np.shape(wallrow)[0], np.int(wallsections)))
            Energyyearwall = np.copy(wallmatrix)  # total irradiance on walls
            # direct sun irradiance on walls
            EnergyyearwallI = np.copy(wallmatrix)
            # diffuse sky irradiance on walls
            EnergyyearwallD = np.copy(wallmatrix)
            # ground reflected irradiance on walls
            EnergyyearwallR = np.copy(wallmatrix)
            ...
            if usevegdem == 1:
                ...
                vegrow, vegcol = np.where(vegdem > 0)
            ...
            for i in range(skyvaultaltint.size):
                for j in range(aziinterval[i]):
                    ...
                    # Shadow image
                    if usevegdem == 1:
                        vegsh, sh, _, _, wallsun, wallshve, _, facesun = (
                            shadowingfunction_wallheight_23(
                                dsm,
                                vegdem,
                                vegdem2,
                        ...
                        sh, _, wallsun, _, facesun, wallgndr = (
                            shadowingfunction_wallheight_13(
                                dsm,
                                hemiangles[index, 1],
                                hemiangles[index, 0],
                    ...
                    if index != 0:
                        shadow_all = np.append(
                            shadow_all, np.copy(shadow), axis=0
                        )
                        wallsun_all = np.append(
                            wallsun_all, np.copy(wallsun), axis=0
                        )
                        facesun_all = np.append(
                            facesun_all, np.copy(facesun), axis=0
                        )
                        wallgndr_all = np.append(
                            wallgndr_all, np.copy(wallgndr), axis=0
                        )
                        wallshve_all = np.append(
                            wallshve_all, np.copy(wallshve), axis=0
                        )
                    else:
            ...
            print(
                "Finished preparing shadow and ground reflection. Start simulations!"
            )
            # Main loop
            for k in range(len(met[:, 0])):
                # moved initialisation into for-loop (MRevesz):
                radinit = np.transpose(
                    np.vstack(
                        (iangle2, skyvaultazi, np.zeros((13, len(iangle2))))
                    )
                )
                ...
                # disp(alt)
                if (
                    alt > 2 and met[k, 14] > 0
                ):  # sun-altitude > 2 deg and G > 0 W/m2
                    ...
                    suniroof = np.sin(slope) * np.cos(alt * deg2rad) * np.cos(
                        (azi * deg2rad) - aspect
                    ...
                    suniwall = np.sin(np.pi / 2) * np.cos(
                        alt * deg2rad
                    ) * np.cos((azi * deg2rad) - dirwalls * deg2rad) + np.cos(
                        np.pi / 2
                    ) * np.sin(
                        (alt * deg2rad)
                    )
                    ...
                    lv, _, _ = Perez_v3(
                        90 - altitude[0, k], azimuth[0, k], D, I, jday[0, k], 1
                    )
                    ...
                    if output["energymonth"] == 1:
                        radmatI[azipos2, met[k, 1] + 2] = (
                            radmatI[azipos2, met[k, 1] + 2] + I
                        )
                        radmatD[:, met[k, 1] + 2] = (
                            radmatD[:, met[k, 1] + 2] + D * lv[:, 2]
                        )
                        radmatR[:, met[k, 1] + 2] = (
                            radmatR[:, met[k, 1] + 2]
                            + G * ((1 / 145) / 2) * albedo
                    ...
                    print(
                        "%d, %d:%d, %.2f, %.2f, %.1f, %.1f, %.1f"
                        % (k, met[k, 2], met[k, 3], alt, azi, G, I, D)
                    )
                    ...
                    for i in range(skyvaultaltint.size):
                        for j in range(aziinterval[i]):
                            ...
                            wallsun = (
                                np.floor(wallsun * (1 / voxelheight))
                                * voxelheight
                            )
                            if usevegdem == 1:
                                wallshve = (
                                    np.floor(wallshve * (1 / voxelheight))
                                    * voxelheight
                                )
                            ...
                            for p in range(np.shape(wallmatrix)[0]):
                                ...
                                if (
                                    wallsun[wrowp, wcolp] > 0
                                ):  # Sections in sun
                                    # Sections in sun
                                    if (
                                        wallsun[wrowp, wcolp]
                                        == wallstot[wrowp, wcolp]
                                    ):
                                        wallmatrix[
                                            p,
                                            0 : int(
                    ...
                    for i in range(skyvaultaltint.size):
                        for j in range(aziinterval[i]):
                            ...
                            if radmatD[index, 0] < 90:
                                alt_index = np.where(
                                    skyvaultaltint == radmatD[index, 0]
                                )[0][0]
                                delta_azi = skyvaultaziint[alt_index]

                                cot_alt_outer = 1 / np.tan(
                                    skyvaultaltint[alt_index] * deg2rad
                                )
                                cot_alt_inner = 1 / np.tan(
                                    skyvaultaltint[alt_index + 1] * deg2rad
                                )
                                cotsq_alt_outer = cot_alt_outer**2
                                cotsq_alt_inner = cot_alt_inner**2

                                # following radii are r/z_wall:
                                radi_xy_half = np.sqrt(
                                    # at circ segment/2
                                    (cotsq_alt_outer + cotsq_alt_inner)
                                    / 2
                                )
                                # at center of outer area
                                radi_outer = np.sqrt(
                                    3 * cotsq_alt_outer + cotsq_alt_inner
                                )
                                # at center of inner area
                                radi_inner = np.sqrt(
                                    cotsq_alt_outer + 3 * cotsq_alt_inner
                                )
                                ...
                                for p in range(shape_wallmatrix[0]):
                                    ...
                                    if wallgndr[wrowp, wcolp] > 0:
                                        ...
                                        r_xy_half = (
                                            np.copy(wall_gndr_idx)
                                            * radi_xy_half
                                        )
                                        r_xy_inner = (
                                            np.copy(wall_gndr_idx) * radi_inner
                                        )
                                        r_xy_outer = (
                                            np.copy(wall_gndr_idx) * radi_outer
                                        )
                                        ...
                                        try:
                                            slope_at_xy = np.where(
                                                walls[y, x] > 0, 0, slope[y, x]
                                            )
                                        except IndexError:
                                        ...
                                        gndIrrad = np.copy(wall_gndr_idx) * 0.0
                                        ...
                                        for patches in range(4):
                                            ...
                                            tmpgndIrrad = (
                                                0  # reinitiate for testing
                                            )
                                            try:
                                                tmpgndIrrad = np.where(
                                                    invalidcoords,
                                                    0,
                                                    Energyroof[y, x],
                                                )
                                        ...
                                        gndIrrad *= 0.25
                                        # commented for testing!! UNcomment for usage!
                                        wallmatrix[
                                            p, 0:max_wallidx
                                        ] += gndIrrad
                                        ...
                                        r_wg_sq = (
                                            r_wg_x**2 + r_wg_y**2 + r_wg_z**2
                                        )

                                        rn_wg = np.where(
                                            rn_w < 0, 0, rn_w
                                        ) * np.where(rn_g > 0, 0, rn_g)

                                        # commented for testing!! UNcomment for usage! New, corrected version!!
                                        wallmatrix[p, 0:max_wallidx] *= (
                                            d_area_true * rn_wg / (r_wg_sq**2)
                                        )
                    ...
                    time2 = time.time()
                    print(
                        "t1=%.2f, t2=%.2f, dt=%.2f, dt-alb=%.2f"
                        % (time1, time2, (time2 - time1), ddt)
                    )
                    ...
                    Energyyearwall = (
                        Energyyearwall + np.copy(Energywall) * timedelta
                    )
                    EnergyyearwallI = (
                        EnergyyearwallI + np.copy(EnergywallI) * timedelta
                    )
                    EnergyyearwallD = (
                        EnergyyearwallD + np.copy(EnergywallD) * timedelta
                    )
                    EnergyyearwallR = (
                        EnergyyearwallR + np.copy(EnergywallR) * timedelta
                    )
                    ...
                    Energyyearroof = (
                        Energyyearroof + np.copy(Energyroof) * timedelta
                    )
                    EnergyyearroofI = (
                        EnergyyearroofI + np.copy(EnergyroofI) * timedelta
                    )
                    EnergyyearroofD = (
                        EnergyyearroofD + np.copy(EnergyroofD) * timedelta
                    )
                    EnergyyearroofR = (
                        EnergyyearroofR + np.copy(EnergyroofR) * timedelta
                    )
                ...
                if self.dlg.checkBoxSaveIntermedi.isChecked():
                    ...
                    time_str = (
                        "{0:.0f}{1:03.0f}-{2:02.0f}{3:02.0f}-{4}".format(
                            met[k, 0], met[k, 1], met[k, 2], met[k, 3], k
                        )
                    )
                    filenamewall = "Energywall-" + time_str + ".txt"
                    self.save_intermediate_res(
                        Energywall, filenamewall, wallcol, wallrow, timedelta
                    )

                    filenamewall = "EnergywallI-" + time_str + ".txt"
                    self.save_intermediate_res(
                        EnergywallI, filenamewall, wallcol, wallrow, timedelta
                    )

                    filenamewall = "EnergywallD-" + time_str + ".txt"
                    self.save_intermediate_res(
                        EnergywallD, filenamewall, wallcol, wallrow, timedelta
                    )

                    filenamewall = "EnergywallR-" + time_str + ".txt"
                    self.save_intermediate_res(
                        EnergywallR, filenamewall, wallcol, wallrow, timedelta
                    )
                else:
                    pass
            ...
            Energyyearwall = np.transpose(
                np.vstack(
                    (wallrow + 1, wallcol + 1, np.transpose(Energyyearwall))
                )
            )
            # adding 1 to wallrow and wallcol so that the tests pass
            EnergyyearwallI = np.transpose(
                np.vstack(
                    (wallrow + 1, wallcol + 1, np.transpose(EnergyyearwallI))
                )
            )
            # adding 1 to wallrow and wallcol so that the tests pass
            EnergyyearwallD = np.transpose(
                np.vstack(
                    (wallrow + 1, wallcol + 1, np.transpose(EnergyyearwallD))
                )
            )
            # adding 1 to wallrow and wallcol so that the tests pass
            EnergyyearwallR = np.transpose(
                np.vstack(
                    (wallrow + 1, wallcol + 1, np.transpose(EnergyyearwallR))
                )
            )
            ...
            print(
                ...
                % (finaltime1, finaltime2, (finaltime2 - finaltime1))
            )

            seberesult = {
                "PVEnergyyearroof": Energyyearroof,
                "PVEnergyyearwall": Energyyearwall,
                "vegdata": vegdata,
                "EnergyyearroofI": EnergyyearroofI,
                "EnergyyearroofD": EnergyyearroofD,
                "EnergyyearroofR": EnergyyearroofR,
                "EnergyyearwallI": EnergyyearwallI,
                "EnergyyearwallD": EnergyyearwallD,
                "EnergyyearwallR": EnergyyearwallR,
            }
    ...
    def save_intermediate_res(
        self, array, filename, wallcol, wallrow, timedelta
    ):
        """
