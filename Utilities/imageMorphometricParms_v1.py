...
def imagemorphparam_v2(dsm, dem, scale, mid, dtheta, dlg, imp_point):
    ...
    try:
        build = dsm - dem
        build[(build < 2.0)] = 0.0  # building should be higher than 2 meter
        ...
        imid = np.floor((n / 2.0))
        if mid == 1:
            # half the length of the grid (y)
            dY = np.int16(np.arange(np.dot(1, imid)))
        else:
            # the whole length of the grid (y)
            dY = np.int16(np.arange(np.dot(1, n)))
        if imp_point == 1:
            dlg.progressBar.setRange(0.0, 360.0 / dtheta)
        ...
        j = int(0)
        for angle in np.arange(0, (360.0 - dtheta + 0) + dtheta, dtheta):
            if imp_point == 1:
            ...
            if dem.sum() != 0:  # ground heights
                ...
                d = sc.rotate(
                    build, angle, order=0, reshape=False, mode="nearest"
                )
