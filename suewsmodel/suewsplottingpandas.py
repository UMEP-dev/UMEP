...
def make_dectime(dataout):
    ...
    for i in range(0, dataout.shape[0]):  # making date number
        datenum_yy[i] = dt.date2num(
            dt.datetime.datetime(int(dataout[i, 0]), 1, 1)
        )
...
class SuewsPlottingPandas(object):
    ...
    def plotforcing(self, df_forcing):
        ...
        df_plot_forcing_x = (
            df_forcing.loc[:, list_var_forcing]
            .copy()
            .shift(-1)
            .dropna(how="any")
        )
        df_plot_forcing = df_plot_forcing_x.resample("1h").mean()
        df_plot_forcing["rain"] = (
            df_plot_forcing_x["rain"].resample("1h").sum()
        )
    ...
    def plotbasic(self, suews_ouput_path, grid):
        ...
        # energy balance
        ax_output = (
            df_output_suews.loc[:, ["QN", "QS", "QE", "QH", "QF"]]
    ...
    def plotmonthly(self, suews_ouput_path, grid):
        ...
        rsmp_1M = (
            df_plot.shift(-1).dropna(how="all").resample("1M", kind="period")
        )
        # mean values
        ...
        df_1M_mean.loc[:, ["QN", "QS", "QE", "QH", "QF"]].rename(
