...
def make_dectime(df_output_suews):
    ...
    for i in range(0, df_output_suews.shape[0]):  # making date number
        datenum_yy[i] = dt.date2num(
            dt.datetime.datetime(int(df_output_suews[i, 0]), 1, 1)
        )
...
def make_datetime(df_output_suews):
    ...
    for i in range(0, df_output_suews.shape[0]):  # making date number
        datenum_yy.append(
            dt.datetime.datetime(int(df_output_suews[i, 0]), 1, 1)
            + datetime.timedelta(
                days=df_output_suews[i, 1] - 1,
                hours=df_output_suews[i, 2],
                minutes=df_output_suews[i, 3],
            )
        )
...
class SuewsPlotting(object):
    ...
    def plotbasic(self, df_output_suews, df_met_forcing):
        ...
        plt.title(
            "Model output of radiation balance, energy budget and water balance"
        )
        ...
        ax2.set_ylim(
            [
                -200,
                df_output_suews.loc[:, ["QE", "QS", "QH", "QF"]].max().max(),
            ]
        )
        ...
        ax3.legend(
            bbox_to_anchor=(1.05, 0.5), loc="upper left", borderaxespad=0.0
        )
        ax3.set_xlabel("Time", fontsize=14)
        ax3.set_ylabel("$LAI$", color="g", fontsize=14)
        ax3.set_xlim(
            [df_output_suews.index.min(), df_output_suews.index.max()]
        )
        ...
        ax4.set_ylim(
            [
                0,
                max(
                    df_met_forcing["rain"].max(), df_output_suews["SMD"].max()
                ),
            ]
        )
    ...
    def plotmonthlystatistics(self, df_output_suews, datain):
        ...
        df_monthly_mean = (
            df_output_suews[["QH", "QE", "QS", "QF", "QN"]]
            .groupby(df_output_suews["month"])
            .mean()
        )

        df_monthly_sum = (
            df_output_suews[["Rain", "WUInt", "Evap", "RO", "TotCh"]]
            .groupby(df_output_suews["month"])
            .sum()
        )
        ...
        df_monthly_mean["QF"].plot(
            style="o-", color="c", label="$Q_F$", ax=ax1
        )
        (-df_monthly_mean["QH"]).plot(
            style="o-", color="r", label="$Q_H$", ax=ax1
        )
        (-df_monthly_mean["QE"]).plot(
            style="o-", color="b", label="$Q_E$", ax=ax1
        )
        (-df_monthly_mean["QS"]).plot(
            style="o-", color="k", label=r"$\Delta Q_S$", ax=ax1
        )
        ax1.set_xlim(1, 12)
        ...
        (-df_monthly_sum["WUInt"]).plot(
            style="o-", color="g", label="$W-use$", ax=ax3
        )
        (-df_monthly_sum["TotCh"]).plot(
            style="o-", color="r", label="$Storage$", ax=ax3
        )
        (-df_monthly_sum["Evap"]).plot(
            style="o-", color="b", label="$Evap$", ax=ax3
        )
        (-df_monthly_sum["RO"]).plot(
            style="o-", color="k", label="$Runoff$", ax=ax3
        )
        ax3.bar(
            df_monthly_sum.index,
            df_monthly_sum["Rain"],
            width=0.5,
            edgecolor="b",
            align="center",
