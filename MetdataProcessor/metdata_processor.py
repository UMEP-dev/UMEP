...
class MetdataProcessor(object):
    ...
    def __init__(self, iface):
        ...
        locale_path = os.path.join(
            self.plugin_dir, "i18n", "MetdataProcessor_{}.qm".format(locale)
        )
        ...
        self.actions = []
        self.menu = self.tr("&Metdata processor")
    ...
    def initGui(self):
        ...
        self.add_action(
            icon_path,
            text=self.tr("Metdata processor"),
            callback=self.run,
    ...
    def unload(self):
        ...
        for action in self.actions:
            self.iface.removePluginMenu(self.tr("&Metdata processor"), action)
            ...
            if result == 1:
                try:
                    ...
                    QMessageBox.critical(
                        self.dlg,
                        "Error: Check the number of columns in each line",
                        str(e),
                    )
                    return
        else:
            ...
            if result == 1:
                ...
                try:
                    ...
                    QMessageBox.information(
                        self.dlg,
                        "File imported",
                        "If invalid data was detected such as strings or "
                        "other non-numrical characters, these data points could "
                        "result in that the MetdataProcessor "
                        "will fail to create your UMEP-formatted inputdata.",
                    )  # , 'Continue'
                except Exception as e:
                    QMessageBox.critical(
                        self.dlg,
    ...
    def start_progress(self):
        ...
        if self.dlg.checkBoxEPW.isChecked():
            ...
            if norain == 0:
                QMessageBox.critical(
                    None,
                    "Value error",
                    "No precipitation found in EPW-file. Find alternative "
            ...
            if self.dlg.checkBoxDectime.isChecked():
                ...
                timeres_old = np.round(
                    (met_old[1, dectime_col] - met_old[0, dectime_col])
                    * (60.0 * 24.0)
                )
                ...
                first_hour = (
                    met_old[0, dectime_col] - np.floor(met_old[0, dectime_col])
                ) * 24
                met_new[0, 2] = first_hour
                first_min = np.round(
                    (met_new[0, 2] - np.floor(met_new[0, 2])) * 60
                )
                ...
                met_new[:, 2] = met_old[
                    :, self.dlg.comboBox_hour.currentIndex()
                ]
                met_new[:, 3] = met_old[
                    :, self.dlg.comboBox_minute.currentIndex()
                ]
            ...
            for i in range(0, met_new.shape[0] - 1):
                dectime0 = (
                    ...
                    + met_new[i, 1]
                    + met_new[i, 2] / 24.0
                    + met_new[i, 3] / (60.0 * 24.0)
                )
                dectime1 = (
                    met_new[i + 1, 0]
                    + met_new[i + 1, 1]
                    + met_new[i + 1, 2] / 24.0
                    + met_new[i + 1, 3] / (60.0 * 24.0)
                )
                timeres_old = np.round((dectime1 - dectime0) * (60.0 * 24.0))
                nshtest = int(timeres_old / 5)
                if nshtest > nsh:
                    QMessageBox.critical(
                        None,
                        "Input data is not continuous",
                        "There seems to be a time gap at line:"
                        " \n" + str(i + 1),
                    )
                    return
            ...
            if self.dlg.checkBox_kdown.isChecked():
                met_new[:, 14] = met_old[
                    :, self.dlg.comboBox_kdown.currentIndex()
                ]
                if self.dlg.checkBoxQuality.isChecked():
                    testwhere = np.where(
                        (met_new[:, 14] < 0.0) | (met_new[:, 14] > 2200.0)
                    )
            ...
            if self.dlg.checkBox_Tair.isChecked():
                met_new[:, 11] = met_old[
                    :, self.dlg.comboBox_Tair.currentIndex()
                ]
                if self.dlg.checkBoxQuality.isChecked():
                    testwhere = np.where(
                        (met_new[:, 11] < -30.0) | (met_new[:, 11] > 55.0)
                    )
                    if testwhere[0].__len__() > 0:
                        QMessageBox.critical(
                            None,
            ...
            if self.dlg.checkBox_RH.isChecked():
                met_new[:, 10] = met_old[
                    :, self.dlg.comboBox_RH.currentIndex()
                ]
                if self.dlg.checkBoxQuality.isChecked():
                    testwhere = np.where(
                        (met_new[:, 10] < 0.00) | (met_new[:, 10] > 100.0)
                    )
                    if testwhere[0].__len__() > 0:
                        QMessageBox.critical(
                            None,
                            "Value error",
                            "Relative humidity - beyond what is expected at line:"
                            " \n" + str(testwhere[0] + 1),
                        )
                        return
            else:
            ...
            if self.dlg.checkBox_pres.isChecked():
                met_new[:, 12] = met_old[
                    :, self.dlg.comboBox_pres.currentIndex()
                ]
                if self.dlg.checkBoxQuality.isChecked():
                    testwhere = np.where(
                        (met_new[:, 12] < 30.0) | (met_new[:, 12] > 107.0)
                    )
                    if testwhere[0].__len__() > 0:
                        QMessageBox.critical(
                            None,
                            "Value error",
                            "Pressure - beyond what is expected at line:"
                            " \n" + str(testwhere[0] + 1),
                        )
                        return
            else:
            ...
            if self.dlg.checkBox_rain.isChecked():
                met_new[:, 13] = met_old[
                    :, self.dlg.comboBox_rain.currentIndex()
                ]
                if self.dlg.checkBoxQuality.isChecked():
                    testwhere = np.where(
                        ((met_new[:, 13] / nsh) < 0.0)
                    ...
                    if testwhere[0].__len__() > 0:
                        QMessageBox.critical(
                            None,
                            "Value error",
                            "Rain - beyond what is expected at line:"
                            " \n" + str(testwhere[0] + 1),
                        )
                        return
            else:
            ...
            if self.dlg.checkBox_snow.isChecked():
                met_new[:, 15] = met_old[
                    :, self.dlg.comboBox_snow.currentIndex()
                ]
                if self.dlg.checkBoxQuality.isChecked():
                    testwhere = np.where(
                        ((met_new[:, 15] / nsh) < 0.0)
                        | ((met_new[:, 15] / nsh) > 300.0)
                    )
                    if testwhere[0].__len__() > 0:
                        QMessageBox.critical(
                            None,
                            "Value error",
                            "Snow - beyond what is expected at line:"
                            " \n" + str(testwhere[0] + 1),
                        )
                        return
            else:
            ...
            if self.dlg.checkBox_ldown.isChecked():
                met_new[:, 16] = met_old[
                    :, self.dlg.comboBox_ldown.currentIndex()
                ]
                if self.dlg.checkBoxQuality.isChecked():
                    testwhere = np.where(
                        (met_new[:, 16] < 100.0) | (met_new[:, 16] > 600.0)
                    )
                    if testwhere[0].__len__() > 0:
                        QMessageBox.critical(
                            None,
                            "Value error",
                            "Ldown - beyond what is expected at line:"
                            " \n" + str(testwhere[0] + 1),
                        )
                        return
            else:
            ...
            if self.dlg.checkBox_fcld.isChecked():
                met_new[:, 17] = met_old[
                    :, self.dlg.comboBox_fcld.currentIndex()
                ]
                if self.dlg.checkBoxQuality.isChecked():
                    testwhere = np.where(
                        (met_new[:, 17] < 0.0) | (met_new[:, 17] > 1.01)
                    )
                    if testwhere[0].__len__() > 0:
                        QMessageBox.critical(
                            None,
                            "Value error",
                            "Fraction of cloud - beyond what is expected at line:"
                            " \n" + str(testwhere[0] + 1),
                        )
                        return
            else:
            ...
            if self.dlg.checkBox_Wuh.isChecked():
                met_new[:, 18] = met_old[
                    :, self.dlg.comboBox_Wuh.currentIndex()
                ]
                if self.dlg.checkBoxQuality.isChecked():
                    testwhere = np.where(
                        ((met_new[:, 18] / nsh) < 0.0)
                        | ((met_new[:, 18] / nsh) > 10.01)
                    )
                    if testwhere[0].__len__() > 0:
                        QMessageBox.critical(
                            None,
                            "Value error",
                            "External water use - beyond what is expected at line:"
                            " \n" + str(testwhere[0] + 1),
                        )
                        return
            else:
            ...
            if self.dlg.checkBox_xsmd.isChecked():  # response to #669
                met_new[:, 19] = met_old[
                    :, self.dlg.comboBox_xsmd.currentIndex()
                ]
                if self.dlg.checkBoxQuality.isChecked():
                    testwhere = np.where(
                        (met_new[:, 19] < 0.01) | (met_new[:, 19] > 0.5)
                    )
                    if testwhere[0].__len__() > 0:
                        QMessageBox.critical(
                            None,
                            "Value error",
                            "Soil moisture - beyond what is expected at line:"
                            " \n" + str(testwhere[0] + 1),
                        )
                        return
            else:
            ...
            if self.dlg.checkBox_lai.isChecked():
                met_new[:, 20] = met_old[
                    :, self.dlg.comboBox_lai.currentIndex()
                ]
                if self.dlg.checkBoxQuality.isChecked():
                    testwhere = np.where(
                        (met_new[:, 20] < 0.0) | (met_new[:, 20] > 15.01)
                    )
                    if testwhere[0].__len__() > 0:
                        QMessageBox.critical(
                            None,
                            "Value error",
                            "Leaf area index - beyond what is expected at line:"
                            " \n" + str(testwhere[0] + 1),
                        )
                        return
            else:
            ...
            if self.dlg.checkBox_kdiff.isChecked():
                met_new[:, 21] = met_old[
                    :, self.dlg.comboBox_kdiff.currentIndex()
                ]
                if self.dlg.checkBoxQuality.isChecked():
                    testwhere = np.where(
                        (met_new[:, 21] < 0.0) | (met_new[:, 21] > 800.0)
                    )
                    if testwhere[0].__len__() > 0:
                        QMessageBox.critical(
                            None,
            ...
            if self.dlg.checkBox_kdir.isChecked():
                met_new[:, 22] = met_old[
                    :, self.dlg.comboBox_kdir.currentIndex()
                ]
                if self.dlg.checkBoxQuality.isChecked():
                    testwhere = np.where(
                        (met_new[:, 22] < 0.0) | (met_new[:, 22] > 2400.0)
                    )
                    if testwhere[0].__len__() > 0:
                        QMessageBox.critical(
                            None,
            ...
            if self.dlg.checkBox_Wd.isChecked():
                met_new[:, 23] = met_old[
                    :, self.dlg.comboBox_Wd.currentIndex()
                ]
                if self.dlg.checkBoxQuality.isChecked():
                    testwhere = np.where(
                        (met_new[:, 23] < 0.0) | (met_new[:, 23] > 360.01)
                    )
                    if testwhere[0].__len__() > 0:
                        QMessageBox.critical(
                            None,
                            "Value error",
                            "Wind directions - beyond what is expected at line:"
                            " \n" + str(testwhere[0] + 1),
                        )
                        return
            else:
            ...
            if self.dlg.checkBox_qn.isChecked():
                ...
                if self.dlg.checkBoxQuality.isChecked():
                    testwhere = np.where(
                        (met_new[:, 4] < -200.0) | (met_new[:, 4] > 800.0)
                    )
                    if testwhere[0].__len__() > 0:
                        QMessageBox.critical(
                            None,
                            "Value error",
                            "Net radiation - beyond what is expected at line:"
                            " \n" + str(testwhere[0] + 1),
                        )
                        return
            else:
            ...
            if self.dlg.checkBox_qh.isChecked():
                ...
                if self.dlg.checkBoxQuality.isChecked():
                    testwhere = np.where(
                        (met_new[:, 5] < -200.0) | (met_new[:, 5] > 750.0)
                    )
                    if testwhere[0].__len__() > 0:
                        QMessageBox.critical(
                            None,
                            "Value error",
                            "Sensible heat flux - beyond what is expected at line:"
                            " \n" + str(testwhere[0] + 1),
                        )
                        return
            else:
            ...
            if self.dlg.checkBox_qe.isChecked():
                ...
                if self.dlg.checkBoxQuality.isChecked():
                    testwhere = np.where(
                        (met_new[:, 6] < -100.0) | (met_new[:, 6] > 650.0)
                    )
                    if testwhere[0].__len__() > 0:
                        QMessageBox.critical(
                            None,
                            "Value error",
                            "Latent heat flux - beyond what is expected at line:"
                            " \n" + str(testwhere[0] + 1),
                        )
                        return
            else:
            ...
            if self.dlg.checkBox_qs.isChecked():
                ...
                if self.dlg.checkBoxQuality.isChecked():
                    testwhere = np.where(
                        (met_new[:, 7] < -200.0) | (met_new[:, 7] > 650.0)
                    )
                    if testwhere[0].__len__() > 0:
                        QMessageBox.critical(
                            None,
                            "Value error",
                            "Storage heat flux - beyond what is expected at line:"
                            " \n" + str(testwhere[0] + 1),
                        )
                        return
            else:
            ...
            if self.dlg.checkBox_qf.isChecked():
                ...
                if self.dlg.checkBoxQuality.isChecked():
                    testwhere = np.where(
                        (met_new[:, 8] < 0.0) | (met_new[:, 8] > 1500.0)
                    )
                    if testwhere[0].__len__() > 0:
                        QMessageBox.critical(
                            None,
        ...
        np.savetxt(
            outputfile[0], met_new, fmt=numformat, header=header, comments=""
        )
        ...
        QMessageBox.information(
            None,
            "Metdata pre-processor",
            "Input data to UMEP processor generated",
        )
