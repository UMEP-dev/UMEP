...
class Model:
    ...
    def __init__(self):
        ...
        self.resultsAvailable = (
            False  # Flag to determine whether the model has run yet
        )
        ...
        self.processedInputData = (
            None  # Folder containing already processed input data
        )
        ...
        self.renderPath = (
            None  # Folder containing rendered images from visualisation
        )
    ...
    def processInputData(self):
        ...
        processedPath = disaggregate(
            qfDataSources=self.ds,
            outputFolder=self.downscaledPath,
            qfParams=self.parameters,
        )
        ...
        if os.path.exists(expectedManifest):
            with open(expectedManifest, "rb") as manf:
                ...
                if type(manif) is not dict:
                    raise Exception(
                        path
                        + " does not seem to be a valid store of processed input data (manifest file missing)"
                    )
                self.processedDataList = manif
                self.processedInputData = path
        else:
            raise Exception(
                "The specified processed input data path "
                + path
                + " does not exist"
            )
    ...
    def run(self):
        ...
        if self.processedInputData != self.downscaledPath:
            dir_util.copy_tree(
                src=self.processedInputData,
                dst=self.downscaledPath,
                update=True,
            )
        ...
        try:
            copyfile(
                self.parameters.inputFile,
                os.path.join(self.configPath, "Parameters.nml"),
            )
            copyfile(
                self.ds.inputFile,
                os.path.join(self.configPath, "DataSources.nml"),
            )
        except Exception:
            pass
    ...
    def mostcalcs(self):
        ...
        [
            bldgEnergy.injectIndustrialElec(
                rp["file"],
                makeUTC(rp["startDate"]),
                rp["attribute"],
                rp["EPSG"],
        ...
        [
            bldgEnergy.injectIndustrialGas(
                rp["file"],
                makeUTC(rp["startDate"]),
                rp["attribute"],
                rp["EPSG"],
        ...
        [
            trans.injectFuelConsumption(
                rp["file"], makeUTC(rp["startDate"]), rp["EPSG"]
            )
        ...
        for tb in timeBins:
            ...
            WH.to_csv(
                os.path.join(
                    self.modelOutputPath, tb.strftime(self.dateStructure)
                ),
    ...
    def loadModelResults(self, path):
        ...
        if not os.path.exists(path):
            raise Exception(
                "Model output directory " + str(path) + " not found"
            )
        ...
        for f in files:
            ...
            if a is not None:
                self.fileList[
                    tz.localize(dt.strptime(f, self.dateStructure))
                ] = os.path.join(self.modelOutputPath, f)
        ...
        self.resultsAvailable = True
        return {
            "outputPath": self.modelOutputPath,
    ...
    def fetchResultsForLocation(self, id, startTime, endTime):
        ...
        if id not in self.outputAreas:
            raise Exception(
                "Requested ID " + id + " not among available output areas"
            )

        if (type(startTime) is not dt) or (type(endTime) is not dt):
            raise ValueError(
                "Requested start and end times must be of type datetime"
            )
        ...
        relevant = self.fileList[
            (self.fileList.index >= start) & (self.fileList.index <= end)
        ]
        if len(relevant) == 0:
            raise ValueError(
                "Requested start and end times must be of type datetime"
    ...
    def getFeatureMapper(self):
        ...
        if self.ds.outputAreas_spat["featureIds"] is None:
            featureMapper = pd.DataFrame(
                shapefile_attributes(a).index,
                index=shapefile_attributes(a).index,
            )
        else:
            # Create mapping from real (numeric) feature ID to desired (string) feature ID
            featureMapper = shapefile_attributes(a)[
                self.ds.outputAreas_spat["featureIds"]
            ]
        a = None
