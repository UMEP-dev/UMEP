...
class TransportProfiles:
    ...
    def getMotorcycle(self, timeBinEnd, timeBinDuration):
        ...
        return self.motorcycles.getValueForDateTime(
    ...
    def dealWithInputFile(self, file):
        ...
        if len(matches) != len(list(dl.keys())) - 1:
            raise ValueError(
                "Top row of transport diurnal profiles must contain each of: "
                + str(expectedHeadings)
            )
        ...
        if "transporttype" not in dl.columns.tolist():
            raise ValueError(
                "First column of row 1 must be 'TransportType' in " + file
            )

        if dl["transporttype"][0].lower() != "startdate":
            raise ValueError(
                "First column of row 2 must be 'StartDate' (of season) in "
                + file
            )

        if dl["transporttype"][1].lower() != "enddate":
            raise ValueError(
                "First column of row 3 must be 'EndDate' (of season) in "
                + file
            )

        if "timezone" != dl["transporttype"][2].lower():
            raise ValueError(
                "First column of row 4 must be 'Timezone' (of data for that season) in "
                + file
            )
        ...
        try:
            # removed pd.datetime
            sd = dt.strptime(dl[dl.columns[1]][0], "%Y-%m-%d")
            # removed pd.datetime
            ed = dt.strptime(dl[dl.columns[1]][1], "%Y-%m-%d")
        except Exception as e:
            raise Exception(
                "Second column of Rows 2 and 3 of "
                + file
        ...
        for transportType in expectedHeadings:
            ...
            dl.loc[firstDataRow:, transportType] = (
                dl.loc[firstDataRow:, transportType].astype("float")
                / dl.loc[firstDataRow:, transportType].astype("float").mean()
            )
            # Assign to the relevant element of this object
            subObj = getattr(self, transportType.lower())
            if subObj is None:
                raise Exception(
                    "Programming error: The wrong transport type has been referenced"
                )
            subObj.addPeriod(
                startDate=sd,
                endDate=ed,
                dataSeries=dl.loc[firstDataRow:, transportType],
            )
