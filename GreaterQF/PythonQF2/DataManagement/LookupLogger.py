...
class LookupLogger:
    ...
    def addEvent(
        self, eventType, requestedDate, actualDate, paramName, description
    ):
        """
    ...
    def writeFile(self, filename):
        ...
        try:
            ...
            raise Exception(
                "Could not write to log file:" + str(filename) + ":" + str(e)
            )
        f.write(
            "Requested Date (if applic):: Date returned (if applic) :: Param name :: Description\r\n"
        )
        ...
        for eventType in list(self.log.keys()):
            ...
            for requestTime in list(self.log[eventType].keys()):
                printReqTime = (
                    ...
                    else requestTime.strftime("%Y-%m-%d %H:%M:%S %Z")
                )
                for logLine in self.log[eventType][requestTime]:
                    printActualTime = (
                        "None"
                        if logLine[0] is None
                        else logLine[0].strftime("%Y-%m-%d %H:%M:%S %Z")
                    )
                    logLin = (
                        printReqTime
                        + " :: "
