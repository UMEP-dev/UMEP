...
def setOutputShapefile(self, shapefile, epsgCode, id_field):
    ...
    if type(shapefile) is str:
        ...
        self.outputLayer = openShapeFileInMemory(
            shapefile, targetEPSG=self.templateEpsgCode, label="Template layer"
        )
    ...
    if id_field not in fields:
        raise ValueError(
            "ID Field " + str(id_field) + " not present in output shapefile"
        )
...
def duplicateVectorLayer(inLayer, targetEPSG=None, label=None):
    ...
    newLayer = QgsVectorLayer(
        "Polygon?crs=EPSG:" + str(targetEPSG), label, "memory"
    )
