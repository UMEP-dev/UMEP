...
class GenericAnnualSampler(object):
    def __init__(self, logger=LookupLogger()):
        ...
        # Dictionary {year:pd.Series()} of years for which there is data
        self.yearContents = None
        # Use UK bank holidays? Set to False for non-UK countries and use special bank holidays
        self.ukBankHolidays = None
        # List of datetime objects for special (non standard UK bank holidays).
        self.extraHolidays = []
        self.dataTimezone = (
            None  # pytz.timezone : Time zone described by the data file
        )
        # Time zone object representing country being modelled (so we know when every DST switch is)
        self.countryTimezone = None
        # Boolean: Does this country have daylight savings? Used to validate input data
        self.dstOccurs = None
        # List of integers that state which days of the week are weekends (0=Mon, 6=Sun)
        self.weekendDays = []
    ...
    def setCountry(self, timezoneString):
        ...
        try:
            ...
            dates = pd.date_range(
                "2015-01-01", "2016-01-01", tz=timezoneString
            )
            self.dstOccurs = (
                len(pd.unique([d.dst().seconds != 0 for d in dates])) > 1
            )
        except Exception:
            raise ValueError(
                "Invalid time zone string: %s" % (timezoneString,)
            )
    ...
    def specialHolidays(self, holidayDates):
        ...
        if holidayDates not in [None, []]:
            self.logger.addEvent(
                ...
                None,
                None,
                "Special bank holidays added: "
                + str(list(map(niceDate, holidayDates))),
            )
    ...
    def useUKHolidays(self, state):
        ...
        """
        self.logger.addEvent(
            "TemporalSampler", None, None, None, "Use UK bank holidays = TRUE"
        )
    ...
    def entriesForYear(self, reqYear):
        ...
        if not (
            (self.yearContents[relevant].index[0].day == 1)
            and (self.yearContents[relevant].index[0].month == 1)
        ):
    ...
    def getDSTStates(self, year):
        ...
        if year not in self.availableYears():
            raise ValueError(
                "DST states requested for a year ("
                + str(year)
    ...
    def addPeriod(self, startDate, endDate, dataSeries):
        ...
        if self.dataTimezone is not None:
            if str(self.dataTimezone) != str(startDate.tzinfo):
                raise ValueError(
                    ...
                    + " has time zone of "
                    + str(startDate.tzinfo)
                    + " is inconsistent with "
                    + str(self.dataTimezone)
                    + ", which was used earlier. All datasets must have the same time zone."
                )
        ...
        localizedStartTime = self.countryTimezone.localize(
            startDate.replace(tzinfo=None) + timedelta(seconds=12 * 3600)
        )
        localizedEndTime = self.countryTimezone.localize(
            endDate.replace(tzinfo=None) + timedelta(seconds=12 * 3600)
        )
        ...
        if self.dstOccurs & (self.dataTimezone != self.countryTimezone):
            ...
            if startTime_isDST != endTime_isDST:
                raise ValueError(
                    "Input diurnal profiles file contains period "
                    + startDate.strftime("%Y-%m-%d")
                    + " to "
        ...
        if self.yearContents is None:
            self.yearContents = pd.Series(
                index=[startDate.date()],
                data=[
                    {"isDST": isDST, "data": formattedSeries.copy(deep=True)}
                ],
            )
        else:
            ...
            if np.sum(beforeStart) > 0:
                start_entry = self.yearContents.index.tolist()[
                    np.max(np.where(beforeStart))
                ]
            if np.sum(beforeEnd) > 0:
                end_entry = self.yearContents.index.tolist()[
                    np.max(np.where(beforeEnd))
                ]
            ...
            if start_entry != end_entry:
                raise ValueError(
                    "There is already data present between "
                    + startDate.strftime("%Y-%m-%d")
                    + " and "
                    + endDate.strftime("%Y-%m-%d")
                )

            series_entries = self.yearContents.index[
                self.yearContents.notnull()
            ]

            if (
                np.sum(
                    (series_entries <= endDate.date())
                    & (series_entries >= startDate.date())
                )
                > 0
            ):
                raise ValueError(
                    "There is already data series starting between between "
                    + startDate.strftime("%Y-%m-%d")
                    + " and "
                    + endDate.strftime("%Y-%m-%d")
                )

            self.yearContents[startDate.date()] = {
                "isDST": isDST,
                "data": formattedSeries.copy(deep=True),
            }
    ...
    def findSuitableYear(self, requestDate):
        ...
        modifiedDate = requestDate + relativedelta(
            weekday=requestDate.weekday(), year=chosenYear
        )
        ...
        if np.sum(mostRecent) > 0:
            ...
            goodEntry = (
                self.yearContents[periodStartStops[mostRecent][-1]] is not None
            )
            if goodEntry:
                # Return year containing suitable start date
                return periodStartStops[mostRecent][0].year
        ...
        revisedRequest = requestDate + relativedelta(
            weekday=requestDate.weekday(), year=chosenYear
        )
    ...
    def getValueForDateTime(self, timeBinEnd, timeBinDuration):
        ...
        if self.countryTimezone is None:
            raise Exception(
                "Country has not been set so cannot look up a value for date/time"
            )
        ...
        if gaps != False:
            raise Exception(
                "There are currently gaps present in years " + str(gaps)
            )

        if self.ukBankHolidays is None:
            raise Exception(
                "User must set whether to use UK bank holidays (True or False)"
            )
        ...
        # This is the start of the requested time bin
        lookupTimeBin = timeBinEnd - timedelta(seconds=timeBinDuration - 1)

        if (timeBinEnd - timedelta(seconds=1)).day != lookupTimeBin.day:
            raise ValueError(
                "Requested time bin crosses a date boundary. This is not allowed"
            )

        # Get the lookup time bin into the same time zone as the data in the object
        localLookupTimeBin = pd.Timestamp(lookupTimeBin).tz_convert(
            self.dataTimezone
        )

        # Get the right year and season from the empirical data
        # findSuitableYear is timezone aware
        yearToUse = self.findSuitableYear(lookupTimeBin)

        if yearToUse is None:
            raise ValueError(
                "No data available at this time of year for time bin ending "
                + str(timeBinEnd)
            )
        ...
        if is_holiday(
            localLookupTimeBin, self.ukBankHolidays, self.extraHolidays
        ):
            # Is requested date a holiday? Revise day of week to latest weekend day in week (if weekend days are provided.)
            if len(self.weekendDays) > 0:
                # Take the latest weekend day in the list as the day (e.g. sunday in UK)
                dowNeeded = int(self.weekendDays[-1])
            else:
                # If no weekend days provided, use existing day of week
                dowNeeded = localLookupTimeBin.weekday()
        else:
            dowNeeded = localLookupTimeBin.weekday()

        localLookupTimeBin = localLookupTimeBin + relativedelta(
            weekday=dowNeeded, year=yearToUse
        )
        ...
        if localLookupTimeBin.year != yearToUse:
            localLookupTimeBin = localLookupTimeBin + relativedelta(
                weekday=dowNeeded, weeks=-51
            )

        # Determine if DST is in effect for the chosen modelled date (at noon)
        isDst = (
            self.countryTimezone.localize(
                lookupTimeBin.replace(tzinfo=None, hour=12, minute=0)
            )
            .dst()
            .total_seconds()
            != 0
        ...
        dstDates = set(dsts.index[dsts == isDst]).intersection(
            hasRelevantDOW.index[hasRelevantDOW]
        )

        if (
            (len(dstDates) == 0)
            & self.dstOccurs
            & (self.countryTimezone != self.dataTimezone)
        ):
            raise Exception(
                "No data for daylight savings = "
                + str(isDst)
                + " during year "
                + str(timeBinEnd.year)
        ...
        if (not self.dstOccurs) or (self.countryTimezone == self.dataTimezone):
            ...
            minDiff = self.yearContents[
                hasRelevantDOW.index[hasRelevantDOW]
            ].index.asof(localLookupTimeBin.date())
        else:
        ...
        extracted, timestamp, dayofweek = self.extractCorrectEntry(
            df, timeBinEnd, timeBinDuration, dowNeeded
        )
