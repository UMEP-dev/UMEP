...
def addPeriod(
    ...
    if obj.timeZone is not None:
        if obj.timeZone != timezone:
            raise ValueError(
                "Timezone specified is not consistent with the one used earlier"
            )

    obj.timeZone = timezone

    # Do simple validations
    startYear, endYear = obj.validateInputs(
        startDate, endDate, timezone, weekSeries
    )
    ...
    for y in years:
        startEndDates[y] = [
            max(dt(y, 1, 1), startDate),
            min(dt(y, 12, 31), endDate),
        ]
    ...
    for y in list(startEndDates.keys()):
        ...
        if y not in list(obj.yearContents.keys()):
            ...
            series_entries = obj.yearContents[y].index[
                obj.yearContents[y].notnull()
            ]
            if start_entry in series_entries:
                raise ValueError(
                    "There is a data series starting between between "
                    + sd.strftime("%Y-%m-%d")
                    + " and "
                    + ed.strftime("%Y-%m-%d")
                )
            ...
            if start_entry not in series_entries:
                obj.yearContents[y][start_entry] = {
                    "isDST": isDST,
