...
class TemporalProfileSampler(GenericAnnualSampler):
    ...
    def dealWithSeries(self, series):
        ...
        # Label each row with the day of the week (same convention as datetime)
        dayCol = np.repeat(np.arange(0, 7, 1), binsPerDay)
        ...
        secCol = np.tile(
            np.linspace(0, int(24.0), int(binsPerDay), endpoint=False) * 3600,
            7,
        )
        # Ensure data is float before going in
        df = pd.concat(
            [
                pd.Series(np.array(dayCol).astype("int")),
                pd.Series(np.array(secCol).astype("int")),
                pd.Series(np.array(series.tolist()).astype("float")),
            ],
            axis=1,
        )
    ...
    def validateInputs(self, startDate, endDate, weekSeries):
        ...
        if len(weekSeries) % 7 > 0:
            raise ValueError(
                "Length of data series must be divisible by 7 (data for each day of week)"
            )

        startAndEndYear = super(TemporalProfileSampler, self).validateInputs(
            startDate, endDate
        )
        return startAndEndYear

    def extractCorrectEntry(
        self, df, endOfTimestep, timestepDuration, dowNeeded
    ):
        """
        ...
        endOfTimestep = (
            pd.Timestamp(endOfTimestep)
            .tz_convert(self.dataTimezone)
            .to_pydatetime()
        )
        startOfTimestep = endOfTimestep - timedelta(seconds=timestepDuration)
        # Subtract 1 microsecond to remove right-binning issues on date boundaries
        almostEndOfTimestep = endOfTimestep - timedelta(microseconds=1)
        ...
        ts = df[:][relevant]
        ts.index = [
            dt(2000, 1, 1) + timedelta(seconds=int(sec))
            for sec in ts["seconds"]
        ]
        ...
        startTime = dt(2000, 1, 1) + timedelta(
            seconds=int(
                startOfTimestep.second
                + startOfTimestep.hour * 3600
                + startOfTimestep.minute * 60
            )
        )
        endTime = dt(2000, 1, 1) + timedelta(
            seconds=int(
                almostEndOfTimestep.second
                + almostEndOfTimestep.hour * 3600
                + almostEndOfTimestep.minute * 60
            )
        )
        ...
        if iStart == iEnd:
            ...
            return (
                rs[rs.index.asof(startTime) : rs.index.asof(endTime)].mean(),
                iEnd,
                dowNeeded,
            )
    ...
    def addPeriod(self, startDate, endDate, dataSeries):
        ...
        super(TemporalProfileSampler, self).addPeriod(
            startDate, endDate, dataSeries
        )
        ...
        for startDate in self.entriesForYear(year):
            if self.yearContents[startDate] is not None:
                ...
                if "dayofweek" in self.yearContents[startDate]["data"].columns:
                    result[startDate] = (
                        dow
                        in self.yearContents[startDate]["data"]["dayofweek"]
                    )
                else:
