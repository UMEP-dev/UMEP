...
class DailyLoading(GenericAnnualSampler):
    ...
    def addPeriod(self, startDate, endDate, dataSeries):
        ...
        # +1 because the start and end date in the file are inclusive
        expectedLength = (endDate.date() - startDate.date()).days + 1

        if expectedLength != len(dataSeries):
            raise ValueError(
                "Daily dataset must have exactly "
                + str(expectedLength)
                + " entries"
            )
        ...
        for st in np.arange(0, len(regimeStarts), 1):
            ...
            self.validateInputs(
                regimeStarts[st],
                regimeStarts[st],
                dataSeries[regimeStarts[st] : regimeStarts[st]],
            )
            super(DailyLoading, self).addPeriod(
                regimeStarts[st],
                regimeStarts[st],
                dataSeries[regimeStarts[st] : regimeStarts[st]],
            )
    ...
    def validateInputs(self, startDate, endDate, series):
        ...
        if len(series) != numDays:
            raise ValueError(
                "Length of daily time series should be integer multiple of days in period. Need "
                + str(numDays)
                + ", got "
                + str(len(series))
            )
        newSeries = pd.Series(
            data=np.array(series.tolist()).astype("float"),
            index=[pd.date_range(start=startDate, end=endDate)],
        )

        startAndEndYear = super(DailyLoading, self).validateInputs(
            startDate, endDate
        )
    ...
    def extractCorrectEntry(self, df, endOfTimestep, timestepDuration, wd):
        ...
        dows_available = list(
            map(self.getDOW, [d.to_pydatetime() for d in df.index])
        )
    ...
    def hasDOW(self, dow, year):
        ...
        for startDate in self.entriesForYear(year):
            if self.yearContents[startDate] is not None:
                ...
                dates = [
                    d.to_pydatetime()
                    for d in self.yearContents[startDate]["data"].index
                ]
                result[startDate] = dow in list(
                    np.unique(list(map(self.getDOW, dates)))
                )
