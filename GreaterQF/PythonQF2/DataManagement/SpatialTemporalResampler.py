...
class SpatialTemporalResampler:
    ...
    def __init__(self, logger=LookupLogger()):
        ...
        # Mapping from desired feature id : actual feature ID (dict) for referencing use
        self.featureMapper = None
        self.outputLayer = None  # actual qgsVectorLayer
        # "raw" data fields
        # pd.Series of resampled QgsVectorLayer objects, or single numbers, indexed by time. Wm-2
        self.dataLayers = None
        # pd.Series of data frames that each contain a copy of the relevant dataLayer's data. Improves performance when looking up values.
        self.attributeTables = None
    ...
    def setOutputShapefile(self, shapefile, epsgCode, id_field):
        ...
        if type(shapefile) is str:
            ...
            self.outputLayer = openShapeFileInMemory(
                shapefile,
        ...
        if id_field not in fields:
            raise ValueError(
                "ID Field "
                + str(id_field)
                + " not present in output shapefile"
            )
        ...
        self.featureMapper = pd.Series(
            index=a.index, data=list(map(intOrString, a.values))
        )
    ...
    def dealWithSingleValue(self, value, startTime, attributeToUse):
        ...
        try:
            ...
            raise ValueError(
                "Value must be float() or convertible to a float()"
            )
    ...
    def dealWithVectorLayer(
        ...
        if type(shapefileInput) not in [str, QgsVectorLayer]:
            raise ValueError(
                "Shapefile input ("
                + str(shapefileInput)
                + ") is not a string or QgsVectorLayer"
            )
        ...
        if type(shapefileInput) is str:
            if not os.path.exists(shapefileInput):
                raise ValueError(
                    "Shapefile  (" + str(shapefileInput) + ") does not exist"
                )
            ...
            if int(epsgCode) != self.templateEpsgCode:
                dest = reprojectVectorLayer(
                    shapefileInput, self.templateEpsgCode
                )
                shapefileInput = dest
                reprojected = True
            ...
            # Load the layer
            try:
                vectorLayer = openShapeFileInMemory(
                    shapefileInput, targetEPSG=self.templateEpsgCode
                )

            except Exception as e:
                raise ValueError(
                    "Could not load shapefile at " + shapefileInput
                )
        ...
        resampled = self.resampleLayer(
            vectorLayer, attributeToUse, weight_by, inputIdField
        )
    ...
    def updateLayers(self, attributeToUse, layer, startTime):
        ...
        satts.index = list(
            map(
                intOrString,
                satts[self.templateIdField].loc[satts.index.tolist()],
            )
        )
    ...
    def getTableForDate(self, requestDate):
        ...
        layer_idx = (
            self.attributeTables.index[0]
            if self.attributeTables.index.asof(requestDate) is np.nan
            else self.attributeTables.index.asof(requestDate)
        )
        var_idx = (
            self.attribute.index[0]
            if self.attribute.index.asof(requestDate) is np.nan
            else self.attribute.index.asof(requestDate)
        )
    ...
    def getLayerForDate(self, requestDate):
        ...
        layer_idx = (
            self.dataLayers.index[0]
            if self.dataLayers.index.asof(requestDate) is np.nan
            else self.dataLayers.index.asof(requestDate)
        )
        var_idx = (
            self.attribute.index[0]
            if self.attribute.index.asof(requestDate) is np.nan
        ...
        attrib = self.attribute[var_idx]
        # More than one attrib can be returned, but return singleton if it's a list of 1
        if (len(attrib) == 1) and (type(attrib) is list):
            attrib = attrib[0]
    ...
    def resampleLayer(
        self, inputLayer, fieldsToSample, weight_by, inputIdField
    ):
        ...
        *** value extracted from each inputLayer polygon will be multiplied by the fraction of that polygon intersected
        ...
        newShapeFile = duplicateVectorLayer(
            self.outputLayer, targetEPSG=self.templateEpsgCode
        )
        ...
        readAcross = pd.Series(
            index=list(map(intorstring, t.values)),
        ...
        intersectedAreas = intersecting_amounts(
            fieldsToSample,
            inputIndex,
            inputLayer,
            newShapeFile,
        ...
        if weight_by in get_field_names(inputLayer):
            ...
            total_weightings = {
                weight_by: {idx: atts[idx] for idx in atts.index}
            }
            self.logger.addEvent(
                "Disagg",
                None,
                ...
                "Found attribute "
                + str(weight_by)
                + " in shapefile to be disaggregated. "
                "Assuming this is the sum of "
                + str(weight_by)
        ...
        if weight_by is None:
            disagg = disaggregate_weightings(
                intersectedAreas,
                newShapeFile,
                weight_by,
                total_weightings,
        ...
        for outputFeat in selectedOutputFeatures:  # For each output feature
            ...
            area_weightings = {
                inputAreaId: disagg[outputFeat[self.templateIdField]][
                    inputAreaId
                ]
                for inputAreaId in list(
                    disagg[outputFeat[self.templateIdField]].keys()
                )
            }
            ...
            for field in fieldsToSample:
                ...
                input_values = {
                    inputAreaId: intersectedAreas[
                        outputFeat[self.templateIdField]
                    ][inputAreaId][field]
                    for inputAreaId in list(
                        intersectedAreas[
                            outputFeat[self.templateIdField]
                ...
                newShapeFile.changeAttributeValue(
                    outputFeat.id(),
                    fieldIndices[field],
    ...
    def addInput(
        ...
        if type(input) is type(""):  # Assume a filename
            if epsgCode is None:
                raise ValueError(
                    "EPSG code must be provided if a shapefile is input"
                )
            return self.dealWithVectorLayer(
                input,
                epsgCode,
                startTime,
                attributeToUse,
        ...
        raise ValueError(
            "Error setting input layer for "
            + str(startTime)
            + ": input was neither string nor float"
        )
