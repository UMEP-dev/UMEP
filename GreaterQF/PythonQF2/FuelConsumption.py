...
class FuelConsumption:
    ...
    def dealWithFile(self, filename):
        ...
        if not os.path.exists(filename):
            raise ValueError(
                "Fuel consumption file " + filename + " does not exist"
            )
        ...
        missingRoads = list(
            set(self.roadTypes).difference(list(map(lower, roadsPresent)))
        )
        if len(missingRoads) > 0:
            raise ValueError(
                "Not all of the required road types were found in "
                + filename
                + ". Expected: "
                + str(self.roadTypes)
                + " but got "
        ...
        missingFuels = list(
            set(self.fuelTypes).difference(list(map(lower, fuelsPresent)))
        )
        if len(missingFuels) > 0:
            raise ValueError(
                "Not all of the required fuel types were found in "
                + filename
                + ". Expected: "
                + str(self.fuelTypes)
                + " but got "
        ...
        missingVehicles = list(
            set(self.vehicleTypes).difference(
                list(map(lower, vehiclesPresent))
            )
        )
        if len(missingVehicles) > 0:
            raise ValueError(
                "Not all of the required vehicle types were found in "
                + filename
                + ". Expected: "
                + str(self.vehicleTypes)
                + " but got "
                + str(missingVehicles)
            )
        ...
        self.roadMatch = {
            expected: roadsPresent[
                list(map(lower, roadsPresent)).index(expected)
            ]
            for expected in self.roadTypes
        }
        self.fuelMatch = {
            expected: fuelsPresent[
                list(map(lower, fuelsPresent)).index(expected)
            ]
            for expected in self.fuelTypes
        }
        self.vehicleMatch = {
            expected: vehiclesPresent[
                list(map(lower, vehiclesPresent)).index(expected)
            ]
            for expected in self.vehicleTypes
        }
    ...
    def getFuelConsumption(self, date, vehicle, road, fuel):
        ...
        return (
            self.data.loc[tidx]
            .loc[self.fuelMatch[fuel]]
            .loc[self.vehicleMatch[vehicle]][self.roadMatch[road]]
            / 1000.0
        )
