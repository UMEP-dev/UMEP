...
class DailyEnergyLoading:
    ...
    def __init__(
        self, city, useUKholidays, customHolidays=[], logger=LookupLogger()
    ):
        self.logger = logger
    ...
    def getElec(self, requestDateTime, timeBinDuration):
        ...
        return self.electricity.getValueForDateTime(
    ...
    def dealWithInputFile(self, file):
        ...
        if "fuel" not in list(dl.keys()):
            raise ValueError(
                "First column of first row must be 'Fuel' in " + file
            )

        if "gas" not in list(dl.keys()):
            raise ValueError(
                "One of the column headers in " + file + " must be 'Gas'"
            )

        if "elec" not in list(dl.keys()):
            raise ValueError(
                "One of the column headers in " + file + " must be 'Elec'"
            )

        rowHeaders = list(map(lower, dl.fuel[0:3]))
        if "startdate" != rowHeaders[0]:
            raise ValueError(
                "First column of second row must be 'StartDate' in " + file
            )

        if "enddate" != rowHeaders[1]:
            raise ValueError(
                "First column of third row must be 'EndDate' in " + file
            )

        if "timezone" != rowHeaders[2]:
            raise ValueError(
                "First column of fourth row must be 'Timezone' in " + file
            )
        ...
        try:
            sd = dt.strptime(
                dl[list(dl.keys())[1]][0], "%Y-%m-%d"
            )  # removed pd.datetime
            ed = dt.strptime(
                dl[list(dl.keys())[1]][1], "%Y-%m-%d"
            )  # removed pd.datetime
        ...
        self.gas.addPeriod(
            startDate=sd, endDate=ed, dataSeries=dl.gas[firstDataLine:]
        )
        self.electricity.addPeriod(
            startDate=sd, endDate=ed, dataSeries=dl.elec[firstDataLine:]
        )
