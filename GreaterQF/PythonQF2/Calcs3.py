...
def QF(
    ...
    """
    dailyFactor = dailyFactor.getFact(
        timeStepEnd
        - timedelta(
            # Get daily factors at start of time bin to ensure correct day
            seconds=timeStepDuration
        )
    )
    ...
    WattHour[columns[2]][areaCodes] = (
        prop["industrial"]["elec"]
        * diurnalProfiles.getIndElec(timeStepEnd, timeStepDuration)[0]
        * dailyEnergy.getElec(timeStepEnd, timeStepDuration)[0]
        * NewElInd
    )  # El industrial

    WattHour[columns[3]][areaCodes] = (
        prop["domestic"]["gas"]
    ...
    WattHour[columns[5]][areaCodes] = (
        prop["industrial"]["crude_oil"]
        * diurnalProfiles.getIndGas(timeStepEnd, timeStepDuration)[0]
        * dailyEnergy.getGas(timeStepEnd, timeStepDuration)[0]
        * annualEnergyUse.getIndustrialOtherValue(areaCodes, timeStepEnd)
    )  # Assume same behaviour as gas

    WattHour[columns[6]][areaCodes] = (
        WattHour[columns[0]][areaCodes]
        + WattHour[columns[1]][areaCodes]
        + WattHour[columns[3]][areaCodes]
    )  # total buildings domestic

    WattHour[columns[7]][areaCodes] = (
        WattHour[columns[2]][areaCodes]
        + WattHour[columns[4]][areaCodes]
        + WattHour[columns[5]][areaCodes]
    )  # total buildings industrial

    WattHour[columns[8]][areaCodes] = (
        WattHour[columns[6]][areaCodes] + WattHour[columns[7]][areaCodes]
    )  # total buildings
    ...
    dslSc = prop["diesel"]
    petSc = prop["petrol"]
    WattHour[columns[9]][areaCodes] = (
        (
            petHoc
            * trans.getMotorcycle(areaCodes, "petrol", timeStepEnd)
            * petSc
            + dslHoc
            * trans.getMotorcycle(areaCodes, "diesel", timeStepEnd)
            * dslSc
        )
        * diurnalTrans.getMotorcycle(timeStepEnd, timeStepDuration)[0]
        / 86400.0
    )  # motorcycles

    WattHour[columns[10]][areaCodes] = (
        (
            petHoc * trans.getTaxi(areaCodes, "petrol", timeStepEnd) * petSc
            + dslHoc * trans.getTaxi(areaCodes, "diesel", timeStepEnd) * dslSc
        )
        * diurnalTrans.getTaxi(timeStepEnd, timeStepDuration)[0]
        / 86400.0
    )  # Taxis

    WattHour[columns[11]][areaCodes] = (
        (
            petHoc * trans.getCar(areaCodes, "petrol", timeStepEnd) * petSc
            + dslHoc * trans.getCar(areaCodes, "diesel", timeStepEnd) * dslSc
        )
        * diurnalTrans.getCar(timeStepEnd, timeStepDuration)[0]
        / 86400.0
    )  # Cars

    WattHour[columns[12]][areaCodes] = (
        (
            petHoc * trans.getBus(areaCodes, "petrol", timeStepEnd) * petSc
            + dslHoc * trans.getBus(areaCodes, "diesel", timeStepEnd) * dslSc
        )
        * diurnalTrans.getBus(timeStepEnd, timeStepDuration)[0]
        / 86400.0
    )  # Bus

    WattHour[columns[13]][areaCodes] = (
        (
            petHoc * trans.getLGV(areaCodes, "petrol", timeStepEnd) * petSc
            + dslHoc * trans.getLGV(areaCodes, "diesel", timeStepEnd) * dslSc
        )
        * diurnalTrans.getLGV(timeStepEnd, timeStepDuration)[0]
        / 86400.0
    )  # LGVs

    WattHour[columns[14]][areaCodes] = (
        (
            petHoc * trans.getRigid(areaCodes, "petrol", timeStepEnd) * petSc
            + dslHoc * trans.getRigid(areaCodes, "diesel", timeStepEnd) * dslSc
        )
        * diurnalTrans.getRigid(timeStepEnd, timeStepDuration)[0]
        / 86400.0
    )  # Rigid HGVs

    WattHour[columns[15]][areaCodes] = (
        (
            petHoc * trans.getArtic(areaCodes, "petrol", timeStepEnd) * petSc
            + dslHoc * trans.getArtic(areaCodes, "diesel", timeStepEnd) * dslSc
        )
        * diurnalTrans.getArtic(timeStepEnd, timeStepDuration)[0]
        / 86400.0
    )  # Articulated HGVs

    WattHour[columns[16]] = WattHour[columns[9:16]].sum(
        axis=1
    )  # Calculate grand total across transport

    # METABOLISM
    # Home:Work balance of people.  1=Workday population, 0= Residential Population
    activeFraction = workingProfiles.getFraction(
        timeStepEnd, timeStepDuration
    )[0]
    a = (
        prop["metab"]
        * workingProfiles.getWattPerson(timeStepEnd, timeStepDuration)[0]
        * (
    ...
    WattHour[columns[18]][areaCodes] = (
        WattHour[columns[8]][areaCodes]
        + WattHour[columns[16]][areaCodes]
        + WattHour[columns[17]][areaCodes]
    )
