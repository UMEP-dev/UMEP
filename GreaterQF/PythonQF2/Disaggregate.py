import os
import pickle
from .DataManagement.spatialHelpers import saveLayerToFile
...
def disaggregate(qfDataSources, qfParams, outputFolder):
    ...
    for rp in qfDataSources.resPop_spat:
        ...
        saveLayerToFile(
            ds, outFile, pop.getOutputLayer().crs(), "Res pop scaled"
        )
    ...
    for wp in qfDataSources.workPop_spat:
        ...
        saveLayerToFile(
            ds, outFile, pop.getOutputLayer().crs(), "Work pop scaled"
        )
    ...
    for ie in qfDataSources.indElec_spat:
        ...
        workpop, workpopattrib = pop.getWorkPopLayer(makeUTC(ie["startDate"]))
        bldgEnergy.setOutputShapefile(
            workpop,
            workpop.dataProvider().crs().authid().split(":")[1],
            outFeatIds,
        )
        bldgEnergy.setIndustrialElec(
            ie["shapefile"],
        ...
        ds, attrib = bldgEnergy.getIndustrialElecLayer(
    ...
    for ig in qfDataSources.indGas_spat:
        output, workpopattrib = pop.getWorkPopLayer(
            makeUTC(ig["startDate"])
        )  # Disaggregate by workplace pop
        bldgEnergy.setOutputShapefile(
            output,
            output.dataProvider().crs().authid().split(":")[1],
            outFeatIds,
        )
        bldgEnergy.setIndustrialGas(
            ig["shapefile"],
    ...
    for dg in qfDataSources.domGas_spat:
        output, respopattrib = pop.getResPopLayer(
            makeUTC(dg["startDate"])
        )  # Disaggregate by residential pop
        bldgEnergy.setOutputShapefile(
            output,
            output.dataProvider().crs().authid().split(":")[1],
            outFeatIds,
        )
        bldgEnergy.setDomesticGas(
            dg["shapefile"],
    ...
    for tr in qfDataSources.transport_spat:
        sf = t.addTransportData(
            shapefile=tr["shapefile"],
        ...
        outFile = os.path.join(
            outputFolder,
            "DailyFuelUse_starting"
            + tr["startDate"].strftime("%Y-%m-%d")
            + ".shp",
        )
        saveLayerToFile(
            sf, outFile, bldgEnergy.getOutputLayer().crs(), "Fuel use daily"
        )
