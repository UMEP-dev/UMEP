...
class Transport:
    ...
    def validateInputs(
        self, startTime, shapefile, epsgCode, roadTypeField, roadTypeNames
    ):
        ...
        if len(matchingRoadClasses) > 0:
            raise ValueError(
                "Dict containing road class identifiers must have keys: "
                + str(self.roadTypes)
            )
    ...
    def injectFuelConsumption(self, filename, startDate, epsgCode):
        ...
        return self.transport.injectInput(
            filename, epsgCode, allFuelFields, startDate
        )

    def addTransportData(
        self,
        shapefile,
        startTime,
        epsgCode,
        ...
        speedDataAvailable = (
            False  # Mean traffic speed available for each road segment
        )
        ...
        self.validateInputs(
            startTime, shapefile, epsgCode, roadTypeField, roadTypeNames
        )
        ...
        fuelConsumption = pd.DataFrame(
            index=list(intersectedLines.keys()), columns=newFieldIndices
        )  # Results container for each feature
        for outfeat_id in list(intersectedLines.keys()):
            ...
            if len(intersectedLines[outfeat_id]) > 0:
                ...
                lengths = (
                    pd.DataFrame().from_dict(intersectedLines[outfeat_id]).T
                )

                proportionIntersected = (
                    lengths["amountIntersected"] / lengths["originalAmount"]
                )
    ...
    def getMotorcycle(self, featureId, fuel, requestDate):
        ...
        return self.getFuelConsTable(requestDate)[
            self.getColumnName(fuelType=fuel, vehType="motorcycle")
        ].loc[featureId]
    ...
    def getTaxi(self, featureId, fuel, requestDate):
        ...
        return self.getFuelConsTable(requestDate)[
            self.getColumnName(fuelType=fuel, vehType="taxi")
        ].loc[featureId]
    ...
    def getCar(self, featureId, fuel, requestDate):
        ...
        return self.getFuelConsTable(requestDate)[
            self.getColumnName(fuelType=fuel, vehType="car")
        ].loc[featureId]
    ...
    def getArtic(self, featureId, fuel, requestDate):
        ...
        return self.getFuelConsTable(requestDate)[
            self.getColumnName(fuelType=fuel, vehType="artic")
        ].loc[featureId]
    ...
    def getRigid(self, featureId, fuel, requestDate):
        ...
        return self.getFuelConsTable(requestDate)[
            self.getColumnName(fuelType=fuel, vehType="rigid")
        ].loc[featureId]
    ...
    def getBus(self, featureId, fuel, requestDate):
        ...
        return self.getFuelConsTable(requestDate)[
            self.getColumnName(fuelType=fuel, vehType="bus")
        ].loc[featureId]
    ...
    def getLGV(self, featureId, fuel, requestDate):
        ...
        return self.getFuelConsTable(requestDate)[
            self.getColumnName(fuelType=fuel, vehType="lgv")
        ].loc[featureId]
