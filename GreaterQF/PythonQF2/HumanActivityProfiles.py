...
class HumanActivityProfiles:
    ...
    def dealWithInputFile(self, file):
        ...
        if (len(list(dl.keys())) - 1) % 3 != 0:
            raise ValueError(
                "There must be 6 columns for each named season in " + file
            )
        ...
        if "type" != rowHeaders[2]:
            raise ValueError(
                "First column of row 3 must be 'Type' (of cycle for that column) in "
                + file
            )

        if "startdate" != rowHeaders[3]:
            raise ValueError(
                "First column of row 4 must be 'StartDate' (of season) in "
                + file
            )

        if "enddate" != rowHeaders[4]:
            raise ValueError(
                "First column of row 5 must be 'EndDate' (of season) in "
                + file
            )

        if "timezone" != rowHeaders[5]:
            raise ValueError(
                "First column of row 6 must be 'Timezone' (of data for that season) in "
                + file
            )
        ...
        if "energy" != lower(dl[1][2]):
            raise ValueError(
                "Second column of row 3 must be 'Energy' (metabolic energy) in "
                + file
            )

        if "fraction" != lower(dl[2][2]):
            raise ValueError(
                "Second column of row 3 must be 'Fraction' (fraction of population doing work) in "
                + file
            )

        if len(pd.unique(dl.iloc[2][1:])) != 2:
            raise ValueError(
                "Headers in row 5 must be alternating list of 'Energy' and 'Fraction' in "
                + file
            )
        ...
        for seasonStart in np.arange(1, dl.shape[1], 6):
            ...
            for i in range(0, 6, 1):
                dl[seasonStart + i][firstDataRow:] = dl[seasonStart + i][
                    firstDataRow:
                ].astype("float")

            # Amalgamate into a template weeks of data
            energyWeek = pd.concat(
                [
            ...
            self.fraction.addPeriod(
                startDate=sd, endDate=ed, dataSeries=fractionWeek
            )
            self.energy.addPeriod(
                startDate=sd, endDate=ed, dataSeries=energyWeek
            )
