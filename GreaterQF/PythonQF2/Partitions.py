class Partitions:
    ...
    def __init__(self, conf, params):
        ...
        self.latent["petrol"] = 1 - (
            params.heatOfCombustion["petrol"]["net"]
            / params.heatOfCombustion["petrol"]["gross"]
        )
        self.latent["diesel"] = 1 - (
            params.heatOfCombustion["diesel"]["net"]
            / params.heatOfCombustion["diesel"]["gross"]
        )
        self.latent["gas"] = 1 - (
            params.heatOfCombustion["gas"]["net"]
            / params.heatOfCombustion["gas"]["gross"]
        )
        self.latent["crude_oil"] = 1 - (
            params.heatOfCombustion["crude_oil"]["net"]
            / params.heatOfCombustion["crude_oil"]["gross"]
        )
        self.latent["elec"] = 0
        self.latent["eco7"] = 0
        ...
        self.sensible["domestic"]["eco7"] = (1 - self.latent["elec"]) * (
            1
            - (
                params.waterHeatFract["domestic"]["eco7"]
                * params.heaterEffic["elec"]
            )
        )
        ...
        self.sensible["domestic"]["elec"] = (1 - self.latent["elec"]) * (
            1
            - (
                params.waterHeatFract["domestic"]["elec"]
                * params.heaterEffic["elec"]
            )
        )
        ...
        self.sensible["domestic"]["gas"] = (1 - self.latent["gas"]) * (
            1
            - (
                params.waterHeatFract["domestic"]["gas"]
                * params.heaterEffic["gas"]
            )
        )

        self.sensible["domestic"]["crude_oil"] = (
            1 - self.latent["crude_oil"]
        ) * (
            1
            - (
                params.waterHeatFract["domestic"]["crude_oil"]
                * params.heaterEffic["gas"]
            )
        )
        ...
        self.sensible["industrial"]["elec"] = (1 - self.latent["elec"]) * (
            1
            - (
                params.waterHeatFract["industrial"]["elec"]
                * params.heaterEffic["elec"]
            )
        )
        ...
        self.sensible["industrial"]["gas"] = (1 - self.latent["gas"]) * (
            1
            - (
                params.waterHeatFract["industrial"]["gas"]
                * params.heaterEffic["gas"]
            )
        )

        self.sensible["industrial"]["crude_oil"] = (
            1 - self.latent["crude_oil"]
        ) * (
            1
            - (
                params.waterHeatFract["industrial"]["crude_oil"]
                * params.heaterEffic["gas"]
            )
        )
        ...
        self.wasteWater["domestic"]["eco7"] = (
            (1 - self.latent["elec"])
            * params.waterHeatFract["domestic"]["eco7"]
            * params.heaterEffic["elec"]
        )
        self.wasteWater["domestic"]["elec"] = (
            (1 - self.latent["elec"])
            * params.waterHeatFract["domestic"]["elec"]
            * params.heaterEffic["elec"]
        )
        self.wasteWater["domestic"]["gas"] = (
            (1 - self.latent["gas"])
            * params.waterHeatFract["domestic"]["gas"]
            * params.heaterEffic["gas"]
        )
        self.wasteWater["domestic"]["crude_oil"] = (
            (1 - self.latent["crude_oil"])
            * params.waterHeatFract["domestic"]["crude_oil"]
            * params.heaterEffic["gas"]
        )
        self.wasteWater["industrial"]["elec"] = (
            (1 - self.latent["elec"])
            * params.waterHeatFract["industrial"]["elec"]
            * params.heaterEffic["elec"]
        )
        self.wasteWater["industrial"]["gas"] = (
            (1 - self.latent["gas"])
            * params.waterHeatFract["industrial"]["gas"]
            * params.heaterEffic["gas"]
        )
        self.wasteWater["industrial"]["crude_oil"] = (
            (1 - self.latent["crude_oil"])
            * params.waterHeatFract["industrial"]["crude_oil"]
            * params.heaterEffic["gas"]
        )
        ...
        for k in list(self.wasteWater.keys()):
            for c in list(self.wasteWater[k].keys()):
                self.wasteWater[k][c] = (
                    conf.wastewater_qf * self.wasteWater[k][c]
                )
        ...
        for k in list(self.sensible.keys()):
            if type(self.sensible[k]) is dict:
                for c in list(self.sensible[k].keys()):
                    self.sensible[k][c] = (
                        conf.sensible_qf * self.sensible[k][c]
                    )
            else:
