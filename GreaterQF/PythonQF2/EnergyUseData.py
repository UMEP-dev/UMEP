...
class EnergyUseData:
    ...
    def injectDomesticElec(
        self, input, startTime, attributeToUse, epsgCode=None
    ):
        """
        ...
        """
        return self.domElec.injectInput(
            input, epsgCode, attributeToUse, startTime
        )

    def setIndustrialElec(
        self,
        input,
        startTime,
        attributeToUse,
    ...
    def injectIndustrialElec(
        self, input, startTime, attributeToUse, epsgCode=None
    ):
        return self.indElec.injectInput(
            input, epsgCode, attributeToUse, startTime
        )
    ...
    def injectDomesticGas(
        self, input, startTime, attributeToUse, epsgCode=None
    ):
        return self.domGas.injectInput(
            input, epsgCode, attributeToUse, startTime
        )

    def setIndustrialGas(
        self,
        input,
        startTime,
        attributeToUse,
    ...
    def injectIndustrialGas(
        self, input, startTime, attributeToUse, epsgCode=None
    ):
        return self.indGas.injectInput(
            input, epsgCode, attributeToUse, startTime
        )
    ...
    def setEconomy7Elec(
        ...
        return self.economy7Elec.addInput(
            input,
            startTime,
            attributeToUse,
            inputFieldId,
    ...
    def injectEconomy7Elec(
        self, input, startTime, attributeToUse, epsgCode=None
    ):
        return self.economy7Elec.injectInput(
            input, epsgCode, attributeToUse, startTime
        )
    ...
    def getEnergyTable(self, requestDate, energyType):
        ...
        combined_refined = (
            combined_refined.astype("float") * 1000 / (365.25 * 24)
        )
        # print('seems pass',1/0)

        # And normalise by area of each polygon to get W/m2
        combined_refined = combined_refined.divide(
            self.domElec.getAreas(), axis="index"
        )
        self.logger.addEvent(
            ...
            None,
            str(typeLabels),
            "Converting from kWh/yr to W/m2",
        )
    ...
    def convertToEnergyDensity(self, layer, attrib):
        ...
        layer = multiply_shapefile_attribute(
            layer, attrib, 1000.0 / (365.25 * 24)
        )
        return layer
...
def testIt():
    ...
    domestic_gas = {}
    domestic_gas["shapefile"] = (
        "C:\\Users\pn910202\Dropbox\Shapefilecombos\LSOA_elec_gas_2014\LSOA_elec_gas_2014.shp"
    )
    domestic_gas["epsg"] = 27700
    # Can be found in QGIS > view attributes table
    domestic_gas["field_to_use"] = "GasDom"
    domestic_gas["start_date"] = datetime.strptime("2014-01-01", "%Y-%m-%d")

    # Domestic electricity - must be kWh/year
    domestic_elec = {}
    domestic_elec["shapefile"] = (
        "C:\\Users\pn910202\Dropbox\Shapefilecombos\LSOA_elec_gas_2014\LSOA_elec_gas_2014.shp"
    )
    domestic_elec["epsg"] = 27700
    # Can be found in QGIS > view attributes table
    domestic_elec["field_to_use"] = "TElDom"
    domestic_elec["start_date"] = datetime.strptime("2014-01-01", "%Y-%m-%d")

    # Industrial gas - must be kWh/year
    industrial_gas = {}
    industrial_gas["shapefile"] = (
        "C:\\Users\pn910202\Dropbox\Shapefilecombos\MSOA_elec_gas_2014\MSOA_elec_gas_2014.shp"
    )
    industrial_gas["epsg"] = 27700
    # Can be found in QGIS > view attributes table
    industrial_gas["field_to_use"] = "GasInd"
    industrial_gas["start_date"] = datetime.strptime("2014-01-01", "%Y-%m-%d")

    # Industrial Electricity - must be kWh/year
    industrial_elec = {}
    industrial_elec["shapefile"] = (
        "C:\\Users\pn910202\Dropbox\Shapefilecombos\LA_energy_2014\LA_energy_2014.shp"
    )
    industrial_elec["epsg"] = 4326
    # Can be found in QGIS > view attributes table
    industrial_elec["field_to_use"] = "ElInd_kWh"
    ...
    a.setDomesticElec(
        ...
        domestic_elec["field_to_use"],
        epsgCode=domestic_elec["epsg"],
    )
    a.setDomesticGas(
        1.0,
        datetime.strptime("2014-01-01", "%Y-%m-%d"),
    ...
    a.setIndustrialElec(
        2.0, datetime.strptime("2014-01-01", "%Y-%m-%d"), "DomGas"
    )
    a.setIndustrialGas(
        3.0, datetime.strptime("2014-01-01", "%Y-%m-%d"), "DomGas"
    )
    a.setEconomy7Elec(
        7.0, datetime.strptime("2014-01-01", "%Y-%m-%d"), "DomGas"
    )
