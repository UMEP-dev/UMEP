...
class EnergyProfiles:
    def __init__(
        self, city, use_uk_holidays, customHolidays=[], logger=LookupLogger()
    ):
        """Instantiate
        ...
        # Future version: Support other weekdays
        self.domesticElectricity.setWeekendDays([5, 6])
    ...
    def getDomElec(self, timeBinEnd, timeBinDuration):
        ...
        return self.domesticElectricity.getValueForDateTime(
            timeBinEnd, timeBinDuration
        )

    # Methods that follow the same pattern as getDomElec()
    def getIndElec(self, timeBinEnd, timeBinDuration):
        return self.industrialElectricity.getValueForDateTime(
            timeBinEnd, timeBinDuration
        )
    ...
    def getDomGas(self, timeBinEnd, timeBinDuration):
        return self.domesticGas.getValueForDateTime(
            timeBinEnd, timeBinDuration
        )

    def getIndGas(self, timeBinEnd, timeBinDuration):
        return self.industrialGas.getValueForDateTime(
            timeBinEnd, timeBinDuration
        )
    ...
    def dealWithInputFile(self, energyComponent, file):
        ...
        if (len(list(dl.keys())) - 1) % 3 != 0:
            raise ValueError(
                "There must be 3 columns for each named season in " + file
            )
        ...
        if "season" != rowHeadings[0]:
            raise ValueError(
                "First column of row 1 must be 'Season' in " + file
            )
        ...
        if "tariff" != rowHeadings[2]:
            raise ValueError(
                "First column of row 3 must be 'Tariff' in " + file
            )

        if "startdate" != rowHeadings[3]:
            raise ValueError(
                "First column of row 4 must be 'StartDate' in " + file
            )

        if "enddate" != rowHeadings[4]:
            raise ValueError(
                "First column of row 5 must be 'EndDate' in " + file
            )

        if "timezone" != rowHeadings[5]:
            raise ValueError(
                "First column of row 6 must be 'Timezone' in " + file
            )
        ...
        for seasonStart in np.arange(1, dl.shape[1], 3):
            try:
                # removed pd.datetime
                sd = dt.strptime(dl[seasonStart][3], "%Y-%m-%d")
                # removed pd.datetime
                ed = dt.strptime(dl[seasonStart][4], "%Y-%m-%d")
            except Exception as e:
                raise Exception(
                    "Rows 4 and 5 of "
                    + file
                    + " must be dates in the format YYYY-mm-dd"
                )
            ...
            for i in range(0, 3, 1):
                dl[seasonStart + i][firstDataLine:] = (
                    dl[seasonStart + i][firstDataLine:].astype("float")
                    / dl[seasonStart + i][firstDataLine:]
                    .astype("float")
                    .mean()
                )
            ...
            energyComponent.addPeriod(
                startDate=sd, endDate=ed, dataSeries=week
            )

        # Check for complete years. Raise exception if not complete.
        fullRange = pd.date_range(
            "%d-01-01" % (earliestStart.year,),
            "%d-12-31" % (latestEnd.year,),
            freq="D",
        )
        if len(fullRange) != len(dateRanges):
            raise Exception(
                "Diurnal profile file %s must contain complete years of data (Jan 1 to Dec 31), but starts on %s and ends on %s"
                % (
                    file,
                    earliestStart.strftime("%Y-%m-%d"),
                    latestEnd.strftime("%Y-%m-%d"),
                )
            )
