...
class Config:
    def __init__(self):
        ...
        self.expected_names = ['start_dates', 'end_dates',
                               'all_qf', 'sensible_qf', 'latent_qf', 'wastewater_qf']
    ...
    def loadFromNamelist(self, configFile):
        ...
        try:
            ...
            raise ValueError('Could not process config file ' +
                             configFile + ': ' + str(e))

        # Try dates out
        try:
            self.dt_start = list(map(
                function=to_date, sequence=CONFIG['input_nml']['start_dates']))
            self.dt_end = list(map(function=to_date,
                              sequence=CONFIG['input_nml']['end_dates']))
        except Exception as e:
            raise ValueError(
                'Could not interpret dates for model configuration: ' + str(e))
...
def testIt():
    ...
    b['latent_qf'] = 0
    b['wastewater_qf'] = 1
    b['start_dates'] = '2016-01-01'
    b['end_dates'] = '2017-01-01'
    a.loadFromDictionary(b)
    a.saveNamelist('./DataManagement/oldconfigfiles/pycharmoutput1.nml')
