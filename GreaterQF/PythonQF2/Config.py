...
class Config:
    ...
    def checkInput(self, inputDict):
        ...
        if len(missingNames) > 0:
            raise ValueError(
                "Required entries missing from config file. This should have "
                + str(missingNames)
            )
    ...
    def loadFromNamelist(self, configFile):
        ...
        try:
            ...
            raise ValueError(
                "Could not process config file " + configFile + ": " + str(e)
            )
        ...
        try:
            self.dt_start = list(
                map(
                    function=to_date,
                    sequence=CONFIG["input_nml"]["start_dates"],
                )
            )
            self.dt_end = list(
                map(
                    function=to_date, sequence=CONFIG["input_nml"]["end_dates"]
                )
            )
        except Exception as e:
            raise ValueError(
