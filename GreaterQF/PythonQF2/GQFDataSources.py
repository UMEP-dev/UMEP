...
def validateInput(x):
    try:
        ...
        if not os.path.exists(x):
            raise ValueError(
                x
                + " could not be found on the filesystem and is not a valid number"
            )
...
class DataSources:
    ...
    def __init__(self, configFile):
        ...
        try:
            ...
            raise ValueError(
                "Unable to read data sources config file at: "
                + str(configFile)
            )

        # Are all main entries present?
        # For shapefile inputs
        expectedKeys_spatial = [
            "outputareas",
            "annualindgas",
            "annualindelec",
            "annualdomgas",
        ...
        destinations_spatial = {
            "outputareas": "outputAreas_spat",
        ...
        for subEntry in expectedKeys_spatial:
            ...
            content = {
                upper(k): content_orig[k] for k in list(content_orig.keys())
            }

            # Check it's all lists or no lists
            # print('here', list(content.values()))
            # types = np.unique(list(map(type, list(content.values()))))
            types = set(map(type, list(content.values())))
            ...
            if (
                subEntry == "transportdata"
            ):  # Special case for transportData, which is very complex
                expectedNames_spat = [
                    "shapefiles",
                    "startDates",
                    "epsgCodes",
            ...
            missing = list(
                set(map(upper, expectedNames_spat)).difference(
                    list(content.keys())
                )
            )
            if len(missing) > 0:
            ...
            if "STARTDATES" in list(content.keys()):
                try:
                    content["STARTDATES"] = list(
                        map(makeTimey, content["STARTDATES"])
                    )
                except Exception as e:
                    raise ValueError(
                        "One or more startDate entries is not in YYYY-mm-dd format for "
                        + subEntry
                        + ":"
                        + str(e)
                    )
                ...
                if len(np.unique(content["STARTDATES"])) != len(
                    content["STARTDATES"]
                ):
                    raise ValueError(
                        "One or more startDates is duplicated for "
                        + subEntry
                        + ":"
                    )
            ...
            if subEntry == "outputareas":  # Special case for output areas
                ...
                dataValues = {
                    "shapefile": content["SHAPEFILES"][i],
                    ...
                    "vehicle_AADT_available": content[
                        "VEHICLE_AADT_AVAILABLE"
                    ][i],
                ...
                structuredData["road_types"] = {
                    "motorway": dataValues["motorway_class"],
        ...
        missing = list(
            set(map(upper, expectedKeys_temporal)).difference(
                list(map(upper, list(ds.keys())))
            )
        )

        if len(missing) > 0:
            raise ValueError(
                "Temporal entries missing from "
                + str(configFile)
                + " in namelist: "
                + str(missing)
            )
        ...
        for entry in expectedKeys_temporal:
            ...
            missing = list(
                set(map(upper, expectedNames_temporal)).difference(
                    list(map(upper, list(content.keys())))
                )
            )
            if len(missing) > 0:
                raise ValueError(
            ...
            if len(types) > 1 and list in types:
                raise ValueError(
                    "The namelist entries for "
                    + entry
                    + " have inconsistent lengths: some are lists and some are not"
                )
            # if list not in types:
            for k in list(content.keys()):
...
def validate_transport(inputDict):
    ...
    if len(missing) > 0:
        raise ValueError(
            "Entries missing from transportData section of data sources file:"
            + str(missing)
        )
    road_class_entries = [
        "class_field",
    ...
    for mandatory in road_class_entries:
        if type(inputDict[mandatory]) is not str:
            raise ValueError(
                'Error in data sources file: "'
                + mandatory
                + '" in transportData is mandatory'
            )
    ...
    if inputDict["speed_available"] == 1:
        if type(inputDict["speed_field"]) is not str:
            raise ValueError(
                'Error in data sources file: "speed" in transportData must be a field name if speed_available set to 1'
            )
        try:
            a = float(inputDict["speed_multiplier"])
        except Exception:
            raise ValueError(
                'Error in data sources file: "speed_multiplier" in transportData must be a number if speed_available set to 1'
            )

    if inputDict["total_AADT_available"] == 1:
        if type(inputDict["AADT_total"]) is not str:
            raise ValueError(
                '"AADT_total" in transportData must be a field name if total_AADT_available set to 1'
            )
    ...
    if inputDict["vehicle_AADT_available"] == 1:
        ...
        if len(missing) > 0:
            raise ValueError(
                "The following AADT field names are missing from the transportData section of the data sources file: "
                + str(missing)
            )

        # See if the conditionally optional fields exist
        combination_aadts = {
            "AADT_total_LGV": ["AADT_petrol_LGV", "AADT_diesel_LGV"],
            "AADT_total_car": ["AADT_petrol_car", "AADT_diesel_car"],
        }
        for one in list(combination_aadts.keys()):
            # If the total value is not present, the other values must be
            if type(inputDict[one]) is not str:
                for other in combination_aadts[one]:
                    if type(inputDict[other]) is not str:
                        raise Exception(
                            "Entry "
                            + other
                            + " in transportData section of data sources file must be an attribute name if "
                            + one
                            + " is not set"
                        )
...
if __name__ == "__main__":
    a = DataSources(
        "C:\\Users\pn910202\.qgis2\python\plugins\GreaterQF\PythonQF2\GQFInputs\dataSources_working.nml"
    )
    print(a.resPop_spat)
