...
class Transport():
    ...
    def __init__(self, fuelConsumption, modelParams, logger=LookupLogger()):
        ...
        self.completeInputs = ['diesel_car', 'petrol_car', 'diesel_lgv', 'petrol_lgv', 'motorcycle', 'taxi', 'bus', 'coach', 'rigid', 'artic']
        # 2. A less specific breakdown that matches the model outputs.
        self.modelledTypes = ['car', 'lgv', 'motorcycle', 'taxi', 'bus','rigid', 'artic']
        self.roadTypes = ['motorway', 'primary_road', 'secondary_road'] # The road type headings used internally by the model

        # The methods in this object manufacture a shapefile of fuel consumption in each area. The names of the fields:
        self.dieselNames = {'motorcycle':'_FC_Dmcyc', 'artic':'_FC_Dart', 'rigid':'_FC_Drig', 'taxi':'_FC_Dtaxi', 'car':'_FC_Dcar', 'bus':'_FC_Dbus', 'lgv':'_FC_Dlgv'}
        self.petrolNames = {'motorcycle':'_FC_Pmcyc', 'artic':'_FC_Part', 'rigid':'_FC_Prig', 'taxi':'_FC_Ptaxi', 'car':'_FC_Pcar', 'bus':'_FC_Pbus', 'lgv':'_FC_Plgv'}
    ...
    def validateInputs(self, startTime, shapefile, epsgCode, roadTypeField, roadTypeNames):
      ...
      # Validate the start time
        ...
        if type(shapefile) is not type(''):
            raise ValueError('Shapefile filename (' + str(shapefile) + ') is not a string')

        if not os.path.exists(shapefile):
            raise ValueError('Shapefile  (' + str(shapefile) + ') does not exist')

        try:
            epsgCode = float(epsgCode)
        except Exception:
            raise ValueError('Numeric EPSG code of input shapefile must be provided')
    ...
    def addTransportData(self, shapefile, startTime, epsgCode, roadTypeField, roadTypeNames, inputIdField, speedDataField=None, speedConversionFactor=None, totalAADTField=None, vAADTFields=None):
        ...
        self.validateInputs(startTime, shapefile, epsgCode, roadTypeField, roadTypeNames)

        roadTypeLookup = {roadTypeNames[key]:key for key in self.roadTypes} # This is a way to look up our version of the road type, given the shapefile's version of the road type
        roadTypeLookup['other'] = 'other'                               # For all other road types that don't match the above

        # Establish what speed data is available
        # TODO: Use speed in fuel efficiency lookup
        fieldsToSample = [roadTypeField] # Master list of all the fields to sample from the shapefile. Gets built up as we go along...
        ...
        shapefile = reprojectVectorLayer(shapefile, self.transport.templateEpsgCode)
        #inputLayer = loadShapeFile(shapefile)
        inputLayer = openShapeFileInMemory(shapefile, self.transport.templateEpsgCode, 'transport')
        ...
        vehDate = pd.datetime.strptime('2005-01-01', '%Y-%m-%d') # For now, just assumes every vehicle was made in 2005 and looks up values from the fuel consumption object
        ...
        outputLayer = duplicateVectorLayer(self.transport.outputLayer, targetEPSG=self.transport.templateEpsgCode)
        ...
        fuelUseDict = calculate_fuel_use(inputLayer,inputIdField,
                                        totalAADTField=totalAADTField, roadTypeField=roadTypeField, vAADTFields=vAADTFields,
                                        completeInputs=self.completeInputs,
                                        modelParams=self.modelParams, age=vehDate, fuelCon=self.fc, roadTypeLookup=roadTypeLookup,
                                        completeInputAADTprovided=completeInputAADTprovided, modelledTypesAADTprovided=modelledTypesAADTprovided)
        ...
        newFieldIndices = [get_field_index(outputLayer, fn) for fn in fuelUseData.columns]
        ...
        for outfeat_id in list(intersectedLines.keys()):
            ...
            if len(intersectedLines[outfeat_id]) > 0:
                ...
                lengths = pd.DataFrame().from_dict(intersectedLines[outfeat_id]).T

                proportionIntersected = lengths['amountIntersected']/lengths['originalAmount']
                # Get total of each fuel/vehicle combination by summing across segments in output area
                fuelConsumption[:].loc[outfeat_id] = (fuelUseData[:].loc[lengths.index].multiply(proportionIntersected, axis=0)).sum(axis=0, skipna=True)
            ...
            [outputLayer.changeAttributeValue(featureMapper[outfeat_id], fi, float(fuelConsumption[fi][outfeat_id])/float(areas[outfeat_id])) for fi in newFieldIndices]
        ...
        confirmedOutput = self.transport.addInput(outputLayer, startTime, allFuelFields, self.transport.templateIdField, epsgCode=epsgCode)
