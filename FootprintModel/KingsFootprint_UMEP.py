...
def footprintiterKAM(
    ...
    domain_y = np.round(maxfetch / 2.0)  # pixels ?
    ...
    for i in np.arange(0, iterations, 1):
        ...
        def fequ_m(m):
            return (
                I_1(2.0 * m, z_1, z_2, z_m)
                * (
                    I_3(m, z_0 / z_m, z_1, z_2, z_m)
                    + J_2(m, z_1, z_2, z_m, L, f_psi_m(z * z_m, z_m, L))
                )
            ) - (
                I_2(2.0 * m, 1, z_1, z_2, z_m)
                * (
                    I_2(2.0 * m, z_0 / z_m, z_1, z_2, z_m)
                    + J_1(m, z_1, z_2, z_m, L, f_psi_m(z * z_m, z_m, L))
                )
            )
        ...
        def fequ_n(n):
            return I_1(2.0 * n, z_1, z_2, z_m) * J_2(
                n, z_1, z_2, z_m, L, f_z_phi_c(z * z_m, z_m, L)
            ) - I_2(2.0 * n, 1, z_1, z_2, z_m) * J_1(
                n, z_1, z_2, z_m, L, f_z_phi_c(z * z_m, z_m, L)
            )

        # solve when fequ_n falls to zero with starting guess of 0.5
        n = fsolve(fequ_n, 0.5)
        ...
        U = (u_star / k) * (
            (
                I_2(m, z_0 / z_m, z_1, z_2, z_m)
                + J_1(m, z_1, z_2, z_m, L, f_psi_m(z * z_m, z_m, L))
            )
            / (I_1(2.0 * m, z_1, z_2, z_m) * (z_m**m))
        )
        # Calculation of Kappa constant EQUATION 41 {X6}
        kappa = (k * u_star) * (
            J_1(n, z_1, z_2, z_m, L, f_z_phi_c(z * z_m, z_m, L))
            / (I_1(2.0 * n, z_1, z_2, z_m) * (z_m ** (n - 1)))
        )
        ...
        # EQUATION 18
        u_bar = (
            (math.gamma(mu) / math.gamma(1.0 / r))
            * ((((r**2) * kappa) / U) ** (m / r))
            * (U * (x[:, :, 0] ** (m / r)))
        )
        ...
        inm = (-x[:, :, 1] ** 2) / (2.0 * (sig**2))
        D_y = ((np.sqrt(2.0 * math.pi) * sig) ** (-1)) * np.exp(inm)
        ...
        full[
            int((fx + 1) / 2) : int((fx + 1) / 2 + (domain_x)),
            int((fy / 2 + 1) - domain_y) : int((fy / 2 + 1) + domain_y + 1),
        ] = phi
        ...
        rotatedphi = scnd.rotate(
            full, rotang, order=0, reshape=False, mode="nearest"
        )
        ...
        Wz_d_output[i], Wz_0_output[i] = rg.RoughnessCalc(
            zH=WzH[i],
            fai=Wfai[i],
            pai=Wpai[i],
            zMax=WzMax[i],
...
def footprintiterKLJ(
    ...
    dlg.progressBar.setRange(0, iterations)
    dlg.progressBar.setValue(0)
    domain_output_inm = maxfetch * 2.0
    domain_output_inpix = domain_output_inm / res

    moddomain_inm = [-maxfetch, maxfetch, -maxfetch, maxfetch]

    totRotatedphi = np.zeros(
        (int(domain_output_inpix), int(domain_output_inpix))
    )
    Wfai = np.zeros((iterations, 1))
    Wpai = np.zeros((iterations, 1))
    WzH = np.zeros((iterations, 1))
    ...
    for i in np.arange(0, iterations, 1):
        ...
        phi_totdist[i] = phi_maxdist[i] + np.nansum(
            (
                phi[
                    int(phi_maxdist[i]) : int(  # Flip Klj for wind angle
                        len(phi)
                    ),
                    int((len(phi) / 2)),
                ]
        ...
        rotatedphi = rotatedphi[
            0 : int(domain_output_inpix), 0 : int(domain_output_inpix)
        ]
    ...
    if n == 0:
        ...
        if rs is not None:
            ...
            if crop:
                ...
                xrs = []
                yrs = []
                xrs, yrs = get_contour_vertices(
                    x_2d, y_2d, fclim_2d, clevs[0][2]
                )
...
def get_contour_vertices(x, y, f, lev):
    ...
    if (
        x.min() >= min(segs[0][:, 0])
        or max(segs[0][:, 0]) >= x.max()
        or y.min() >= min(segs[0][:, 1])
        or max(segs[0][:, 1]) >= y.max()
    ):
        return [None, None]
...
def plot_footprint(
    ...
    if clevs is not None:
        ...
        for f, c in zip(fs, cs):
            ...
            if show_footprint:
                cp = ax.contour(
                    x_2d, y_2d, f, levs, colors="r", linewidths=line_width
                )
            else:
                cp = ax.contour(
...
def CalcWeightedMorphVegV2(bld, veg, porosity, rotatedphi, wd_input, scale):
    ...
    build = bld
    build[(build < 2.0)] = (
        0.0  # building should be higher than 2 meter therefore 'gets rid' of ones that aren't
    )
    # Rid of where overlap in buildings and veg
    # boolean where veg and builds >0 (i.e. overlap)
    bool_vb = (veg > 0) * (build > 0)
    veg = veg * ~bool_vb
    ...
    # weight foorprint function of pixels
    W_isb_fp = np.nansum(isb_fp * rotatedphi)
    # plan area index veg
    isv_fp = (veg > 0) * fpA  # veg pixels in source area
    # weight foorprint function of pixels
    W_isv_fp = np.nansum(isv_fp * rotatedphi)
    ...
    build_fp[np.where(build_fp == 0.0)] = np.nan
    veg_fp[np.where(veg_fp == 0.0)] = np.nan
    buildveg_fp[np.where(buildveg_fp == 0.0)] = np.nan
    ...
    # rot_dsm[rot_dsm<0]=0
    # TESTING### remove [0]
    # rotated buildings into wind direction
    rot_build = scnd.rotate(
        build, wd_input, order=0, reshape=False, mode="nearest"
    )
    rot_build[rot_build < 0] = 0
    # rotated veg into wind direction
    rot_veg = scnd.rotate(
        veg, wd_input, order=0, reshape=False, mode="nearest"
    )
    rot_veg[rot_veg < 0] = 0
    ...
    # Dont rotate cos already rotated
    rot_phi = scnd.rotate(
        rotatedphi, wd_input, order=0, reshape=False, mode="nearest"
    )
    rot_phi[rot_phi < 0] = 0
    d = np.shape(build)
    # buildings
    ...
    wall[np.where(wall <= 2.0)] = 0
    wall[np.where(rot_build == 0.0)] = 0
    frontbld_fp = np.float32(wall) * (rot_phi > 0)
    # weight by footprint function
    W_frontbld_fp = np.nansum(frontbld_fp * rot_phi)
    d_input = 1 / scale
    ...
    leaf[np.where(rot_veg == 0.0)] = 0
    frontveg_fp = np.float32(leaf) * (rot_phi > 0)
    # weight by footprint function
    W_frontveg_fp = np.nansum(frontveg_fp * rot_phi)
    ...
    Pv = (
        ...
        + ((0.489 * porosity) / 1.2)
        + (0.803 / 1.2)
    )  # factor accounting for porosity
...
# This should move at some point (Utilities.misc)
