...
def create_GridLayout_dict():
    ...
    # geom
    # height of top of each layer (start with 0 i.e. one more than nlayers)
    ssDict["height"] = [0.0, 11.0, 15.0, 22.0]
    # fraction of building coverage of each layer; the first one is plan area index of buildings
    ssDict["building_frac"] = [0.43, 0.38, 0.2]
    # fraction of vegetation coverage of each layer
    ssDict["veg_frac"] = [0.01, 0.02, 0.01]
    # building scale of each layer [m]
    ssDict["building_scale"] = [50.0, 50.0, 50]
    ssDict["veg_scale"] = [
        10.0,
        10.0,
        10,
    ]  # vegetation scale of each layer [m]

    # roof
    ssDict["sfr_roof"] = [0.3, 0.3, 0.4]  # (sum should be 1)
    ssDict["tin_roof"] = [5, 5, 6]  # initial temperatures of roofs [degC]
    ssDict["alb_roof"] = [0.5, 0.5, 0.2]  # T albedo of roofs
    ssDict["emis_roof"] = [0.95, 0.95, 0.95]  # emissivity of roofs
    ...
    ssDict["dz_roof(1,:)"] = [0.2, 0.1, 0.1, 0.01, 0.01]
    # TODO thermal conductivity of each layer [W/m/K]
    ssDict["k_roof(1,:)"] = [1.2, 1.2, 1.2, 1.2, 1.2]
    # TODO specific heat capacity of each layer [J/kg/K]
    ssDict["cp_roof(1,:)"] = [2e6, 2e6, 2e6, 2e6, 2e6]

    ssDict["dz_roof(2,:)"] = [0.2, 0.1, 0.1, 0.01, 0.01]  # TODO
    ssDict["k_roof(2,:)"] = [2.2, 1.2, 1.2, 1.2, 1.2]  # TODO
    ssDict["cp_roof(2,:)"] = [2e6, 2e6, 2e6, 2e6, 2e6]  # TODO

    ssDict["dz_roof(3,:)"] = [0.2, 0.1, 0.1, 0.01, 0.01]  # TODO
    ...
    ssDict["sfr_wall"] = [0.3, 0.3, 0.4]  # (sum should be 1)
    ssDict["tin_wall"] = [5, 5, 5]
    ssDict["alb_wall"] = [0.5, 0.5, 0.5]
    ssDict["emis_wall"] = [0.95, 0.95, 0.95]
...
def writeGridLayout(ssVect, fileCode, featID, outputFolder, gridlayoutIn):
    ...
    for r in range(1, gridlayoutIn[featID]["nlayer"] + 1):
        ssDict["dz_roof(" + str(r) + ",:)"] = gridlayoutIn[featID][
            "dz_roof(" + str(r) + ",:)"
        ]
        ssDict["k_roof(" + str(r) + ",:)"] = gridlayoutIn[featID][
            "k_roof(" + str(r) + ",:)"
        ]
        ssDict["cp_roof(" + str(r) + ",:)"] = gridlayoutIn[featID][
            "cp_roof(" + str(r) + ",:)"
        ]
    ...
    for r in range(1, gridlayoutIn[featID]["nlayer"] + 1):
        ssDict["dz_wall(" + str(r) + ",:)"] = gridlayoutIn[featID][
            "dz_wall(" + str(r) + ",:)"
        ]
        ssDict["k_wall(" + str(r) + ",:)"] = gridlayoutIn[featID][
            "k_wall(" + str(r) + ",:)"
        ]
        ssDict["cp_wall(" + str(r) + ",:)"] = gridlayoutIn[featID][
            "cp_wall(" + str(r) + ",:)"
        ]
    ...
    write_GridLayout_file(
        ssDict, outputFolder + "/", "GridLayout" + fileCode + str(featID)
    )
...
def write_GridLayout_file(ss_object, refdir, fname):
    ...
    for j in range(1, ss_object["nlayer"] + 1):
        f.write(
            ...
            + str(j)
            + ",:) = {}\n".format(
                str(ss_object["dz_roof(" + str(j) + ",:)"])[1:-1]
            )
        )
        f.write(
            "k_roof("
            + str(j)
            + ",:) = {}\n".format(
                str(ss_object["k_roof(" + str(j) + ",:)"])[1:-1]
            )
        )
        f.write(
            "cp_roof("
            + str(j)
            + ",:) = {}\n".format(
                str(ss_object["cp_roof(" + str(j) + ",:)"])[1:-1]
            )
        )
        f.write("\n")
    ...
    for j in range(1, ss_object["nlayer"] + 1):
        f.write(
            ...
            + str(j)
            + ",:) = {}\n".format(
                str(ss_object["dz_wall(" + str(j) + ",:)"])[1:-1]
            )
        )
        f.write(
            "k_wall("
            + str(j)
            + ",:) = {}\n".format(
                str(ss_object["k_wall(" + str(j) + ",:)"])[1:-1]
            )
        )
        f.write(
            "cp_wall("
            + str(j)
            + ",:) = {}\n".format(
                str(ss_object["cp_wall(" + str(j) + ",:)"])[1:-1]
            )
        )
        f.write("\n")
