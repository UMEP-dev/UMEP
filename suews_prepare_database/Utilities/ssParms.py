...
def getVertheights(ssVect, heightMethod, vertHeightsIn, nlayerIn, skew, id):
    ...
    if heightMethod > 1:  # determine if exponential height should be used
        # TODO: Fix if no buildings and/or no veg is present.
        intervals = np.ceil(ssVect["z"].max() / nlayerOut)
        zMax = np.ceil(ssVect["z"].max())
        heightIntervals = []
        heightIntervals.append(0.0)
        for i in range(1, nlayerOut):
            t = (intervals * i) / zMax
            y = 0.0 + (zMax - 0.0) * (np.exp(skew * t) - 1) / (
                np.exp(skew) - 1
...
def calculate_fractions(dictTypofrac, heights):
    ...
    for i in range(len(heights) - 1):
        ...
        relevant_levels = [
            k for k in dictTypofrac if lower_bound <= k < upper_bound
        ]
        ...
        for level in relevant_levels:
            for typo, data in dictTypofrac[level].items():
                ...
                roof_fraction = (
                    data["roofSum"] / total_roof_sum
                    if total_roof_sum > 0
                    else 0
                )
                wall_fraction = (
                    data["wallSum"] / total_wall_sum
                    if total_wall_sum > 0
                    else 0
                )
                ...
                if typo not in fractions[i]:
                    ...
                    fractions[i][typo]["roofFraction"] += round(
                        roof_fraction, 5
                    )
                    fractions[i][typo]["wallFraction"] += round(
                        wall_fraction, 5
                    )
...
def ss_calc_gridlayout(
    ...
    ssVect = pd.read_csv(
        ss_dir + "/" + pre + "_IMPGrid_SS_" + str(id) + ".txt", sep="\s+"
    )

    # empty dict to calc fractions for current grid and typology for each meter
    dictTypofrac = {}
    ...
    for i in range(1, len(gridlayoutOut[id]["height"])):
        index += 1
        startH = int(gridlayoutOut[id]["height"][index - 1])
        endH = int(gridlayoutOut[id]["height"][index])
        if index == 1:
            vertical_layers["building_frac"]["value"].append(
                ssVect.loc[0, "paib"]
            )  # first is plan area index of buildings
            vertical_layers["veg_frac"]["value"].append(
                ssVect.loc[0, "paiv"]
            ...
            vertical_layers["veg_frac"]["value"].append(
                np.round(
                    np.mean(
                        ...
                        ssVect.loc[startH:endH, "paiv"]
                    ),
                    3,
                )
            )

        vertical_layers["building_scale"]["value"].append(
            np.round(
                np.mean(
                    # intergrated bscale mean in ith vertical layer
                    ssVect.loc[startH:endH, "bScale"]
                ),
                3,
            )
        )
        vertical_layers["veg_scale"]["value"].append(
            np.round(
                np.mean(
                    # intergrated vscale mean in ith vertical layer
                    ssVect.loc[startH:endH, "vScale"]
                ),
                3,
            )
        )
    ...
    if len(typoList) != 1:  # more than one typology
        ...
        # Calculate fractions
        typ_frac_heights_dict = calculate_fractions(
            dictTypofrac, gridlayoutOut[id]["height"]
        )

        typology_ss_dict = {}
        for typology in list(grid_dict.keys()):  # typoList[1:]:
            building_code = db_dict["Types"].loc[typology, "Buildings"]
            spartacus_code = db_dict["NonVeg"].loc[
                building_code, "Spartacus Surface"
            ]
            ...
            typology_ss_dict[typology] = {
                ...
                "wall": {
                    "thermal_layers": {
                        "value": horizontal_aggregation(
                            spartacus_code, "w", db_dict, no_rho=False
                        )
                    },
                    "albedo": db_dict["Spartacus Material"].loc[
                        (
                            db_dict["Spartacus Surface"].loc[
                                spartacus_code, "w1Material"
                            ]
                        ),
                        "Albedo",
                    ],
                    "emissivity": db_dict["Spartacus Material"].loc[
                        (
                            db_dict["Spartacus Surface"].loc[
                                spartacus_code, "w1Material"
                            ]
                ...
                "roof": {
                    "thermal_layers": {
                        "value": horizontal_aggregation(
                            spartacus_code, "r", db_dict, no_rho=False
                        )
                    },
                    "albedo": db_dict["Spartacus Material"].loc[
                        (
                            db_dict["Spartacus Surface"].loc[
                                spartacus_code, "r1Material"
                            ]
                        ),
                        "Albedo",
                    ],
            ...
            for vert_heights in list(typ_frac_heights_dict.keys()):
                ...
                typology_ss_dict[typology][vert_heights] = {
                    "wallfrac": typ_frac_heights_dict[vert_heights][typology][
                        "wallFraction"
                    ],
                    "rooffrac": typ_frac_heights_dict[vert_heights][typology][
                        "roofFraction"
                    ],
                }
        ...
        for surface in ["roof", "wall"]:
            ...
            for vlayer in range(gridlayoutOut[id]["nlayer"]):
                ...
                for hlayer in range(5):
                    dz_wall = [
                        typology_ss_dict[typology][surface]["thermal_layers"][
                            "value"
                        ]["dz"]["value"][hlayer]
                        for typology in typology_ss_dict
                    ]
                    rho_wall = [
                        typology_ss_dict[typology][surface]["thermal_layers"][
                            "value"
                    ...
                    alb_wall = [
                        typology_ss_dict[typology][surface]["albedo"]
                        for typology in typology_ss_dict
                    ]
                    emis_wall = [
                        typology_ss_dict[typology][surface]["emissivity"]
                        for typology in typology_ss_dict
                    ]

                    weights = [
                        typ_frac_heights_dict[vlayer][typology][
                            f"{surface}Fraction"
                        ]
                        for typology in typology_ss_dict
                    ]
                    ...
                    filtered_dz_wall = [
                        dz_wall[i]
                        for i in range(len(dz_wall))
                        if not np.isnan(dz_wall[i])
                    ]
                    if len(filtered_dz_wall) > 0:

                        filtered_rho_wall = [
                            rho_wall[i]
                            for i in range(len(rho_wall))
                            if not np.isnan(rho_wall[i])
                        ]
                        filtered_cp_wall = [
                            cp_wall[i]
                            for i in range(len(cp_wall))
                            if not np.isnan(cp_wall[i])
                        ]
                        filtered_k_wall = [
                            k_wall[i]
                            for i in range(len(k_wall))
                            if not np.isnan(k_wall[i])
                        ]
                        ...
                        dz_agg = np.average(
                            filtered_dz_wall, weights=normalized_weights
                        )

                        # Weighted average of density (if needed, but you mention rho not in YAML)
                        rho_agg = np.average(
                            filtered_rho_wall, weights=normalized_weights
                        )

                        # Aggregate thermal conductivity
                        r_wall = [
                            dz / k
                            for dz, k in zip(filtered_dz_wall, filtered_k_wall)
                        ]
    ...
    # TODO Add reference texts
        ...
        for surface in ["roof", "wall"]:
            ...
            for vlayer in range(0, gridlayoutOut[id]["nlayer"]):
                ...
                try:
                    layer["alb"]["ref"] = {
                        "desc": db_dict["Spartacus Material"].loc[
                            spartacus_sel[f"{surface[0]}1Material"],
                            "nameOrigin",
                        ],
