    ...
    for col in sheets:
        ...
        if col == "Name":
            db[col]["nameOrigin"] = (
                ...
                + ", "
                + db[col]["Origin"].astype(str)
            )
        elif col == "References":
            db[col]["authorYear"] = (
                db[col]["Author"].astype(str)
                + ", "
                + db[col]["Year"].astype(str)
            )
        elif col == "Country":
            db[col]["nameOrigin"] = (
                db[col]["Country"].astype(str)
                + ", "
                + db[col]["City"].astype(str)
            )
        elif col == "Region":
            pass
        ...
        # Calculate U-values for roof and wall new columns u_value_wall and u_value_roof
            ...
            buildings = db["Spartacus Surface"][
                db["Spartacus Surface"]["Surface"] == "Buildings"
            ]
            ...
            for prefix in ["w", "r"]:
                ...
                if prefix == "w":
                    ...
                    pr = "roof"
                materials = buildings[
                    [f"{prefix}{i}Material" for i in range(1, 6)]
                ].values
                thicknesses = buildings[
                    [f"{prefix}{i}Thickness" for i in range(1, 6)]
                ].values

                thicknesses[isnan(thicknesses)] = 0

                for i in range(0, 5):
                    materials[isnan(materials)] = materials[
                        nonzero(isnan(materials))[0],
                        nonzero(isnan(materials))[1] - 1,
                    ]

                thermal_conductivities = vectorize(
                    lambda x: db["Spartacus Material"].loc[
                ...
                db["Spartacus Surface"].loc[
                    buildings.index, f"u_value_{pr}"
                ] = u_values
            ...
            for prop in ["Albedo", "Emissivity"]:
                for prefix, pr in zip(["w", "r"], ["wall", "roof"]):
                    ...
                    db["Spartacus Surface"].loc[
                        buildings.index, f"{prop.lower()}_{pr}"
                    ] = (
                        db["Spartacus Material"]
                        .loc[buildings[material_col], prop]
                        .values
                    )

        elif col == "Profiles":
            # Normalise traffic and energy use profiles to ensure that average of all columns = 1
            normalisation_rows = db[col][
                (db[col]["Profile Type"] == "Traffic")
                | (db[col]["Profile Type"] == "Energy use")
            ]
            cols = list(range(24))
            ...
            # Scale the values
            db[col].loc[normalisation_rows_index, cols] = (
                db[col]
                .loc[normalisation_rows_index, cols]
                .multiply(scaling_factors, axis=0)
            )
...
def read_morph_txt(txt_file):
    ...
    """
    morph_dict = read_csv(txt_file, sep=r"\s+", index_col=[0]).to_dict(
        orient="index"
    )
    return morph_dict
...
def check_fraction_consistency(ss_dict):
    ...
    building_frac_lc = ss_dict["properties"]["land_cover"]["bldgs"]["sfr"][
        "value"
    ]
    paved_frac_lc = ss_dict["properties"]["land_cover"]["paved"]["sfr"][
        "value"
    ]
    ...
    building_frac_spartacus = ss_dict["properties"]["vertical_layers"][
        "building_frac"
    ]["value"][0]
    ...
    if abs(diff) == 0:
        ...
        print(
            f"Small difference of {round(abs(diff), 4)} between PAI calculated from vertical morphology and Building Fraction detected, OK"
        )
        ...
        if dataset_of_choice == 0:
            diff = abs(diff)
            ss_dict["properties"]["vertical_layers"]["building_frac"]["value"][
                0
            ] = building_frac_lc

        elif dataset_of_choice == 1:
            diff = abs(diff)
            ss_dict["properties"]["land_cover"]["paved"]["sfr"]["value"] = (
                paved_frac_lc - diff
            )
            ss_dict["properties"]["land_cover"]["bldgs"]["sfr"]["value"] = (
                building_frac_lc + diff
            )

    elif abs(diff) > 0.03:
        print("error! ")
        print(
            f"Unreasonable Difference of {round(abs(diff), 4)} between PAI calculated from vertical morphology and Building Fraction detected"
        )
...
def decide_country_or_region(col, country_sel, reg):
    if str(country_sel[col].item()) == "nan":
        var = reg.loc[
            reg["Region"] == country_sel["Region"].item(), col
        ].item()
    else:
...
def horizontal_aggregation(surface_code, roofwall, db_dict, no_rho=False):
    ...
    for layer in range(1, 6):
        try:
            mat = db_dict["Spartacus Material"].loc[
                surface[f"{roofwall}{layer}Material"]
            ]
    ...
    if insulation == 6:
        ...
        agg_surface.loc[1, "k"] = sum(d_list) / sum(
            d / k for d, k in zip(d_list, k_list)
        )
        agg_surface.loc[1, "rho"] = sum(
            d * rho for d, rho in zip(d_list, rho_list)
        ) / sum(d_list)
        agg_surface.loc[1, "cp"] = sum(
            d * rho * cp for d, rho, cp in zip(d_list, rho_list, cp_list)
        ) / (agg_surface.loc[1, "rho"] * agg_surface.loc[1, "dz"])
    ...
    # If insulation Exists.
        ...
        if len(layer1) == 1:
            mat = db_dict["Spartacus Material"].loc[
                surface[f"{roofwall}{layer1[0]}Material"]
            ]
            agg_surface.loc[1, "dz"] = surface[
                f"{roofwall}{layer1[0]}Thickness"
            ]
            ...
            for layer in layer1:
                ...
                agg_surface.loc[1, "rho"] = sum(
                    d * k for d, k in zip(d_list, rho_list)
                ) / sum(d_list)
                agg_surface.loc[1, "cp"] = sum(
                    d * rho * cp
                    for d, rho, cp in zip(d_list, rho_list, cp_list)
                ) / (agg_surface.loc[1, "rho"] * agg_surface.loc[1, "dz"])
        ...
        mat = db_dict["Spartacus Material"].loc[
            surface[f"{roofwall}{layer2}Material"]
        ]
        ...
        if len(layer3) > 0:
            ...
            if len(layer3) == 1:
                mat = db_dict["Spartacus Material"].loc[
                    surface[f"{roofwall}{layer3[0]}Material"]
                ]
                agg_surface.loc[3, "dz"] = surface[
                    f"{roofwall}{layer3[0]}Thickness"
                ]
                ...
                for layer in layer3:
                    ...
                    for layer in layer3:
                        ...
                        agg_surface.loc[3, "k"] = sum(d_list) / sum(
                            d / k for d, k in zip(d_list, k_list)
                        )
                        agg_surface.loc[3, "rho"] = sum(
                            d * k for d, k in zip(d_list, rho_list)
                        ) / sum(d_list)
                        agg_surface.loc[3, "cp"] = sum(
                            d * rho * cp
                            for d, rho, cp in zip(d_list, rho_list, cp_list)
                        ) / (
                            agg_surface.loc[3, "rho"]
                            * agg_surface.loc[3, "dz"]
                        )
    ...
    agg_surface = {
        key: list(value.values()) for key, value in agg_surface.items()
    }
...
def findwalls(arr_dsm, walllimit):
    ...
    footprint = array([[0, 1, 0], [1, 0, 1], [0, 1, 0]])
    ...
    for profile in profiles_list:
        ...
        for dict_value in profile_dict:
            for key in list(profile_dict[dict_value].keys()):
                profile_dict[dict_value][key] = round(
                    profile_dict[dict_value][key], 2
                )
        ...
        "laipower": {
            ...
            "growth_lai": {
                "value": db_dict["Leaf Growth Power"].loc[
                    LGP_code, "LeafGrowthPower1"
                ],
                "ref": {
                    "desc": db_dict["Leaf Growth Power"].loc[
                        LGP_code, "nameOrigin"
                    ],
                    "ID": str(LGP_code),
                    "DOI": zenodo,
...
def fill_bare_soil_yaml(
    indict, db_dict, LCF_baresoil, irrFr, zenodo, temp_bsoil
):
    """
     OB 20241216
    ...
    temp_bsoil["soildepth"] = {
        ...
        "ref": {
            "desc": db_dict["Soil"].loc[
                indict["SoilTypeCode"]["value"], "nameOrigin"
            ],
...
def fill_water_yaml(
    indict, db_dict, LCF_water, IrrFr_Water, zenodo, temp_water
):
    """
    OB 20241216
...
def fill_veg_yaml(
    ...
    temp_veg["storedrainprm"] = {
        ...
        # TODO THIS IS probably wrong this is current storage according to manual
        "store_cap": veg_dict[surface]["StorageMax"],
    ...
    temp_veg = temp_veg | fill_BiogenCO2_yaml(
        veg_dict[surface], db_dict, zenodo
    )
    temp_veg = temp_veg | fill_soil_yaml(
        db_dict, veg_dict[surface], "Veg", zenodo
    )
...
def fill_nonveg_yaml(
    ...
    temp_nonveg["storedrainprm"] = {
        ...
        # TODO THIS IS probably wrong this is current storage according to manual
        "store_cap": nonveg_dict[surface]["StorageMax"],
    ...
    temp_nonveg = temp_nonveg | fill_soil_yaml(
        db_dict, nonveg_dict[surface], "NonVeg", zenodo
    )
    ...
    anthropogenic_emissions = {
        ...
        "co2": {
            ...
            "frfossilfuel_nonheat": {
                "value": AnEm_sel["FrFossilFuel_NonHeat"]
            },
...
def fill_SUEWS_NonVeg_typologies(
    db_dict, parameter_dict, locator_code, zenodo, surface
):
    """
    ...
    # 'ref':{'desc': 'N/A', 'ID': 'N/A', 'DOI': zenodo}}}
    table_dict = {"Code": {"value": locator}}
    ...
    for key, (db_key, value_col, desc_col) in keys.items():
        table_dict[key] = {
            ...
            "ref": {
                "desc": (
                    db_dict[db_key].loc[locator[db_key], desc_col]
                    if desc_col
                    else f"{key} value"
                ),
    ...
    table_dict["SoilTypeCode"] = {
        ...
        "ref": {
            "desc": db_dict["Soil"].loc[
                parameter_dict["SoilTypeCode"], "nameOrigin"
            ],
    ...
    table_dict["Spartacus Surface"] = {
        ...
        "ref": {
            "desc": db_dict["Spartacus Surface"].loc[
                locator["Spartacus Surface"], "nameOrigin"
            ],
...
def blend_SUEWS_NonVeg(grid_dict, db_dict, parameter_dict, zenodo, surface):
    ...
    for typology in typology_list:
        ...
        temp_nonveg_dict[typology] = fill_SUEWS_NonVeg_typologies(
            db_dict, parameter_dict, locator, zenodo, surface
        )
        fractions.append(grid_dict[typology]["SAreaFrac"])
        temp_nonveg_dict[typology]["Code"]["value"] = typology
        temp_nonveg_dict[typology]["Code"]["desc"] = db_dict["Types"].loc[
            typology, "nameOrigin"
        ]
    ...
    for param in param_list:
        values_dict[param] = [
            temp_nonveg_dict[typology][param] for typology in typology_list
        ]
    ...
    def aggregate_values(param):
        ...
        if param in [
            ...
            weighted_values = average(
                [v["value"] for v in values_dict[param]], weights=fractions
            )
            ...
            desc_items = [
                f"{perc:.1f}% {desc}" for desc, perc in desc_dict.items()
            ]
            if len(desc_items) == 1:
        ...
        return {
            "value": round(weighted_values, 3),
            "ref": {"desc": desc, "ID": ids, "DOI": zenodo},
        }

    # Create the new aggregated typology dictionary
    new_edit = {
        # Assuming create_code('NonVeg') generates a new code
        "Code": "Aggregated",
        ...
        "Spartacus": {
            ...
            "ID": [
                db_dict["NonVeg"].loc[
                    db_dict["Types"].loc[typology, "Buildings"],
                    "Spartacus Surface",
                ]
                for typology in typology_list
            ],
        ...
        "ref": {
            "desc": "Aggregated from: "
            + ", ".join(
                [
                    f"{frac*100:.1f}% {temp_nonveg_dict[typology]['Code']['desc']}"
                    for typology, frac in zip(typology_list, fractions)
                ]
            ),
            # ', '.join([str(temp_nonveg_dict[typology]['Code']['value']) for typology in typology_list]),
            "ID": ", ".join(
                [
                    str(temp_nonveg_dict[typology]["Code"]["value"])
                    for typology in typology_list
                ]
            ),
            "DOI": zenodo,
...
def fill_SUEWS_Veg(db_dict, settings_dict, parameter_dict, zenodo):
    ...
    for surface in ["Evergreen Tree", "Deciduous Tree", "Grass"]:
        ...
        bioco2Code = db_dict["Veg"].loc[
            table_dict[surface]["Code"]["value"], "Biogen CO2"
        ]
        ...
        # Add the ohm_coef dictionary to the table dictionary
        table_dict[surface]["ohm_coef"] = fill_ohm_coefs(
            db_dict, locator, zenodo
        )
...
def fill_SUEWS_Water(surface_code, db_dict, parameter_dict, zenodo):
    ...
    # Initialize the table dictionary with the typology code
    # 'Code': {'value': locator.name, 'ref':{'desc': 'N/A', 'ID': 'N/A', 'DOI': zenodo}}}
    table_dict = {}
...
def update_desc_with_materials(data_dict, db_dict, variable):
    ...
    for ID, percent in zip(data_dict["ID"], data_dict["desc"]):
        ...
        merge_string = (
            merge_string + str(round(percent, 1)) + "% " + mat + ", "
        )
...
def update_desc_with_surface(data_dict, db_dict):
    ...
    for ID, percent in zip(data_dict["ID"], data_dict["desc"]):
        ...
        merge_string = (
            merge_string + str(round(percent, 1)) + "% " + surface + ", "
        )
