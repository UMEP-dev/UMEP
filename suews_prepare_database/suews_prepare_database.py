...
class SUEWSPrepareDatabase(object):
    ...
    def initGui(self):
        ...
        self.iface.addPluginToMenu(
            self.tr("&SUEWS Prepare - Database Typologies"), self.action
        )
    ...
    def unload(self):
        ...
        self.iface.removePluginMenu(
            "&SUEWS Prepare - Database Typologies", self.action
        )
    ...
    def run(self):
        ...
        self.actions = []
        self.menu = self.tr("&SUEWS Prepare (Database Typologies)")
        ...
        # Read Database
        self.db_path = (
            self.plugin_dir.split("suews_prepare_database")[0]
            + "suews_database_manager/data/database.xlsx"
        )
        ...
        building_list = db_dict["NonVeg"]["nameOrigin"].loc[
            db_dict["NonVeg"]["Surface"] == "Buildings"
        ]
        paved_list = db_dict["NonVeg"]["nameOrigin"].loc[
            db_dict["NonVeg"]["Surface"] == "Paved"
        ]
        bsoil_list = db_dict["NonVeg"]["nameOrigin"].loc[
            db_dict["NonVeg"]["Surface"] == "Bare Soil"
        ]
        grass_list = db_dict["Veg"]["nameOrigin"].loc[
            db_dict["Veg"]["Surface"] == "Grass"
        ]
        dec_tree_list = db_dict["Veg"]["nameOrigin"].loc[
            db_dict["Veg"]["Surface"] == "Deciduous Tree"
        ]
        evr_tree_list = db_dict["Veg"]["nameOrigin"].loc[
            db_dict["Veg"]["Surface"] == "Evergreen Tree"
        ]
        anthro_list = db_dict["AnthropogenicEmission"]["nameOrigin"]  # steg1
        traffic_listWD = (
            db_dict["Profiles"]["nameOrigin"]
            .loc[db_dict["Profiles"]["Profile Type"] == "Traffic"]
            .loc[db_dict["Profiles"]["Day"] == "Weekday"]
        )
        traffic_listWE = (
            db_dict["Profiles"]["nameOrigin"]
            .loc[db_dict["Profiles"]["Profile Type"] == "Traffic"]
            .loc[db_dict["Profiles"]["Day"] == "Weekend"]
        )
        human_listWD = (
            db_dict["Profiles"]["nameOrigin"]
            .loc[db_dict["Profiles"]["Profile Type"] == "Human activity"]
            .loc[db_dict["Profiles"]["Day"] == "Weekday"]
        )
        human_listWE = (
            db_dict["Profiles"]["nameOrigin"]
        ...
        popdens_listWD = (
            db_dict["Profiles"]["nameOrigin"]
            .loc[db_dict["Profiles"]["Profile Type"] == "Population density"]
            .loc[db_dict["Profiles"]["Day"] == "Weekday"]
        )
        popdens_listWE = (
            db_dict["Profiles"]["nameOrigin"]
        ...
        energy_listWD = (
            db_dict["Profiles"]["nameOrigin"]
            .loc[db_dict["Profiles"]["Profile Type"] == "Energy use"]
            .loc[db_dict["Profiles"]["Day"] == "Weekday"]
        )
        energy_listWE = (
            db_dict["Profiles"]["nameOrigin"]
        ...
        snow_listWD = (
            db_dict["Profiles"]["nameOrigin"]
            .loc[db_dict["Profiles"]["Profile Type"] == "Snow removal"]
            .loc[db_dict["Profiles"]["Day"] == "Weekday"]
        )
        snow_listWE = (
            db_dict["Profiles"]["nameOrigin"]
        ...
        wateruse_listWD = (
            db_dict["Profiles"]["nameOrigin"]
            .loc[db_dict["Profiles"]["Profile Type"] == "Water use (manual)"]
            .loc[db_dict["Profiles"]["Day"] == "Weekday"]
        )
        wateruse_listWE = (
            db_dict["Profiles"]["nameOrigin"]
        ...
        wateruse_listAWD = (
            db_dict["Profiles"]["nameOrigin"]
            .loc[
                db_dict["Profiles"]["Profile Type"] == "Water use (automatic)"
            ]
            .loc[db_dict["Profiles"]["Day"] == "Weekday"]
        )
        wateruse_listAWE = (
        ...
        for cbox in [
            self.dlg.comboBoxPaved,
            self.dlg.comboBoxBuilding,
            self.dlg.comboBoxBareSoil,
            ...
            self.dlg.comboBoxPopdensWD,
            self.dlg.comboBoxPopdensWE,
            self.dlg.comboBoxEnergyUseWD,
            self.dlg.comboBoxEnergyUseWE,
            self.dlg.comboBoxIrrigationAWD,
            self.dlg.comboBoxIrrigationAWE,
            self.dlg.comboBoxIrrigationWD,
            self.dlg.comboBoxIrrigationWE,
        ]:  # steg3
            cbox.setCurrentIndex(-1)

        self.dlg.comboBoxCountry.addItems(
            sorted(set(list(db_dict["Country"]["nameOrigin"])))
        )
        self.dlg.comboBoxCountry.setCurrentIndex(-1)

        self.dlg.comboBoxCountry.currentIndexChanged.connect(
            lambda: self.country_changed(db_dict)
        )

        self.dlg.checkBoxTypology.stateChanged.connect(
            lambda: self.use_typologies()
        )
        self.dlg.checkBox_day.stateChanged.connect(lambda: self.popdaystate())
        self.dlg.pop_density.currentIndexChanged.connect(
            lambda: self.set_pop_density()
        )
        self.dlg.pop_density_day.currentIndexChanged.connect(
            lambda: self.set_pop_density_day()
        )

        self.dlg.layerComboManagerPolygrid.setCurrentIndex(-1)
        self.dlg.layerComboManagerPolygrid.setFilters(
            QgsMapLayerProxyModel.Filter.PolygonLayer
        )
        self.dlg.layerComboManagerPolygrid.setFixedWidth(175)
        self.dlg.layerComboManagerPolyField.setFilters(
            QgsFieldProxyModel.Filter.LongLong
        )
        self.dlg.layerComboManagerPolygrid.layerChanged.connect(
            self.dlg.layerComboManagerPolyField.setLayer
        )

        # New for Typology database
        self.dlg.layerComboManagerDSM.setFilters(
            QgsMapLayerProxyModel.Filter.RasterLayer
        )
        self.dlg.layerComboManagerDSM.setFixedWidth(175)
        self.dlg.layerComboManagerDSM.setCurrentIndex(-1)
        self.dlg.layerComboManagerDEM.setFilters(
            QgsMapLayerProxyModel.Filter.RasterLayer
        )
        self.dlg.layerComboManagerDEM.setFixedWidth(175)
        self.dlg.layerComboManagerDEM.setCurrentIndex(-1)

        self.dlg.layerComboManagerPolygridTypo.setFilters(
            QgsMapLayerProxyModel.Filter.PolygonLayer
        )
        ...
        self.dlg.layerComboManagerPolygrid.layerChanged.connect(
            self.dlg.pop_density.setLayer
        )

        self.dlg.pop_density_day.setFilters(QgsFieldProxyModel.Filter.Numeric)
        self.dlg.layerComboManagerPolygrid.layerChanged.connect(
            self.dlg.pop_density_day.setLayer
        )

        self.dlg.pushButtonImportLCF.clicked.connect(
            lambda: self.set_LCFfile_path()
        )
        self.dlg.pushButtonImportIMPVeg.clicked.connect(
            lambda: self.set_IMPvegfile_path()
        )

        self.dlg.checkBox_twovegfiles.clicked.connect(
            lambda: self.use_twovegfiles()
        )
        self.dlg.pushButtonImportIMPVeg_eve.clicked.connect(
            lambda: self.set_IMPvegfile_path_eve()
        )
        self.dlg.pushButtonImportIMPVeg_dec.clicked.connect(
            lambda: self.set_IMPvegfile_path_dec()
        )

        self.dlg.pushButtonImportIMPBuild.clicked.connect(
            lambda: self.set_IMPfile_path()
        )
        self.dlg.pushButtonImportMet.clicked.connect(
            lambda: self.set_metfile_path()
        )

        self.dlg.spinBoxStartDLS.valueChanged.connect(
            lambda: self.start_DLS_changed(self.dlg.spinBoxStartDLS.value())
        )
        self.dlg.spinBoxEndDLS.valueChanged.connect(
            lambda: self.end_DLS_changed(self.dlg.spinBoxEndDLS.value())
        )

        self.dlg.spinBoxSoilMoisture.valueChanged.connect(
            lambda: self.soil_moisture_changed(
                self.dlg.spinBoxSoilMoisture.value()
            )
        )
        self.dlg.comboBoxLeafCycle.currentIndexChanged.connect(
            lambda: self.leaf_cycle_changed(
                self.dlg.comboBoxLeafCycle.currentIndex()
            )
        )
        self.dlg.fileCodeLineEdit.textChanged.connect(
            lambda: self.file_code_changed(self.dlg.fileCodeLineEdit.text())
        )
        self.dlg.lineEditUTC.textChanged.connect(
            lambda: self.utc_changed(self.dlg.lineEditUTC.text())
        )

        # SS related GUI things
        self.dlg.checkBoxSS.stateChanged.connect(lambda: self.use_spartacus())
        self.dlg.SS_comboBox.currentIndexChanged.connect(
            lambda: self.height_option_SS(self.dlg.SS_comboBox.currentIndex())
        )
        self.dlg.SS_LineEdit_constant.textChanged.connect(
            lambda: self.vertHeights_changed(
                self.dlg.SS_LineEdit_constant.text()
            )
        )
        self.dlg.spinBoxLayers.valueChanged.connect(
            lambda: self.layersSS_changed(self.dlg.spinBoxLayers.value())
        )
        ...
        self.dlg.layerComboManagerTraffic.setFilters(
            QgsMapLayerProxyModel.Filter.LineLayer
        )
        self.dlg.layerComboManagerTraffic.setFixedWidth(175)
        self.dlg.layerComboManagerTrafficField.setFilters(
            QgsFieldProxyModel.Filter.LongLong
        )
        self.dlg.layerComboManagerTraffic.layerChanged.connect(
            self.dlg.layerComboManagerTrafficField.setLayer
        )
        self.dlg.show()
        self.dlg.adjustSize()
        self.dlg.exec()
    ...
    def country_changed(self, db_dict):
        ...
        reg_sel = (
            db_dict["Country"]
            .loc[db_dict["Country"]["nameOrigin"] == country_sel]["Region"]
            .item()
        )
        ...
        def decide_country_region(col, country_sel, reg_sel, comboBox):
            country_df = db_dict["Country"][
                db_dict["Country"]["nameOrigin"] == country_sel
            ]

            if str(country_df[col].item()) == "nan":
                reg_df = db_dict["Region"][
                    db_dict["Region"]["Region"] == reg_sel
                ]
                var = reg_df[col].item()
        ...
        decide_country_region(
            "Paved", country_sel, reg_sel, self.dlg.comboBoxPaved
        )
        decide_country_region(
            "Buildings", country_sel, reg_sel, self.dlg.comboBoxBuilding
        )
        decide_country_region(
            "Bare Soil", country_sel, reg_sel, self.dlg.comboBoxBareSoil
        )
        decide_country_region(
            "Evergreen Tree", country_sel, reg_sel, self.dlg.comboBoxEvrTree
        )
        decide_country_region(
            "Deciduous Tree", country_sel, reg_sel, self.dlg.comboBoxDecTree
        )
        decide_country_region(
            "Grass", country_sel, reg_sel, self.dlg.comboBoxGrass
        )
        decide_country_region(
            "AnthropogenicCode", country_sel, reg_sel, self.dlg.comboBoxAnthro
        )
        decide_country_region(
            "TraffProfWD", country_sel, reg_sel, self.dlg.comboBoxTrafficWD
        )  # finns bara i country?
        decide_country_region(
            "TraffProfWE", country_sel, reg_sel, self.dlg.comboBoxTrafficWE
        )
        decide_country_region(
            "ActivityProfWD", country_sel, reg_sel, self.dlg.comboBoxHumanWD
        )
        decide_country_region(
            "ActivityProfWE", country_sel, reg_sel, self.dlg.comboBoxHumanWE
        )
        decide_country_region(
            "PopProfWD", country_sel, reg_sel, self.dlg.comboBoxPopdensWD
        )
        decide_country_region(
            "PopProfWE", country_sel, reg_sel, self.dlg.comboBoxPopdensWE
        )
        decide_country_region(
            "EnergyUseProfWD",
            country_sel,
        ...
        decide_country_region(
            "SnowClearingProfWD", country_sel, reg_sel, self.dlg.comboBoxSnowWD
        )
        decide_country_region(
            "SnowClearingProfWE", country_sel, reg_sel, self.dlg.comboBoxSnowWE
        )
    ...
    def generate(self):
        ...
        if self.output_dir is None:
            QMessageBox.critical(
                self.dlg, "Error", "No output directory selected"
            )
            return
        ...
        if poly_field == "":
            QMessageBox.critical(
                None,
                "Error",
                "An attribute field with unique fields must be selected",
            )
            return
        ...
        if self.dlg.pop_density.currentField() == "":
            QMessageBox.critical(
                None,
                "Error",
                "An attribute field including night-time population density (pp/ha) must be selected",
            )
            return
        ...
        if self.daypop == 1:
            if self.dlg.pop_density_day.currentField() == "":
                QMessageBox.critical(
                    None,
                    "Error",
                    "An attribute field including working population density (pp/ha) must be selected",
                )
                return
        ...
        if self.leaf_cycle == 0:
            QMessageBox.critical(
                self.dlg, "Error", "No leaf cycle period has been selected"
            )
            return
        else:
        ...
        if self.IMP_from_file:
            if self.IMPfile_path is None:
                QMessageBox.critical(
                    None,
                    "Error",
                    "Building morphology file has not been provided",
                )
                return
        ...
        if self.IMPveg_from_file:
            if self.IMPvegfile_path is None:
                QMessageBox.critical(
                    None,
                    "Error",
                    "Vegetation morphology file has not been provided",
                )
                return
            if not os.path.isfile(self.IMPvegfile_path[0]):
                QMessageBox.critical(
                    None,
                    "Error",
                    "Could not find the file containing vegetation morphology",
                )
                return
        ...
        if self.LCF_from_file:
            if self.LCFfile_path is None:
                QMessageBox.critical(
                    None,
                    "Error",
                    "Land cover fractions file has not been provided",
                )
                return
        ...
        if self.typologies == 1 and polyTypolayer is None:
            QMessageBox.critical(
                None,
                "Error",
                "No valid Urban typology polygon layer is selected",
            )
            return
        ...
        if self.spartacus == 1:
            if self.heightMethod == 0:
                QMessageBox.critical(
                    self.dlg,
                    "Error",
                    "No method to set height intervals is specified",
                )
                return
                ...
                try:
                    ...
                    QMessageBox.critical(
                        self.dlg,
                        "Error",
                        "One or more inputs in Fixed height [option 1] is non-numeric. "
                ...
                for x in floats:
                    if x > start:
                        ...
                        QMessageBox.critical(
                            self.dlg,
                            "Error",
                            "One or more inputs in Fixed height [option 1] is not increasing.",
                        )
                        return
        ...
        settings_dict = {
            ...
            "Metfile_path": self.Metfile_path,
            "start_DLS": self.start_DLS,
            "end_DLS": self.end_DLS,
            "LCF_from_file": self.LCF_from_file,
            "LCFfile_path": self.LCFfile_path,
            "plugin_dir": self.plugin_dir,
            "map_units": map_units,
            "output_dir": self.output_dir,
            "file_code": self.file_code,
            "utc": self.utc,
            "IMP_from_file": self.IMP_from_file,
            "IMPfile_path": self.IMPfile_path,
            "IMP_z0": self.IMP_z0,
            "IMP_zd": self.IMP_zd,
            "IMP_fai": self.IMP_fai,
            "IMPveg_from_file": self.IMPveg_from_file,
            "IMPvegfile_path": self.IMPvegfile_path,
            "IMPveg_fai_eve": self.IMPveg_fai_eve,
            "IMPveg_fai_dec": self.IMPveg_fai_dec,
            "pop_density": self.pop_density,
            "twovegfiles": self.twovegfiles,
            "IMPvegfile_path_dec": self.IMPvegfile_path_dec,
            # Typology
            "IMPvegfile_path_eve": self.IMPvegfile_path_eve,
            "pop_density_day": self.pop_density_day,
            "daypop": self.daypop,
            "polyTypolayer": polyTypolayer,
            "typologyFieldName": typologyFieldName,
            "dsmlayer": dsmlayer,
            "demlayer": demlayer,
            "region_str": self.region_str,
            "country_str": self.country_str,
            "checkBoxTypologies": self.typologies,
            # Spartacus
            "heightMethod": self.heightMethod,
            "vertheights": self.vertheights,
            "nlayers": self.nlayers,
            "skew": self.skew,
            "ss_dir": self.ss_dir,
            "spartacus": self.spartacus,
        }
    ...
    def generateSiteSelect(self, settings_dict):  # yml-file
        ...
        # Used to collect potential Spartacus error and present for user
        ...
        for var, lookup in zip(
            ...
            try:
                settings_dict[var] = gui_lookup_dict[lookup][
                    settings_dict[var]
                ]
            except:
                for lu in [
                    ...
                    try:
                        settings_dict[var] = gui_lookup_dict[lu][
                            settings_dict[var]
                        ]
                        break
                    except:
        ...
        country_conv_dict = (
            db_dict["Country"]["Country"] + ", " + db_dict["Country"]["City"]
        ).to_dict()
        country_conv_dict_inv = {v: k for k, v in country_conv_dict.items()}

        # Get selected country
        country_sel = db_dict["Country"].loc[
            [country_conv_dict_inv[settings_dict["country_str"]]]
        ]

        # Drop irrelevant columns
        column_list = (
            db_dict["Country"]
            .drop(columns=["Region", "Country", "City"])
            .columns
        )

        # Set parameters from regional or country level and populate parameter_dict
        parameter_dict = {
            column: decide_country_or_region(
                column, country_sel, db_dict["Region"]
            )
        ...
        if settings_dict["twovegfiles"] == 1:
            IMPveg_dict_dec = read_morph_txt(
                settings_dict["IMPvegfile_path_dec"][0]
            )
            IMPveg_dict_eve = read_morph_txt(
                settings_dict["IMPvegfile_path_eve"][0]
            )

        geodata_output = (
            {}
        )  # Dict for storing the output of the QGIS geodata processes

        # New approach 20240219. Create typologylayer if no typoloayer exist
        if settings_dict["checkBoxTypologies"] == 0:
            parin = {
                "INPUT": settings_dict["vlayer"],
                "FIELD": [],
                "SEPARATE_DISJOINT": False,
                "OUTPUT": temp_folder + "/defaultTypo.shp",
            }
            geodata_output["defaulttypo"] = processing.run(
                "native:dissolve", parin
            )

            # defaultTypoID = db_dict['NonVeg'].loc[db_dict['NonVeg']['nameOrigin']==defTypo].index.item()

            defaultTypoID = create_code("Types")
            # db_dict['NonVeg'][(db_dict['NonVeg']['nameOrigin'] == settings_dict['Paved']) & (db_dict['NonVeg']['Surface'] == 'Paved')].index.item()
            db_dict["Types"].loc[defaultTypoID, "Paved"] = settings_dict[
                "Paved"
            ]
            # db_dict['NonVeg'][(db_dict['NonVeg']['nameOrigin'] == settings_dict['Buildings']) & (db_dict['NonVeg']['Surface'] == 'Buildings')].index.item()
            db_dict["Types"].loc[defaultTypoID, "Buildings"] = settings_dict[
                "Buildings"
            ]

            parin = {
                "INPUT": geodata_output["defaulttypo"]["OUTPUT"],
                "FIELD_NAME": "TypolID",
                "FIELD_TYPE": 1,
                "FIELD_LENGTH": 0,
                "FIELD_PRECISION": 0,
            ...
            geodata_output["defaulttypo2"] = processing.run(
                "native:fieldcalculator", parin
            )

            settings_dict["polyTypolayer"] = QgsVectorLayer(
                geodata_output["defaulttypo2"]["OUTPUT"], "typo", "ogr"
            )
        ...
        parin = {
            "INPUT": settings_dict["vlayer"],
            "INPUT_FIELDS": [],
            # 'TEMPORARY_OUTPUT',#urbantypelayer,
            "OUTPUT": temp_folder + "/urbantypelayer.shp",
            "OVERLAY": settings_dict["polyTypolayer"],
            "OVERLAY_FIELDS": [],
            "OVERLAY_FIELDS_PREFIX": "",
        }
        geodata_output["gridded_shp"] = processing.run(
            "native:intersection", parin
        )
        ...
        geodata_output["gridded_shp_diss"] = processing.run(
            "native:dissolve", parin
        )
        ...
        if settings_dict["spartacus"] == 1:
            ...
            # feedback, total) # 0.5 meter difference in kernel filter identify a wall
            walls = findwalls(build_arrW, 0.5)
            saveraster(gdal.Open(filePath_dsm), walls_raster_out, walls)

            # find prefix for filename in IMP-file
            pre = os.path.basename(settings_dict["IMPfile_path"][0])[
                : os.path.basename(settings_dict["IMPfile_path"][0]).find("_")
            ]
        ...
        geodata_output["mean_build_height"] = processing.run(
            "native:zonalstatisticsfb", parin
        )
        ...
        geodata_output["mean_build_height_pixel_count"] = processing.run(
            "native:zonalstatisticsfb", parin
        )

        cols = [
            f.name()
            for f in geodata_output["mean_build_height_pixel_count"][
                "OUTPUT"
            ].fields()
        ]
        datagen = (
        ...
        # Set ID as index in df to be able to slice in df
        df_build_frac = df_build_frac.set_index("ID")

        df_build_frac["pixel_count"] = df_build_frac["pixel_count"] * pixelSize
        df_build_frac["volume"] = (
            df_build_frac["pixel_count"] * df_build_frac["z_mean"]
        )
        ...
        grid_dict = (
            {}
        )  # Dict that holds information on general typologies, uvalues, albedos and emmissivites and fractions
        gridlayoutOut = (
            {}
        )  # Dict that holds information on fractions of, albedos, emmissivitiees and uvalues for GridLaytout.nml
        dir_poly = plugin_dir + "/tempdata/poly_temp.shp"  # move later

        if settings_dict["checkBoxTypologies"] == 0:
            QgsProject.instance().removeMapLayer(
                settings_dict["polyTypolayer"].source()
            )
        ...
        for f in settings_dict["vlayer"].getFeatures():
            ...
            nonVeg_dict[id] = {}
            gridlayoutOut[id] = {}
            df_build_frac.loc[id, "tot_fraction"] = (
                df_build_frac.loc[id, "volume"]
                / df_build_frac.loc[id, "volume"].sum()
            )
            ...
            for row in df_build_frac.loc[[id]].iterrows():
                ...
                # populate fractions in grid_dict
                grid_dict[id][typology]["SAreaFrac"] = fraction
                ...
                locator = db_dict["Spartacus Surface"].loc[
                    db_dict["NonVeg"].loc[
                        db_dict["Types"].loc[typology]["Buildings"]
                    ]["Spartacus Surface"]
                ]
                grid_dict[id][typology]["uvalue_wall"] = locator[
                    "u_value_wall"
                ]
                grid_dict[id][typology]["uvalue_roof"] = locator[
                    "u_value_roof"
                ]
                grid_dict[id][typology]["albedo_roof"] = locator["albedo_roof"]
                grid_dict[id][typology]["albedo_wall"] = locator["albedo_wall"]
                grid_dict[id][typology]["emissivity_roof"] = locator[
                    "emissivity_roof"
                ]
                grid_dict[id][typology]["emissivity_wall"] = locator[
                    "emissivity_wall"
                ]
            ...
            if settings_dict["spartacus"] == 1:
                ...
                if settings_dict["heightMethod"] == 1:
                    if ssVect["z"].max() <= max(vertHeightsIn):
                        error_output = {
                            # issue 773
                            id: f"zMax ({str(ssVect['z'].max())}) is lower than max fixed height  {str(max(vertHeightsIn))}."
                        }
                        print(
                            "error in ID: ",
                            str(id),
                            f". zMax is lower than max fixed height {str(max(vertHeightsIn))}.",
                        )
                        QMessageBox.critical(
                            None,
                            "Error in Vertical Morphology Spartcus (opt 1)",
                            "error in ID: "
                            + str(id)
                            + ". zMax ("
                            + str(ssVect["z"].max())
                            + ") lower than max fixed height "
                            + str(max(vertHeightsIn))
                            + ". Use other method or reduce fixed layer height.",
                        )
                        self.dlg.progressBar.setValue(index)
                        return

                # always nlayers layer based on percentiles
                elif settings_dict["heightMethod"] == 2:
                    nlayerOut = settings_dict["nlayers"]
                    if ssVect["z"].max() <= nlayerOut:
                        error_output = {
                            # issue 773
                            id: f"zMax ({str(ssVect['z'].max())}) is to low to use {str(nlayerOut)} vertical layers."
                        }
                        print(
                            "error in ID: ",
                            str(id),
                            f". zMax is to low to use {str(nlayerOut)} vertical layers.",
                        )
                        QMessageBox.critical(
                            None,
                            "Error in Vertical Morphology Spartcus (opt 2)",
                            "error in ID: "
                            + str(id)
                            + ". zMax ("
                            + str(ssVect["z"].max())
                            + ") is to low when using "
                            + str(nlayerOut)
                            + " vertical layers. Use other method or reduce number of vertical layers.",
                        )
                        self.dlg.progressBar.setValue(index)
                        return
                ...
                clip_spec = gdal.WarpOptions(
                    format="GTiff", cutlineDSName=dir_poly, cropToCutline=True
                )
                gdal.Warp(
                    temp_folder + "/clipbuild.tif",
                    bigraster,
                    options=clip_spec,
                )
                ...
                gdal.Warp(
                    temp_folder + "/clipwall.tif", bigraster, options=clip_spec
                )
                ...
                gdal.Warp(
                    temp_folder + "/cliptypo.tif", bigraster, options=clip_spec
                )
                ...
                buildings_count = count_buildings(
                    dsm_clip_array, dem_clip_array
                )

                gridlayoutOut[id]["wallAreaGrid"] = (
                    wall_array.sum() * pixelSize
                )
            ...
            if len(typology_list) > 1:
                nonVeg_dict[id]["Paved"] = blend_SUEWS_NonVeg(
                    grid_dict[id], db_dict, parameter_dict, zenodo, "Paved"
                )
                nonVeg_dict[id]["Buildings"] = blend_SUEWS_NonVeg(
                    grid_dict[id], db_dict, parameter_dict, zenodo, "Buildings"
                )
            ...
            # if only one typology exists, no need to aggregate/combine/blend
                ...
                if settings_dict["checkBoxTypologies"] == 0:
                    ...
                    nonVeg_dict[id]["Paved"] = fill_SUEWS_NonVeg_typologies(
                        db_dict,
                        parameter_dict,
                        settings_dict["Paved"],
                        zenodo,
                    ...
                    nonVeg_dict[id]["Buildings"] = (
                        fill_SUEWS_NonVeg_typologies(
                            db_dict,
                            parameter_dict,
                            settings_dict["Buildings"],
                            zenodo,
                            "Buildings",
                        )
                    )
            ...
            nonVeg_dict[id]["Bare Soil"] = fill_SUEWS_NonVeg_typologies(
                db_dict,
                parameter_dict,
                settings_dict["Bare Soil"],
                zenodo,
                "Bare Soil",
            )
        ...
        veg_dict = fill_SUEWS_Veg(
            db_dict, settings_dict, parameter_dict, zenodo
        )

        # Save Profiles
        profiles_dict = profiles_to_dict(
            settings_dict, db_dict["Profiles"], db_dict["References"]
        )

        # set correct values and write txt.files based on the parameters found at country/regional level
        snow_dict = fill_SUEWS_Snow(
            parameter_dict["SnowCode"], db_dict, zenodo
        )
        water_dict = fill_SUEWS_Water(
            parameter_dict["Water"], db_dict, parameter_dict, zenodo
        )

        cond_dict = (
            db_dict["Conductance"].loc[parameter_dict["Conductance"]].to_dict()
        )
        ...
        if settings_dict["Metfile_path"] is None:
            ...
            df["datetime"] = (
                pd.to_datetime(df["iy"], format="%Y")
                + pd.to_timedelta(df["id"] - 1, unit="D")
                + pd.to_timedelta(df["it"], unit="h")
                + pd.to_timedelta(df["imin"], unit="m")
            )
            ...
            dectime0 = (
                df["iy"].iloc[0]
                + df["it"].iloc[0] / 24
                + df["imin"].iloc[0] / (60 * 24)
            )
            dectime1 = (
                df["iy"].iloc[1]
                + df["it"].iloc[1] / 24
                + df["imin"].iloc[1] / (60 * 24)
            )
            ...
            YYYY = int(df["iy"].min())
            data_out = (
                f"{self.output_dir[0]}/{self.file_code}_{YYYY}_data_{res}.txt"
            )
            ...
            np.savetxt(
                data_out,
                df_out.to_numpy(),
                fmt="%3d %2d %3d %2d %6.2f %6.2f %6.2f %6.2f %6.2f %6.2f "
        ...
        for feature in settings_dict["vlayer"].getFeatures():
            ...
            # This is used for sun position calculations and have very small effect.
            altitude = 1
            ...
            if settings_dict["twovegfiles"] == 1:
                ...
                # TODO not used yet
                IMPveg_max_eve = IMPveg_dict_eve[feat_id]["zHmax"]
                # TODO not used yet
                IMPveg_sd_eve = IMPveg_dict_eve[feat_id]["zHmax"]
                ...
                # TODO not used yet
                IMPveg_max_eve = IMPveg_dict[feat_id]["zHmax"]
                # TODO not used yet
                IMPveg_sd_eve = IMPveg_dict[feat_id]["zHmax"]
            ...
            if LCF_de == 0 and LCF_ev == 0 and LCF_bu == 0:
                ...
                sdComb = np.sqrt(float(IMP_sd) ** 2.0 / (LCF_bu * float(area)))
            elif LCF_tr != 0 and LCF_bu == 0:
                sdComb = np.sqrt(
            ...
            fai = Pv * (float(IMPveg_fai_eve) + float(IMPveg_fai_dec)) + float(
                IMP_fai
            )
            ...
            try:
                z = (
                    (
                        ...
                        + float(IMPveg_heights_mean_eve) * float(LCF_evergreen)
                        + float(IMPveg_heights_mean_dec) * float(LCF_decidious)
                    )
                    / (
                        float(LCF_buildings)
                        + float(LCF_evergreen)
            ...
            AnEm = fill_AnEm_yaml(
                db_dict,
                parameter_dict["AnthropogenicCode"],
                profiles_dict,
                parameter_dict,
                settings_dict["start_DLS"],
                settings_dict["end_DLS"],
                TrafficRate_WD,
                TrafficRate_WE,
                pop_density_night,
                pop_density_day,
            ...
            temp_grid["properties"]["lat"]["value"] = round(
                float(lonlat[0]), 3
            )
            temp_grid["properties"]["lng"]["value"] = round(
                float(lonlat[1]), 3
            )
            ...
            temp_grid["properties"]["conductance"] = {
                ...
                "ref": {
                    "desc": db_dict["Conductance"].loc[
                        cond_dict["Code"], "nameOrigin"
                    ],
            ...
            temp_grid["properties"]["irrigation"] = fill_irrigation_yaml(
                db_dict["Irrigation"].loc[parameter_dict["IrrigationCode"]],
                profiles_dict,
                zenodo,
            )
            ...
            temp_grid["properties"]["snow"] = fill_snow_yaml(
                db_dict["Snow"], snow_dict, profiles_dict, zenodo
            )

            temp_land_cover = {
                # Paved has no mean height or FAI
                "paved": fill_nonveg_yaml(
                    nonVeg_dict[feat_id],
                    "Paved",
                    db_dict,
                    LCF_paved,
                    ...
                    0,
                    zenodo,
                    temp_grid["properties"]["land_cover"]["paved"],
                ),
                "bldgs": fill_nonveg_yaml(
                    nonVeg_dict[feat_id],
                    "Buildings",
                    db_dict,
                    LCF_buildings,
                    ...
                    IMP_heights_mean,
                    zenodo,
                    temp_grid["properties"]["land_cover"]["bldgs"],
                ),
                "evetr": fill_veg_yaml(
                    veg_dict,
                    db_dict,
                    ...
                    parameter_dict["IrrigationCode"],
                    zenodo,
                    temp_grid["properties"]["land_cover"]["evetr"],
                ),
                "dectr": fill_veg_yaml(
                    veg_dict,
                    db_dict,
                    ...
                    parameter_dict["IrrigationCode"],
                    zenodo,
                    temp_grid["properties"]["land_cover"]["dectr"],
                ),
                # grass has no mean height or FAI
                "grass": fill_veg_yaml(
                    veg_dict,
                    db_dict,
                    "Grass",
                    ...
                    parameter_dict["IrrigationCode"],
                    zenodo,
                    temp_grid["properties"]["land_cover"]["grass"],
                ),
                "bsoil": fill_bare_soil_yaml(
                    nonVeg_dict[feat_id]["Bare Soil"],
                    db_dict,
                    LCF_baresoil,
                    IrrFr_BSoil,
                    zenodo,
                    temp_grid["properties"]["land_cover"]["bsoil"],
                ),
                "water": fill_water_yaml(
                    water_dict,
                    db_dict,
                    LCF_water,
                    IrrFr_Water,
                    zenodo,
                    temp_grid["properties"]["land_cover"]["water"],
                ),
            }
            ...
            if settings_dict["spartacus"] == 1:
                temp_grid["properties"]["vertical_layers"] = gridlayoutOut[
                    feat_id
                ]["vertical_layers"][0]
                temp_grid["properties"]["lambda_c"]["value"] = round(
                    float(
                        (gridlayoutOut[feat_id]["wallAreaGrid"])
                        / (hectare * LCF_buildings)
                    )
                    + 1,
                    4,
                )
            ...
            temp_grid["initial_states"]["evetr"]["gdd_id"] = {
                "value": leaf_cycle_dict[leaf_cycle]["gdd_1_0"]
            }
            temp_grid["initial_states"]["evetr"]["sdd_id"] = {
                "value": leaf_cycle_dict[leaf_cycle]["gdd_2_0"]
            }
            # dectr
            ...
            temp_grid["initial_states"]["dectr"]["gdd_id"] = {
                "value": leaf_cycle_dict[leaf_cycle]["gdd_1_0"]
            }
            temp_grid["initial_states"]["dectr"]["sdd_id"] = {
                "value": leaf_cycle_dict[leaf_cycle]["gdd_2_0"]
            }
            temp_grid["initial_states"]["dectr"]["porosity_id"] = {
                "value": leaf_cycle_dict[leaf_cycle]["porosity0"]
            }
            temp_grid["initial_states"]["dectr"]["decidcap_id"] = {
                "value": leaf_cycle_dict[leaf_cycle]["decidCap0"]
            }
            # grass
            ...
            temp_grid["initial_states"]["grass"]["gdd_id"] = {
                "value": leaf_cycle_dict[leaf_cycle]["gdd_1_0"]
            }
            temp_grid["initial_states"]["grass"]["sdd_id"] = {
                "value": leaf_cycle_dict[leaf_cycle]["gdd_2_0"]
            }
            ...
            #         }] * gridlayoutOut[feat_id]['nlayer']
            # }
            # Check fraction consistency between land cover data and spartcus fractions
            if settings_dict["spartacus"] == 1:
        ...
        with open(
            save_txt_folder + settings_dict["file_code"] + ".yml", "w"
        ) as file:
        ...
        if spartacus_error_dict:
            ...
            iface.messageBar().pushMessage(
                "Warnings related to vertical morphology",
                error_string,
        ...
        is_valid, errors = validate_single_file(
            save_txt_folder + settings_dict["file_code"] + ".yml",
            schema,
            show_details=True,
        )
        if not is_valid:
            ...
            QMessageBox.information(
                ...
                "See information in the Message bar or QGIS Message log",
            )
            iface.messageBar().pushMessage(
                "Process completed with warnings from pydantic testing",
                pydanticError,
                level=Qgis.MessageLevel.Warning,
            )
        else:
            iface.messageBar().pushMessage(
                "Process completed without warnings",
                level=Qgis.MessageLevel.Info,
            )
            QMessageBox.information(
                None, "Process Complete", "Input files for SUEWS generated"
            )
