...
class UWGPrepare:
    ...
    def __init__(self, iface):
        ...
        locale_path = os.path.join(
            self.plugin_dir, "i18n", "UWGPrepare_{}.qm".format(locale)
        )
    ...
    def unload(self):
        ...
        for action in self.actions:
            self.iface.removePluginMenu(self.tr("&UWG Prepare"), action)
    ...
    def run(self):
        ...
        self.layerComboManagerPolygridBT = QgsMapLayerComboBox(
            self.dlg.widgetPolygonLayerBT
        )
        self.layerComboManagerPolygridBT.setCurrentIndex(-1)
        self.layerComboManagerPolygridBT.setFilters(
            QgsMapLayerProxyModel.Filter.PolygonLayer
        )
        ...
        self.dlg.comboBoxClimateZone.currentIndexChanged.connect(
            lambda: self.zone_changed(
                self.dlg.comboBoxClimateZone.currentIndex()
            )
        )
    ...
    def generate(self):
        ...
        if poly is None:
            QMessageBox.critical(
                None, "Error", "No valid Polygon grid layer is selected"
            )
            return
        ...
        if self.zone == 0:
            QMessageBox.critical(
                None, "Error", "Please specify a climate zone."
            )
            return
        else:
        ...
        if not self.dlg.fileCodeLineEdit.text():
            QMessageBox.critical(
                None, "Error", "Please specify a file prefix."
            )
            return
        else:
