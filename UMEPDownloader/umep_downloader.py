...
def getLayerMetadata(baseURL, layer_name):
    ...
    coverageInfoURL = (
        baseURL
        + "/wcs?SERVICE=WCS&VERSION=1.0.0&REQUEST=DescribeCoverage&coverage=%s"
        % (layer_name,)
    )
    ...
    originField = gridField.find(
        "{http://www.opengis.net/gml}origin/{http://www.opengis.net/gml}pos"
    )
    # assume x and y respectively.
    origin = list(map(float, originField.text.split(" ")))
...
class UMEP_Data_Download(object):
    ...
    def __init__(self, iface):
        ...
        locale_path = os.path.join(
            self.plugin_dir, "i18n", "UMEP_Data_Download_{}.qm".format(locale)
        )
        ...
        self.actions = []
        self.menu = self.tr("&UMEP data downloader")
    ...
    def updateAbstract(self):
        ...
        if (
            self.dlg.tblDatasets.currentRow()
            > len(list(self.catalogue[collection].keys())) - 1
        ):
            # If the selected row is beyond the end of the requested data, it's probably a bug
            return
        try:
            subEntry = list(self.catalogue[collection].keys())[1:][
                self.dlg.tblDatasets.currentRow()
            ]
        except:
            return
        ...
        self.extraAbstractText = (
            ...
            + self.catalogue[collection][subEntry][8]
        )  # Extra text for abstract box from namelist
    ...
    def refreshList(self):
        ...
        self.dlg.tblDatasets.setHorizontalHeaderLabels(
            "Source;Description;Date;Resolution;Extent".split(";")
        )
        self.readacross = {}
        ...
        for i, key in enumerate(self.catalogue.keys()):  # Each group
            try:
                ...
                if i == 0:
                    self.dlg.lstCategory.itemSelectionChanged.connect(
                        self.updateList
                    )
    ...
    def updateList(self):
        ...
        self.dlg.tblDatasets.setRowCount(
            len(list(self.catalogue[txt].keys())) - 1
        )
        self.dlg.tblDatasets.setHorizontalHeaderLabels(
            "Source;Description;Date;Resolution;Extent".split(";")
        )
        self.dlg.tblDatasets.setSelectionMode(
        ...
        # Set column widths
        # Use these entries from each list in the catalogue file
        indicesToUse = [0, 4, 5, 6, 7]
        idx = 0
        # Each resource within each group
        for i, dataSource in enumerate(self.catalogue[txt]):
    ...
    def runDownload(self):
        ...
        if self.bbox["xmin"] >= self.bbox["xmax"]:
            self.bbox = {}
            raise ValueError(
                "Lower left longitude must be less than upper right longitude"
            )

        if self.bbox["ymin"] >= self.bbox["ymax"]:
            self.bbox = {}
            raise ValueError(
                "Lower left latitude must be less than upper right longitude"
            )

        collection = self.readacross[collection[0].text()]
        # Omit first key from list as it should just be collection label

        if self.dlg.tblDatasets.currentRow() > len(
            list(self.catalogue[collection].keys())[1:]
        ):
            # If the selected row is beyond the end of the requested data, it's probably a bug
            return
        ...
        if meta["SRS"] != "EPSG:4326":
            ...
            requested_rect = QgsRectangle(
                QgsPointXY(self.bbox["xmin"], self.bbox["ymin"]),
                QgsPointXY(self.bbox["xmax"], self.bbox["ymax"]),
            )
            transformed_rect = (
                QgsGeometry()
                .fromRect(crs_transform.transform(requested_rect))
                .boundingBox()
            )
        else:
            transformed_rect = QgsRectangle(
                QgsPointXY(self.bbox["xmin"], self.bbox["ymin"]),
                QgsPointXY(self.bbox["xmax"], self.bbox["ymax"]),
            )
        ...
        if (meta["extent"]["xMax"] < request_bbox["xmin"]) | (
            meta["extent"]["xMin"] > request_bbox["xmax"]
        ):
            raise ValueError(
                "There is no data in the requested bounding box (invalid longitude range)"
            )

        if (meta["extent"]["yMax"] < request_bbox["ymin"]) | (
            meta["extent"]["yMin"] > request_bbox["ymax"]
        ):
            raise ValueError(
                "There is no data in the requested bounding box (invalid latitude range)"
            )
        ...
        if (req_resY > meta["resolution"]["y"]) or (
            req_resX > meta["resolution"]["x"]
        ):
            res_message = (
                "The largest raster returned by this program is 500x500. The dataset has higher native "
        ...
        if native:
            xLocs = np.arange(
                meta["extent"]["xMin"],
                meta["extent"]["xMin"]
                + meta["resolution"]["x"] * (meta["gridPoints"]["x"] + 1),
                meta["resolution"]["x"],
            )
            # Index of nearest x grid line to minimum
            xMinLoc = np.argmin(abs(request_bbox["xmin"] - xLocs))
            # Index of nearest x grid line to maximum
            xMaxLoc = np.argmin(abs(request_bbox["xmax"] - xLocs))

            yLocs = np.arange(
                meta["extent"]["yMin"],
                meta["extent"]["yMin"]
                + meta["resolution"]["y"] * (meta["gridPoints"]["y"] + 1),
                meta["resolution"]["y"],
            )
            # Index of nearest y grid line to minimum
            yMinLoc = np.argmin(abs(request_bbox["ymin"] - yLocs))
            # Index of nearest u grid line to maximum
            yMaxLoc = np.argmin(abs(request_bbox["ymax"] - yLocs))
        ...
        if (
            abs(request_bbox["xmin"] - request_bbox["xmax"])
            < 3 * meta["resolution"]["x"]
        ) | (
            abs(request_bbox["ymin"] - request_bbox["ymax"])
            < 3 * meta["resolution"]["y"]
        ):
            raise ValueError(
                "The requested area is too small. It must be at least 3x3 pixels. Widen the area and try again"
            )

        # Everything checks out, so try downloading the layer. Present the user with a Save As...
        self.filename = QFileDialog.getSaveFileName(
            caption="Save GeoTIFF file as...", filter="*.tif"
        )
        ...
        if self.dlg.checkBoxReproject.isChecked():
            if self.dlg.spinBoxResolution.value() < 1:
                QMessageBox.critical(
                    None,
                    "Error in pixel resolution raster data",
                    "Pixel resolution must be greater than 1 map unit",
                )
                return
    ...
    def error(self, errorContent):
        msgBox = QMessageBox.critical(
            None, "Error downloading raster data", str(errorContent)
        )
    ...
    def unload(self):
        ...
        for action in self.actions:
            self.iface.removePluginMenu(
                self.tr("&UMEP data data downloader"), action
            )
