...
class DownloadDataWorker(QObject):
    ...
    def run(self):
        try:
            ...
            self.finished.emit(
                {"filename": output, "srs": self.srs, "progress": 100}
            )
...
def webToRaster(
    baseURL, layer_name, output_file, bbox, resolution, srs, update
):
    """
    ...
    bigURL = (
        baseURL
        + "/wcs?SERVICE=WCS&VERSION=1.0.0&REQUEST=GetCoverage&coverage=%s&identifier=%s&bbox=%s&FORMAT=image/geotiff&CRS=%s&RESX=%f&RESY=%f"
        % (
            layer_name,
            layer_name,
            bboxString,
