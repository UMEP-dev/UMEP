...
def LCZ_fractions(lc_grid, dlg):
        ...
        try:
            wai[l] = (
                (
                    ...
                    * buildingsx
                    * (1 - buildingsx)
                    * float(dlg.tableWidget.item(15, l).text())
                    # wai according to Lindberg et al. 2015 eq. 7
                )
                / buildingsx
            ) * lczfrac[l]
        except:
    ...
    veg_aero_all[0, 0] = round(
        grassf.sum(axis=0) + dtreesf.sum(axis=0) + etreesf.sum(axis=0), 3
    )
    ...
    lczfractions = {
        ...
        "bui_aero": bui_aero_all,
        "veg_aero": veg_aero_all,
    }
