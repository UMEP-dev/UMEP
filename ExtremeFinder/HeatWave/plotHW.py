...
def level(minvalue, maxvalue):
    ...
    levels = [
        int(
            (i - lev[0]) / (lev[-1] - lev[0]) * (maxvalue - minvalue)
            + minvalue
        )
        for i in lev
    ]
...
def strlocation(lat, lon):
    ...
    if lon >= 0:
        if lat >= 0:
            strloc = str(abs(lon)) + "°E, " + str(abs(lat)) + "°N"
        else:
            strloc = str(abs(lon)) + "°E, " + str(abs(lat)) + "°S"
    else:
        if lat >= 0:
            strloc = str(abs(lon)) + "°W, " + str(abs(lat)) + "°N"
        else:
            strloc = str(abs(lon)) + "°W, " + str(abs(lat)) + "°S"
    return strloc
...
def plotHW(lat, lon, Tmax, xHW, hw_year_start, hw_year_end, labelsForPlot):
    ...
    ydate_first = pd.date_range(
        str(hw_year_start) + "-01-01", str(hw_year_start) + "-12-31"
    )
    ydate_final = pd.date_range(
        str(hw_year_start + yearnumber - 1) + "-01-01",
        str(hw_year_start + yearnumber - 1) + "-12-31",
    )
    ...
    dy = 1
    x2, y2 = np.meshgrid(
        np.arange(xmin, xmax + dx, dx) - dx / 2.0,
        np.arange(ymin, ymax + dy, dy) - dy / 2.0,
    )
    z = DataForPlot
    minvalue = z.min()
    maxvalue = z.max()
    ...
    fig.colorbar(
        im, ax=ax0, label="%s %s" % (labelsForPlot[0], labelsForPlot[3])
    )
    ...
    plt.xticks(
        xticks.rolling(window=2).mean()[1:],
        [
            "Jan",
            "Feb",
            "Mar",
    ...
    for iHW in xHW:
        ...
        day_of_year_f = ts_datetime[0].to_pydatetime() - datetime.datetime(
            ts_datetime[0].year, 1, 1
        )
