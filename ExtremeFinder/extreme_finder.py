...
class ExtremeFinder(object):
    ...
    def __init__(self, iface):
        ...
        locale_path = os.path.join(
            self.plugin_dir, "i18n", "ExtremeFinder_{}.qm".format(locale)
        )
        ...
        self.actions = []
        self.menu = self.tr("&Extreme Finder")
    ...
    def unload(self):
        ...
        for action in self.actions:
            self.iface.removePluginMenu(self.tr("&Extreme Finder"), action)
    ...
    def run(self):
        ...
        try:
            ...
            QMessageBox.critical(
                None,
                "Error",
                "This plugin requires the pandas package "
        ...
        try:
            ...
            QMessageBox.critical(
                None,
                "Error",
                "This plugin requires the netCDF4 package "
                "to be installed. Please consult the manual for further information",
            )
            return
    ...
    def error(self, exception, text):
        ...
        QMessageBox.critical(
            None,
            "Error",
            "Data download not completed: %s" % (str(exception),),
        )
    ...
    def start_progress(self):
        ...
        if self.dlg.toolBox.currentIndex() == 0:
            ...
            self.dlg.runButton.setEnabled(True)
            raise ValueError(
                "Please choose whether to investigate extreme high or low values"
            )
        ...
        if filein.split(".")[-1] == "nc":
            try:
                Tdata, unit, self.lat, self.lon, self.hw_start, self.hw_end = (
                    get_ncdata(
            ...
            unit = "(" + str(unit) + ")"
            sd = QDate.fromString(
                self.hw_start.strftime("%Y-%m-%d"), "yyyy-MM-dd"
            )
            ed = QDate.fromString(
                self.hw_end.strftime("%Y-%m-%d"), "yyyy-MM-dd"
            )
            ...
            if var != "Tair":
                raise ValueError(
                    "Only Tair may be analysed using a text file for data. "
            ...
            # we don't know what we've been given, so assume the user knows which method(s) to use given the input data
            daily_max = Tdata
            daily_avg = Tdata
            daily_min = Tdata
        ...
        if mode == "HW":
            if self.dlg.comboBox_HW.currentIndex() == 1:
                ...
                labelsForPlot = [
                    var + "_max",
                    "Extreme high values (Meehl and Tebaldi)",
                    "HVs",
                    unit,
                ]
                ...
                labelsForPlot = [
                    var + "_max",
                    "Extreme high values (Fischer and Schar)",
                    "HVs",
                    unit,
                ]

            elif self.dlg.comboBox_HW.currentIndex() == 3:
                xHW = findHW_Schoetter(
                    daily_max,
                    self.hw_start,
                    self.hw_end,
                    self.hw_start.year,
                    self.hw_end.year,
                    t4_quantile_Schoetter,
                    self.lat,
                )
                Tplot = daily_max
                labelsForPlot = [
                    var + "_max",
                    "Extreme high values (Shoetter)",
                    "HVs",
                    unit,
                ]
        ...
        if mode == "CW":
            if self.dlg.comboBox_CW.currentIndex() == 1:
                ...
                labelsForPlot = [
                    var + "_min",
                    "Extreme low values (Keevallik)",
                    "LVs",
                    unit,
                ]

            elif self.dlg.comboBox_CW.currentIndex() == 2:
                xHW = findCW_Srivastava(
                    daily_min, self.hw_start, self.hw_end, TempDif_Srivastava
                )
                Tplot = daily_min
                labelsForPlot = [
                    var + "_min",
                    "Extreme low values (Srivastava)",
                    "LVs",
                    unit,
                ]
                ...
                xHW = findCW_Busuioc(
                    daily_min, self.hw_start, self.hw_end, TempDif_Busuioc
                )
                Tplot = daily_min
                labelsForPlot = [
                    var + "_min",
                    "Extreme low values (Busuioc)",
                    "LVs",
                    unit,
                ]
        ...
        try:
            ...
            if mode == "HW":
                QMessageBox.critical(
                    None, "Error", "No extreme high values found"
                )
            if mode == "CW":
                QMessageBox.critical(
                    None, "Error", "No extreme low values found"
                )
            self.dlg.runButton.setEnabled(True)
            return
        else:
        ...
        plotHW(
            ...
            Tplot,
            xHW,
            int(self.hw_start.strftime("%Y")),
            int(self.hw_end.strftime("%Y")),
            labelsForPlot,
        )
        self.dlg.runButton.setEnabled(True)
