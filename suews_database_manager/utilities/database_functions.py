...
def set_nameOrigin(db_dict):
    ...
    for col in list(db_dict.keys()):
        ...
        if col == "Name":
            db_dict[col]["nameOrigin"] = (
                db_dict[col]["Name"].astype(str)
                + ", "
                + db_dict[col]["Origin"].astype(str)
            )
        elif col == "References":
            db_dict[col]["authorYear"] = (
                db_dict[col]["Author"].astype(str)
                + ", "
                + db_dict[col]["Year"].astype(str)
            )
        elif col == "Country":
            db_dict[col]["nameOrigin"] = (
                db_dict[col]["Country"].astype(str)
                + ", "
                + db_dict[col]["City"].astype(str)
            )
        elif col == "Region":
            pass
        elif col == "Spartacus Material":
            db_dict[col]["nameOrigin"] = (
                db_dict[col]["Name"].astype(
                    # + '; ' + db_dict[col]['Material Type'].astype(str)
                    str
                )
                + "; "
                + db_dict[col]["Color"].astype(str)
            )
        # Calculate U-values for roof and wall new columns u_value_wall and u_value_roof
            ...
            buildings = db_dict["Spartacus Surface"][
                db_dict["Spartacus Surface"]["Surface"] == "Buildings"
            ]
            ...
            for prefix in ["w", "r"]:
                ...
                thermal_conductivities = vectorize(
                    lambda x: db_dict["Spartacus Material"].loc[
            ...
            normalisation_rows = db_dict[col][
                (db_dict[col]["Profile Type"] == "Traffic")
                | (db_dict[col]["Profile Type"] == "Energy use")
            ]
            cols = list(range(24))
            ...
            # Scale the values
            db_dict[col].loc[normalisation_rows_index, cols] = (
                db_dict[col]
...
def save_to_db(db_path, db_dict):
    ...
    for col in db_dict.keys():
        if col == "References":
            db_dict[col] = db_dict[col].drop(
                columns="authorYear", errors="ignore"
            )
        elif col not in ["Country", "Region"]:
            db_dict[col] = db_dict[col].drop(
                columns="nameOrigin", errors="ignore"
            )
...
def update_db(db_dict, db_path, updated_db_path, backup_path):
    ...
    # This dict is the database that is to be updated from
    db_dict_update = read_DB(updated_db_path)
...
def get_limits_from_json_schema(
    variable_name: str, schema: dict = None
) -> dict:
    """
...
param_info_dict = {
    ...
    "Biogen CO2": {
        ...
        "param": {
            ...
            "beta_enh": {
                "min": 0,
                "max": 1,
                "tooltip": "Part of the beta coefficient related to the fraction of vegetation.",
            },
            "resp_a": {
                "min": 0,
                "max": 1,
                "tooltip": "Respiration coefficient a.",
            },
            "resp_b": {
                "min": 0,
                "max": 1,
                "tooltip": "Respiration coefficient b - related to air temperature dependency.",
            },
            "min_respi": {
                "min": 0,
                "max": 1,
                "tooltip": "Minimum soil respiration rate (for cold-temperature limit) [umol m-2 s-1].",
            },
        },
    },
    "Conductance": {
        ...
        "param": {
            ...
            "TH": {
                "min": 0,
                "max": 1,
                "tooltip": "Upper air temperature limit [°C]",
            },
            "TL": {
                "min": 0,
                "max": 1,
                "tooltip": "Lower air temperature limit [°C]",
            },
            "S1": {
                "min": 0,
                "max": 1,
    ...
    "Leaf Area Index": {
        ...
        "param": {
            ...
            "LAIMin": {
                "min": 0,
                "max": 1,
                "tooltip": "leaf-off wintertime value",
            },
            "LAIMax": {
                "min": 0,
                "max": 1,
    ...
    "Max Vegetation Conductance": {
        ...
        "param": {
            "MaxConductance": {
                "min": 0,
                "max": 1,
                "tooltip": "The maximum conductance of each vegetation or surface type. [mm s-1]",
            }
        },
    },
    "Porosity": {
        ...
        "param": {
            "PorosityMin": {
                "min": 0,
                "max": 1,
                "tooltip": "leaf-off wintertime value Used only for DecTr (can affect roughness calculation)",
            },
            "PorosityMax": {
                "min": 0,
                "max": 1,
                "tooltip": "full leaf-on summertime value Used only for DecTr (can affect roughness calculation)",
            },
        },
    },
    "Vegetation Growth": {
        ...
        "param": {
            "BaseT": {
                "min": 0,
                "max": 1,
                "tooltip": "Base Temperature for initiating growing degree days (GDD) for leaf growth. [°C]",
            },
            "BaseTe": {
                "min": 0,
                "max": 1,
                "tooltip": "Base temperature for initiating sensesance degree days (SDD) for leaf off. [°C]",
            },
            "GDDFull": {
                "min": 0,
                "max": 1,
                "tooltip": "The growing degree days (GDD) needed for full capacity of the leaf area index (LAI) [°C].",
            },
            "SDDFull": {
                "min": 0,
                "max": 1,
                "tooltip": "The sensesence degree days (SDD) needed to initiate leaf off. [°C]",
            },
        },
    },
    "Water State": {
        ...
        "param": {
            "StateLimit": {
                "min": 0,
                "max": 1,
                "tooltip": "Upper limit to the surface state. [mm]. Currently only used for the water surface. Set to a large value (e.g. 20000 mm = 20 m) if the water body is substantial (lake, river, etc) or a small value (e.g. 10 mm) if water bodies are very shallow (e.g. fountains). WaterDepth (column 9) must not exceed this value.",
            },
    ...
    "Water Storage": {
        ...
        "param": {
            "StorageMin": {
                "min": 0,
                "max": 1,
                "tooltip": "Minimum water storage capacity for upper surfaces (i.e. canopy).",
            },
            "StorageMax": {
                "min": 0,
                "max": 1,
    ...
    "Soil": {
        ...
        "param": {
            "SoilDepth": {
                "min": 0,
                "max": 1,
                "tooltip": "Soil density [kg m-3]",
            },
            "SoilStoreCap": {
                "min": 0,
                "max": 1,
                "tooltip": "Limit value for SoilDepth [mm]",
            },
            "SatHydraulicCond": {
                "min": 0,
                "max": 1,
                "tooltip": "Hydraulic conductivity for saturated soil [mm s-1]",
            },
            "SoilDensity": {
                "min": 0,
                "max": 1,
            ...
            "InfiltrationRate": {
                "min": 0,
                "max": 1,
                "tooltip": "Infiltration rate (Not currently used)",
            },
            "OBS_SMCap": {
                "min": 0,
                "max": 1,
                "tooltip": "The maximum observed soil moisture. [m3 m-3 or kg kg-1]",
            },
            "OBS_SMDepth": {
                "min": 0,
                "max": 1,
                "tooltip": "The depth of soil moisture measurements. [mm]",
            },
            "OBS_SoilNotRocks": {
                "min": 0,
                "max": 1,
                "tooltip": "Fraction of soil without rocks. [-]",
            },
        },
    },
    "OHM": {
    ...
    "SnowLimPatch": {
        ...
        "param": {
            "SnowLimPatch": {
                "tooltip": "Limit for the snow water equivalent when snow cover starts to be patchy [mm]. Not needed if SnowUse = 0 in RunControl.nml"
            }
        },
    },
}
