...
def setup_typology_creator(self, dlg, db_dict, db_path):
    ...
    def fill_cbox():
        ...
        dlg.comboBoxPavedType.addItems(
            (
                db_dict["NonVeg"]["nameOrigin"][
                    db_dict["NonVeg"]["Surface"] == "Paved"
                ]
            )
        )
        dlg.comboBoxBuildingType.clear()
        dlg.comboBoxBuildingType.addItems(
            (
                db_dict["NonVeg"]["nameOrigin"][
                    db_dict["NonVeg"]["Surface"] == "Buildings"
                ]
            )
        )
    ...
    def check_type():
        if dlg.textEditName.isNull():
            QMessageBox.warning(
                None, "Error in Name", "Enter a name for new type"
            )
        ...
        # Origin
            ...
            QMessageBox.warning(
                None, "Error in Origin", "Enter a Origin for new type"
            )
    ...
    def changed():
        ...
        def type_changed(cbox, table, surface):
            ...
            if urb_type != "":
                indexer = db_dict["Types"][
                    db_dict["Types"]["nameOrigin"] == urb_type
                ][surface].item()
        ...
        if urb_type != "":
            indexer = (
                db_dict["Types"]["ProfileType"]
                .loc[db_dict["Types"]["nameOrigin"] == urb_type]
                .item()
            )
    ...
    def typology_info():
        ...
        if dlg.comboBoxTableSelect.currentIndex() != -1:
            ...
            typology_sel = db_dict["Types"].loc[
                db_dict["Types"]["nameOrigin"] == typology_str
            ]
            buildID = typology_sel["Buildings"].item()
            PavedID = typology_sel["Paved"].item()
            wall1ID = (
                db_dict["Spartacus Surface"]
                .loc[db_dict["NonVeg"].loc[buildID].loc["Spartacus Surface"]][
                    "w1Material"
                ]
                .item()
            )
            ...
            wall1Th = str(
                round(
                    db_dict["Spartacus Surface"].loc[
                        db_dict["NonVeg"].loc[buildID].loc["Spartacus Surface"]
                    ]["w1Thickness"],
                    2,
                )
                * 100
            ...
            dlg.textBrowserTypo.setText(
                ...
                + "\n"
                + "Name: "
                + typology_sel["Name"].item()
                + "\n"
                + "Origin: "
                + typology_sel["Origin"].item()
                + "\n"
                + "Description: "
                + typology_sel["Description"].item()
                + "\n"
                + "Construction perion: "
                + typology_sel["Period"].item()
                + "\n"
                + "Type of land use: "
                + typology_sel["ProfileType"].item()
                + "\n"
                + " "
                + "\n"
                ...
                + "\n"
                + "Name: "
                + db_dict["NonVeg"].loc[buildID]["Name"]
                + "\n"
                + "Origin: "
                + db_dict["NonVeg"].loc[buildID]["Origin"]
                + "\n"
                + "Mean bulk albedo (min): "
                + str(
                    db_dict["Albedo"]
                    .loc[db_dict["NonVeg"].loc[buildID]["Albedo"]]["Alb_min"]
                    .item()
                )
                + "\n"
                + "Mean bulk albedo (max): "
                + str(
                    db_dict["Albedo"]
                    .loc[db_dict["NonVeg"].loc[buildID]["Albedo"]]["Alb_min"]
                    .item()
                ...
                + str(
                    db_dict["Emissivity"]
                    .loc[db_dict["NonVeg"].loc[buildID]["Emissivity"]][
                        "Emissivity"
                    ]
                    .item()
                )
                ...
                + str(
                    round(
                        db_dict["Spartacus Surface"]
                        .loc[
                            db_dict["NonVeg"].loc[buildID]["Spartacus Surface"]
                        ]["u_value_roof"]
                        .item(),
                        2,
                    )
                ...
                + "\n"
                + "Name: "
                + db_dict["NonVeg"].loc[PavedID]["Name"]
                + "\n"
                + "Origin: "
                + db_dict["NonVeg"].loc[PavedID]["Origin"]
                + "\n"
                + "Mean bulk albedo (min): "
                + str(
                    round(
                        db_dict["Albedo"]
                        .loc[db_dict["NonVeg"].loc[PavedID]["Albedo"]][
                            "Alb_min"
                        ]
                        .item(),
                        2,
                ...
                + str(
                    round(
                        db_dict["Emissivity"]
                        .loc[db_dict["NonVeg"].loc[PavedID]["Emissivity"]][
                            "Emissivity"
                        ]
                        .item(),
                        2,
            ...
            if typology_sel["Url"].item() == None:
                ...
                setup_image(
                    dlg.label_2,
                    typology_sel["Url"].item(),
                    typology_sel["imageSource"].item(),
                )
        ...
        if type(url) == str:
            ...
            if parsed_url.scheme not in ("http", "https"):
                QgsMessageLog.logMessage(
                    "The URL provided is not valid. It should start with http:// or https://",
            ...
            if imageSource != None:
                dlg.textBrowser_source.setText(
                    "Picture Author:, " + str(imageSource)
                )

            try:
                resp = requests.get(
                    safe_url, headers={"User-Agent": "Mozilla/5.0"}, timeout=5
                )
    ...
    def generate_type():
        ...
        dict_reclass = {
            # str('Type' + str(int(round(time.time())))),
            "ID": create_code("Name"),
        ...
        # , dlg.comboBoxGrassType, dlg.comboBoxDecType, dlg.comboBoxEvrType,dlg.comboBoxBsoilType, dlg.comboBoxWaterType]
        cbox_list = [dlg.comboBoxPavedType, dlg.comboBoxBuildingType]
        # , dlg.textBrowser_2, dlg.textBrowser_3, dlg.textBrowser_4, dlg.textBrowser_5, dlg.textBrowser_6]
        textbrowser_list = [dlg.textBrowser_0, dlg.textBrowser_1]
        ...
        for cbox, textbrowser in zip(cbox_list, textbrowser_list):
            ...
            dict_reclass[var] = surf_table[
                surf_table["nameOrigin"] == cbox.currentText()
            ].index.item()
