...
def setup_landcover_creator(self, dlg, db_dict, db_path):
    ...
    def handle_surface_selection(surface):
        ...
        dlg.comboBoxBase.addItems(
            sorted(table["nameOrigin"][table["Surface"] == surface])
        )
        dlg.comboBoxBase.setCurrentIndex(-1)
        ...
        if surface != "Deciduous Tree":
            # Exception for just Deciduous tree in Veg
        ...
        for i, col_name in enumerate(col_list):
            ...
            if col_name == "Spartacus Surface":
                ...
                try:
                    table_surf = table_sel[
                        (table_sel["Surface"] == surface)
                        & (table_sel["LAIEq"] == int(LAI_sel))
                    ]
                except:
                ...
                table_surf = table_sel[
                    (table_sel["Surface"] == surface)
                    | (table_sel["Surface"] == "All vegetation")
                ]
                ...
                if surface in ["Grass", "Evergreen Tree", "Deciduous Tree"]:
                    table_surf = table[
                        ...
                        | (table["Surface"] == "All vegetation")
                        | (table["Surface"] == "cropland")
                    ]
                elif surface in ["Buildings", "Paved", "Bare Soil"]:
                    table_surf = table[
                        (table["Surface"] == surface)
                        | (table["Surface"] == "All nonveg")
                    ]
                elif surface == "Water":
            ...
            Nc_fill_list = [
                f"{idx}: {name}, {orig}"
                for idx, (name, orig) in enumerate(
                    zip(table_surf["Name"], table_surf["Origin"])
                )
            ]
    ...
    def base_typology_changed():
        ...
        if dlg.comboBoxBase.currentIndex() != -1:
            ...
            surf_row = (
                surface_table[
                    (surface_table["nameOrigin"] == base_typology)
                    & (surface_table["Surface"] == surface)
                ]
                .squeeze()
                .to_dict()
            )
            ...
            for i in range(21):
                ...
                if Tb and Cb:
                    ...
                    cbox_table = (
                        ...
                        else db_dict.get(cbox_table_indexer)
                    )

                    # Find the correct index for the combo box

                    if cbox_table_indexer in [
                        "Leaf Area Index",
                        "Leaf Growth Power",
                    ]:

                        LAI_index = cbox_table.loc[surf_row_id, "LAIEq"].item()
                        dlg.comboBoxProfileType.setCurrentIndex(
                            dlg.comboBoxProfileType.findText(
                                str(int(LAI_index))
                            )
                        )
                    ...
                    for j in range(21):
                        ...
                        if Tb and Tb.toPlainText() == type_to_check:
                            ...
                            filtered_LAI = db_dict[type_to_check][
                                (
                                    db_dict[type_to_check]["LAIEq"]
                                    == current_LAIEq
                                )
                                & (
                                    db_dict[type_to_check]["Surface"]
                                    == surface
                                )
                            ]
                            ...
                            formatted_list = [
                                f"{k + 1}: {value}"
                                for k, value in enumerate(LAIList)
                            ]
                            ...
                            getattr(
                                ...
                                lambda _, idx=j: print_table(idx)
                            )
    ...
    def print_table(idx):
        ...
        if dlg.comboBoxSurface.currentIndex() != -1:
            ...
            if table_var == "Spartacus Surface":
                ...
                table_spar["Reference"] = (
                    table_spar["Ref"].map(ref_show).fillna("")
                )  # Map references
                ...
                for i in range(1, 6):
                    ...
                    table_spar[f"r{i}Material2"] = table_spar[
                        f"r{i}Material"
                    ].map(
                        mat_show
                    )  # Map materials
                    table_spar[f"w{i}Material2"] = table_spar[
                        f"w{i}Material"
                    ].map(
                        mat_show
                    )  # Map materials
                ...
                Tb.setText(
                    str(
                        table_spar.reset_index()
                        .drop(columns=["ID", "Ref", "nameOrigin", "Surface"])
                        .to_html(index=True)
                    )
                )
                ...
                table_surf["Reference"] = (
                    table_surf["Ref"].map(ref_show).fillna("")
                )  # Map references
                Tb.setText(
                    str(
                        table_surf.reset_index()
                        .drop(columns=["Ref", "ID", "Surface"])
                        .to_html(index=True)
                    )
                )
                Tb.setLineWrapMode(0)
    ...
    def check_typology():
        ...
        otf_nameOrigin = (
            str(dlg.textEditName.value())
            + ", "
            + str(dlg.textEditOrig.value())
        )

        # Ensure that user does not create same name as already exist
        if len(dlg.comboBoxSurface.currentText()) < 1:
            QMessageBox.warning(
                None, "Surface Missing", "Please select a surface"
            )
        elif dlg.textEditName.value().startswith("test"):
            QMessageBox.warning(
                None, "Error in Name", "Please, don´t use test as type name.."
            )
        elif dlg.textEditName.value().startswith("Test"):
            QMessageBox.warning(
                None, "Error in Name", "Please, don´t use test as type name.."
            ...
            QMessageBox.warning(
                None, "Name Missing", "Please fill in the Name Box"
            )
        elif len(dlg.textEditOrig.text()) < 1:
            QMessageBox.warning(
                None, "Origin Missing", "Please fill in the Origin Box"
            )
        elif otf_nameOrigin == dlg.comboBoxBase.currentText():
            QMessageBox.warning(
                None,
                "Nameing Error",
                "Name and origin Matches what already is in DB",
            )
        else:
            generate_typology()
    ...
    def generate_typology():
        ...
        for i in range(21):
            ...
            if len(Oc.toPlainText()) < 1:
                ...
                # Remove number added for interpretation in GUI
                sel_att = sel_att.split(": ")[1]
        ...
        db_dict[surf_df_dict[surface]] = concat(
            [db_dict[surf_df_dict[surface]], new_edit]
        )

        save_to_db(db_path, db_dict)

        QMessageBox.information(
            None, "Sucessful", "Typology entry added to local database"
        )
    ...
    dlg.comboBox_4.activated.connect(lambda: print_table(4))
    dlg.comboBox_5.activated.connect(lambda: print_table(5))
    dlg.comboBox_6.activated.connect(lambda: print_table(6))
    dlg.comboBox_7.activated.connect(lambda: print_table(7))
    dlg.comboBox_8.activated.connect(lambda: print_table(8))
    dlg.comboBox_9.activated.connect(lambda: print_table(9))
    dlg.comboBox_10.activated.connect(lambda: print_table(10))
    dlg.comboBox_11.activated.connect(lambda: print_table(11))
    dlg.comboBox_12.activated.connect(lambda: print_table(12))
    dlg.comboBox_13.activated.connect(lambda: print_table(13))
    dlg.comboBox_14.activated.connect(lambda: print_table(14))
    dlg.comboBox_15.activated.connect(lambda: print_table(15))
    dlg.comboBox_16.activated.connect(lambda: print_table(16))
    dlg.comboBox_17.activated.connect(lambda: print_table(17))
    dlg.comboBox_18.activated.connect(lambda: print_table(18))
    dlg.comboBox_19.activated.connect(lambda: print_table(19))
    dlg.comboBox_20.activated.connect(lambda: print_table(20))
