...
def setup_SS_material_creator(self, dlg, db_dict, db_path):
    ...
    def base_changed():
        ...
        if base_str != "":
            ...
            base = db_dict["Spartacus Material"].loc[
                db_dict["Spartacus Material"]["nameOrigin"] == base_str
            ]
            dlg.comboBoxMatType.setCurrentIndex(
                dlg.comboBoxMatType.findText(base["Material Type"].item())
            )

            dlg.textEditAlbedo.setValue(str(base["Albedo"].item()))
            dlg.textEditEmissivity.setValue(str(base["Emissivity"].item()))
            dlg.textEditThermalC.setValue(
                str(base["Thermal Conductivity"].item())
            )
    ...
    def generate_material():
        ...
        db_dict["Spartacus Material"] = concat(
            [db_dict["Spartacus Material"], new_edit]
        )
        print(new_edit)
