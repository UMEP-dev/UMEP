...
def setup_snow_creator(self, dlg, db_dict, db_path):
    ...
    def fill_cbox():
        ...
        for i in range(0, 6):
            ...
            if tablename.startswith("OHM"):
                Cb.addItems(
                    sorted(
                        db_dict["OHM"]["nameOrigin"][
                            db_dict["OHM"]["Surface"] == "Snow"
                        ].tolist()
                    )
                )
            else:
                Cb.addItems(
                    sorted(
                        db_dict[tablename]["nameOrigin"][
                            db_dict[tablename]["Surface"] == "Snow"
                        ].tolist()
                    )
                )
    ...
    def base_snow_changed():
        ...
        if dlg.comboBoxBase.currentIndex() != -1:
            ...
            snow_sel = db_dict["Snow"].loc[
                db_dict["Snow"]["nameOrigin"] == base_snow
            ]
            ...
            for i in range(0, 6):
                ...
                tablename = Tb.toPlainText()
                if tablename.startswith("OHM"):
                    indexer = db_dict["OHM"].loc[
                        snow_sel[tablename].item(), "nameOrigin"
                    ]
                else:
                    indexer = db_dict[tablename].loc[
                        snow_sel[tablename].item(), "nameOrigin"
                    ]
    ...
    def add_snow():
        ...
        for i in range(0, 6):
            ...
            if tablename.startswith("OHM"):
                dict_reclass[Tb.toPlainText()] = (
                    db_dict["OHM"]
                    .loc[db_dict["OHM"]["nameOrigin"] == current_text]
                    .index.item()
                )
            else:
                dict_reclass[Tb.toPlainText()] = (
                    db_dict[tablename]
                    .loc[db_dict[tablename]["nameOrigin"] == current_text]
                    .index.item()
                )
        ...
        QMessageBox.information(
            None, "Succesful", "Snow Entry added to your local database"
        )
