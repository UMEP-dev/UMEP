...
def setup_SUEWS_SS_creator(self, dlg, db_dict, db_path):
    ...
    def fill_cboxes():
        ...
        mat_list = list(
            sorted(
                db_dict["Spartacus Material"]["nameOrigin"], key=str.casefold
            )
        )
        ...
        for roofwall in ["r", "w"]:
            for layer in range(
                1, 6
            ):  # 6 #TODO Change 4->6 if we want all 5 layers
                ...
                lineEdit = getattr(
                    dlg, f"lineEdit_{roofwall}{str(layer)}", None
                )
                lineEdit.setText("")
    ...
    def print_table(dlg, idx, rw):
        try:
            ...
            if material != "None":
                ...
                # Set correct table from db
                mat_table = db_dict["Spartacus Material"]
                # Slice correct material from table
                material_sel = mat_table[mat_table["nameOrigin"] == material]

                if idx == 1:
                    Tb.setText(  # Write texts of the selected material in the textbrowser
                        "Albedo: "
                        + str(material_sel["Albedo"].item())
                        + "\n"
                        + "Emissivity: "
                        + str(material_sel["Emissivity"].item())
                        + "\n"
                        + "Thermal Conductivity: "
                        + str(material_sel["Thermal Conductivity"].item())
                        + "\n"
                        + "Specific Heat Capacity: "
                        + str(material_sel["Specific Heat"].item())
                        + "\n"
                        + "Density:  "
                        + str(material_sel["Density"].item())
                        + "\n"
                    )
                else:
                    Tb.setText(  # Write texts of the selected material in the textbrowser
    ...
    def new_edit():
        ...
        wall_insulation = int(
            dlg.buttonGroupWall.checkedButton().objectName()[-1]
        )
        roof_insulation = int(
            dlg.buttonGroupRoof.checkedButton().objectName()[-1]
        )
        ...
        spartacus_dict["Ref"] = db_dict["References"][
        ...
        # Roof
        if r1_mat != "None":
            spartacus_dict["r1Material"] = mat_table[
                mat_table["nameOrigin"] == r1_mat
            ].index.item()
            spartacus_dict["r1Thickness"] = float(dlg.lineEdit_r1.text())
            if r2_mat != "None":
                spartacus_dict["r2Material"] = mat_table[
                    mat_table["nameOrigin"] == r2_mat
                ].index.item()
                spartacus_dict["r2Thickness"] = float(dlg.lineEdit_r2.text())
                if r3_mat != "None":
                    spartacus_dict["r3Material"] = mat_table[
                        mat_table["nameOrigin"] == r3_mat
                    ].index.item()
                    spartacus_dict["r3Thickness"] = float(
                        dlg.lineEdit_r3.text()
                    )
                    if r4_mat != "None":
                        spartacus_dict["r4Material"] = mat_table[
                            mat_table["nameOrigin"] == r4_mat
                        ].index.item()
                        spartacus_dict["r4Thickness"] = float(
                            dlg.lineEdit_r4.text()
                        )
                        if r5_mat != "None":
                            spartacus_dict["r5Material"] = mat_table[
                                mat_table["nameOrigin"] == r5_mat
                            ].index.item()
                            spartacus_dict["r5Thickness"] = float(
                                dlg.lineEdit_r5.text()
                            )
                        else:
                            pass
                    else:
        ...
        # Wall
        if w1_mat != "None":
            spartacus_dict["w1Material"] = mat_table[
                mat_table["nameOrigin"] == w1_mat
            ].index.item()
            spartacus_dict["w1Thickness"] = float(dlg.lineEdit_w1.text())
            if w2_mat != "None":
                spartacus_dict["w2Material"] = mat_table[
                    mat_table["nameOrigin"] == w2_mat
                ].index.item()
                spartacus_dict["w2Thickness"] = float(dlg.lineEdit_w2.text())
                if w3_mat != "None":
                    spartacus_dict["w3Material"] = mat_table[
                        mat_table["nameOrigin"] == w3_mat
                    ].index.item()
                    spartacus_dict["w3Thickness"] = float(
                        dlg.lineEdit_w3.text()
                    )
                    if w4_mat != "None":
                        spartacus_dict["w4Material"] = mat_table[
                            mat_table["nameOrigin"] == w4_mat
                        ].index.item()
                        spartacus_dict["w4Thickness"] = float(
                            dlg.lineEdit_w4.text()
                        )
                        if w5_mat != "None":
                            spartacus_dict["w5Material"] = mat_table[
                                mat_table["nameOrigin"] == w5_mat
                            ].index.item()
                            spartacus_dict["w5Thickness"] = float(
                                dlg.lineEdit_w5.text()
                            )
                        else:
                            pass
                    else:
        ...
        db_dict["Spartacus Surface"] = concat(
            [db_dict["Spartacus Surface"], new_edit]
        )
        ...
        QMessageBox.information(
            None,
            "Succesful",
            f"New edit {spartacus_dict['Name']}, {spartacus_dict['Origin']} added to your local database",
        )
        fill_cboxes()
    ...
    def base_surface_changed():
        ...
        if dlg.comboBoxBase.currentIndex() != -1:
            ...
            spartacus_sel = surf_table[
                surf_table["nameOrigin"] == spartacus_str
            ]
            ...
            getattr(
                dlg, f"radioButtonR{str(int(insulation_roof.item()))}"
            ).setChecked(True)
            getattr(
                dlg, f"radioButtonW{str(int(insulation_wall.item()))}"
            ).setChecked(True)
            ...
            for roofwall in ["r", "w"]:
                for layer in range(
                    ...
                    cbox = getattr(
                        dlg, f"comboBox_{roofwall}{str(layer)}", None
                    )
                    lineEdit = getattr(
                        dlg, f"lineEdit_{roofwall}{str(layer)}", None
                    )

                    mat_idx = spartacus_sel.loc[
                        :, (roofwall + str(layer) + "Material")
                    ].item()
                    ...
                    if isnan(mat_idx) != True:
                        ...
                        thickness = spartacus_sel.loc[
                            :, (roofwall + str(layer) + "Thickness")
                        ].item()
            ...
            try:
                ...
                ref_index = (
                    db_dict["References"].loc[ref_id, "authorYear"].item()
                )
                dlg.comboBoxRef.setCurrentIndex(
                    dlg.comboBoxRef.findText(ref_index)
                )
            except:
                dlg.comboBoxRef.setCurrentIndex(-1)
    ...
    dlg.comboBoxRef.currentIndexChanged.connect(
        lambda: ref_changed(dlg, db_dict)
    )
    dlg.comboBoxBase.currentIndexChanged.connect(base_surface_changed)
    dlg.comboBox_r1.currentIndexChanged.connect(
        lambda: print_table(dlg, 1, "r")
    )
    dlg.comboBox_r2.currentIndexChanged.connect(
        lambda: print_table(dlg, 2, "r")
    )
    dlg.comboBox_r3.currentIndexChanged.connect(
        lambda: print_table(dlg, 3, "r")
    )
    dlg.comboBox_r4.currentIndexChanged.connect(
        lambda: print_table(dlg, 4, "r")
    )
    dlg.comboBox_r5.currentIndexChanged.connect(
        lambda: print_table(dlg, 5, "r")
    )
    dlg.comboBox_w1.currentIndexChanged.connect(
        lambda: print_table(dlg, 1, "w")
    )
    dlg.comboBox_w2.currentIndexChanged.connect(
        lambda: print_table(dlg, 2, "w")
    )
    dlg.comboBox_w3.currentIndexChanged.connect(
        lambda: print_table(dlg, 3, "w")
    )
    dlg.comboBox_w4.currentIndexChanged.connect(
        lambda: print_table(dlg, 4, "w")
    )
    dlg.comboBox_w5.currentIndexChanged.connect(
        lambda: print_table(dlg, 5, "w")
    )
