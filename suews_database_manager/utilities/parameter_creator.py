...
def setup_parameter_creator(self, dlg, db_dict, db_path):
    ...
    def table_changed():
        ...
        if dlg.comboBoxTableSelect.currentIndex() != -1:
            ...
            for i in range(0, 15):
                ...
                dlg.comboBoxSurface.addItems(
                    param_info_dict[table_name]["surface"]
                )
                ...
                for idx in range(len(params)):
                    ...
                    Oc.setToolTip(
                        param_info_dict[table_name]["param"][params[idx]][
                            "tooltip"
                        ]
                    )
    ...
    def surface_changed():
        ...
        if dlg.comboBoxTableSelect.currentIndex() != -1:
            ...
            if table_name in ["Soil", "Conductance"]:
                ...
                current_parameters = db_dict[table_name][
                    db_dict[table_name]["Surface"] == surface_sel
                ]

            dlg.comboBoxBase.clear()
            dlg.comboBoxBase.addItems(
                current_parameters["nameOrigin"].tolist()
            )
    ...
    def base_parameter_changed():
        ...
        if base_str != "":
            ...
            if table_name in ("Conductance", "Soil"):
                base_parameter = db_dict[table_name].loc[
                    db_dict[table_name]["nameOrigin"] == base_str
                ]
            else:
                base_parameter = db_dict[table_name][
                    db_dict[table_name]["Surface"] == surface_sel
                ].loc[db_dict[table_name]["nameOrigin"] == base_str]
    ...
    def print_table():
        ...
        if table_var == "Conductance":
            ...
            table_show["Reference"] = (
                table_show["Ref"].map(ref_show).fillna("")
            )  # Map references
            Tb.setText(
                str(
                    table_show.reset_index()
                    .drop(columns=["Ref", "ID"])
                    .to_html(index=True)
                )
            )
    ...
    def add_table():
        ...
        if not dlg.comboBoxRef.currentText():
            QMessageBox.warning(
                None, "Reference Info Missing", "Please add information"
            )
            return
        else:
            dict_reclass["Ref"] = db_dict["References"][
        ...
        if not checker == 1:
            db_dict[table_name] = db_dict[table_name].drop(
                new_edit.index, errors="ignore"
            )
        else:
            QMessageBox.information(
                None,
                "Succesful",
                table_name + " Entry added to your local database",
            )
    ...
    def checkerInput(table_list):
        ...
        if len(dlg.comboBoxSurface.currentText()) < 1:
            ...
            QMessageBox.warning(
                None, "References Missing", "Please select a references"
            )
            return
        ...
        for idx in range(len_list):
            ...
            if (len(Nc.value())) < 1:
                QMessageBox.warning(
                    None, oldField + " Missing", "Enter value for " + oldField
                )
                return
        ...
        if var == "Albedo":
            ...
            if (
                float(dlg.textEdit_Edit_0.value()) < limMin["min"]
                or float(dlg.textEdit_Edit_0.value()) > limMin["max"]
            ):
                QMessageBox.warning(
                    None, "Albedo Min error", "Alb_min must be between 0-1"
                )
                return
            elif (
                float(dlg.textEdit_Edit_1.value()) < limMax["min"]
                or float(dlg.textEdit_Edit_1.value()) > limMax["max"]
            ):
                QMessageBox.warning(
                    None, "Albedo Max error", "Alb_max must be between 0-1"
                )
                return
            elif float(dlg.textEdit_Edit_0.value()) > float(
                dlg.textEdit_Edit_1.value()
            ):
                QMessageBox.warning(
                    None,
                    "Value error",
                    dlg.textBrowser_0.toPlainText()
                    + " must be smaller or equal to "
                    + dlg.textBrowser_1.toPlainText(),
                )
                return

        elif var == "Leaf Area Index":
            if (
                float(dlg.textEdit_Edit_1.value()) < 0
                or float(dlg.textEdit_Edit_1.value()) > 1
            ):
                QMessageBox.warning(
                    None, "LAImin error", "LAImin must be between 0-1"
                )
                return
            elif (
                float(dlg.textEdit_Edit_2.value()) < 0
                or float(dlg.textEdit_Edit_2.value()) > 1
            ):
                QMessageBox.warning(
                    None, "LAImax error", "LAImax must be between 0-1"
                )
                return
            elif float(dlg.textEdit_Edit_1.value()) > float(
                dlg.textEdit_Edit_2.value()
            ):
                QMessageBox.warning(
                    None,
                    "Value error",
                    dlg.textBrowser_1.toPlainText()
                    + " must be smaller or equal to "
                    + dlg.textBrowser_2.toPlainText(),
                )
                return
            elif int(dlg.textEdit_Edit_0.value()) > 1:
                QMessageBox.warning(
                    None, "LAI Equation error", "LAIeq choices are 0 or 1"
                )
                return

        elif var == "Porosity":
            if float(dlg.textEdit_Edit_0.value()) > float(
                dlg.textEdit_Edit_1.value()
            ):
                QMessageBox.warning(
                    None,
            ...
            if (
                float(dlg.textEdit_Edit_0.value()) < 0
                or float(dlg.textEdit_Edit_0.value()) > 1
            ):
                QMessageBox.warning(
                    None, "Emissivity error", "Emissivity must be between 0-1"
                )
                return
        elif var == "Conductance":
            if (
                float(dlg.textEdit_Edit_11.value()) < 1
                or float(dlg.textEdit_Edit_11.value()) > 2
            ):
                QMessageBox.warning(
                    None, "gsModel error", "gsModel Choices are 1 & 2"
                )
                return
