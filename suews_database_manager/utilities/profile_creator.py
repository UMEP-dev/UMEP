...
def setup_profile_creator(self, dlg, db_dict, db_path):
    ...
    def fill_cbox():
        ...
        db_dict["Profiles"]["baseProfIndexer"] = [
            f"{row['Name']}, {row['City']}"
            for index, row in db_dict["Profiles"].iterrows()
        ]
    ...
    def profile_setting(setting):
        ...
        if setting == "Profile":
            ...
            dlg.comboBoxMain.addItems(
                sorted(list(set(db_dict["Profiles"]["Country"])))
            )
            dlg.comboBoxMain.setCurrentIndex(-1)
        ...
        if dlg.radioButtonProfile.isChecked() is True:
            ...
            if dlg.comboBoxMain.currentText() == "":
                ...
                country_list = sorted(
                    list(
                        set(
                            list(
                                db_dict["Profiles"]["Country"].loc[
                                    db_dict["Profiles"]["Profile Type"]
                                    == main_sel_sel
                                ]
                            )
                        )
            ...
            if dlg.comboBoxMain.currentText() == "":
                ...
                profile_list = list(
                    set(
                        list(
                            db_dict["Profiles"]["Profile Type"].loc[
                                db_dict["Profiles"]["Country"] == main_sel
                            ]
                        )
                    )
                )
    ...
    def sub_changed():
        ...
        if (
            dlg.comboBoxMain.currentText() == ""
            and dlg.comboBoxSub.currentText() == ""
        ):
            pass
            ...
            if dlg.radioButtonProfile.isChecked() is True:
                ...
                profiles = db_dict["Profiles"][
                    (db_dict["Profiles"]["Profile Type"] == main_sel)
                    & (db_dict["Profiles"]["Country"] == sub_sel)
                ]
                ...
                profiles = db_dict["Profiles"][
                    (db_dict["Profiles"]["Profile Type"] == sub_sel)
                    & (db_dict["Profiles"]["Country"] == main_sel)
                ]
                day_list = list(set(list(profiles["Day"])))
    ...
    def day_changed():
        ...
        if (
            dlg.comboBoxMain.currentText() == ""
            and dlg.comboBoxSub.currentText() == ""
            and dlg.comboBoxDay.currentText() == ""
        ):
            pass
        else:
            ...
            if dlg.radioButtonProfile.isChecked() is True:
                ...
                profiles = db_dict["Profiles"][
                    (db_dict["Profiles"]["Day"] == day_sel)
                    & (db_dict["Profiles"]["Profile Type"] == main_sel)
                    & (db_dict["Profiles"]["Country"] == sub_sel)
                ]

                dlg.comboBoxBaseProfile.clear()
                dlg.comboBoxBaseProfile.addItems(
                    sorted(profiles["baseProfIndexer"])
                )
        ...
        if (
            ...
            if dlg.radioButtonProfile.isChecked() is True:
                prof_sel = db_dict["Profiles"][
                    ...
                    & (db_dict["Profiles"]["Profile Type"] == main_sel)
                    & (db_dict["Profiles"]["Country"] == sub_sel)
                    & (db_dict["Profiles"]["baseProfIndexer"] == base_sel)
                ]
    ...
    def update_plot():
        # Not sure why this has to be here, but if i remove it, nothing works...
        a = dlg.comboBoxMain.currentText()
    ...
    def add_profile():
        ...
        QMessageBox.information(
            None, "Succesful", "Profile Entry added to your local database"
        )
        fill_cbox()
