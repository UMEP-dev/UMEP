...
def readSO(fn, plugin_dir):
    ...
    resDA = xr.DataArray(
        data=resValue, coords=[resDT, varname[5:]], dims=["time", "var"]
    )
    return resDA
...
def load_res_all(fn_nml, plugin_dir):
    ...
    rawdata_cfg = {
        cfg: readSO(fn, plugin_dir) for cfg, fn in list(zip(name_cfg, fn_cfg))
    }
...
def benchmark_SUEWS(fn_nml, plugin_dir):
    ...
    res_metric = {
        f: pd.DataFrame(
            [list_func[f](res_cfg[cfg], res_cfg["obs"]) for cfg in list_cfg],
            index=list_cfg,
        )
        .dropna(axis=1)
...
def plot_score_metric(df_metric):
    ...
    fig_metric = {
        name_func: plotMatMetric(df_metric[name_func], len_metric, name_func)
...
def background_gradient(res, cmap="PuBu"):
    ...
    c_rgba = [
        (
            plt.cm.get_cmap(cmap)(x)
            if x != 0.5
            else tuple(list(plt.cm.get_cmap(cmap)(x)[:3]) + [0])
        )
        for x in normed
    ]
    # print c_rgba
...
def bar_score_HTML(df_metric, styles):
    ...
    df_score = (
        benchmark_score(df_metric).to_frame().rename(columns={0: "score"})
    )
...
def report_benchmark_HTML(fn_nml, plugin_dir):
    ...
    styles = [
        ...
        dict(selector="thead", props=[("background-color", "#ABB2B9")]),
        # background shading
        dict(
