...
def readSO(fn, plugin_dir):
    ...
    dict_2016to2017 = dict(
        pd.read_excel(
            os.path.join(plugin_dir, "header-2016to2017.xlsx")
        ).values
    )
...
def load_res_all(fn_nml, plugin_dir):
    ...
    rawdata_cfg = {
        cfg: readSO(fn, plugin_dir) for cfg, fn in zip(listCfg, fn_cfg)
    }
    ...
    res = pd.Panel(
        {
            k: df.dropna(axis=1, how="all").dropna(axis=0, how="any")
...
def plotMatMetric(res, base, func, title):
    ...
    resPlot = pd.DataFrame(
        [func(x, base) for k, x in res.items()], index=list(res.keys())
    )
    ...
    plt.pcolormesh(np.array(sub), cmap="PiYG_r", edgecolors="w", linewidths=6)

    # annotate the color swatches with metric values
    for x in np.arange(ncol):
        for y in np.arange(nrow):
            # print x,y
            if 0.4 < sub.iloc[y, x] < 0.6:
                xcolor = "black"
            else:
                xcolor = "white"
            plt.annotate(
                "{:4.2f}".format(sub_val.iloc[y, x]),
                xy=(x + 0.5, y + 0.5),
    ...
    plt.yticks(
        np.arange(0.5, nrow + 1, 1), sub.index, fontsize=12, weight="bold"
    )

    # label ticks with configuration names
    plt.xticks(
        np.arange(0.5, ncol + 1, 1), sub.columns, fontsize=14, weight="bold"
    )
...
def plotMatMetricX(res_panel, func, title):
    res_comp = pd.Panel(
        {x: res_panel[x] for x in list(res_panel.keys()) if not x == "base"}
    )
...
def plotBarScore(res_score):
    ...
    ax.barh(
        y_pos[mask_better],
        res_score[mask_better],
        align="center",
        color="green",
    )
    ax.barh(
        y_pos[mask_worse], res_score[mask_worse], align="center", color="gray"
    )
    ...
    # ax.invert_yaxis()
    ax.set_xlabel(
        "Performance Score (larger scores indicate better performance)",
        fontsize=12,
        weight="bold",
    )
...
def benchmarkSUEWS(fn_nml, plugin_dir):
    ...
    res_metric = {
        f: pd.DataFrame(
            [
                list_func[f](data[x], data["base"])
                for x in list(data.keys())
                if not x == "base"
            ],
            index=[x for x in list(data.keys()) if not x == "base"],
        ).dropna(axis=1)
    ...
    fig_metric = {
        name_func: plotMatMetricX(data, list_func[name_func], name_func)
